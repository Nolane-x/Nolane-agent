...
test('0.0.0 release platform truth is machine-readable and fail-closed per platform', async () => {
  ...
  assert.equal(windows.updateTrust.enabled, false);
  assert.equal(windows.inAppUpdateHandoff.enabled, false);
  assert.match(windows.inAppUpdateHandoff.reason, /signing|evidence|verified/i);
  ...
  assert.equal(mac.updateTrust.enabled, false);
  ...
  assert.equal(linux.updateTrust.enabled, false);
  ...
  for (const platform of Object.values(table.platforms)) {
    ...
    assert.ok(['verified', 'external-gate', 'unknown', 'blocked', 'deferred'].includes(platform.recovery.realUpdateReplay));
  }
});
