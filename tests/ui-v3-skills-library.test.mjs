...
test('skill library CSS uses semantic tokens and collapses the catalogue split on narrow screens', async () => {
  ...
  assert.match(styles, /\.skill-library-item__body em\{[^}]*var\(--text-secondary\)/);
  ...
  assert.match(styles, /\.skills-library__preview header>span\{[^}]*var\(--text-secondary\)/);
  assert.match(styles, /\.skills-library__provenance dt\{[^}]*var\(--text-secondary\)/);
