...
test('external gate workflow aggregates three PR runner receipts into a read-only certification candidate artifact', async () => {
  ...
  assert.match(workflow, /if:\s*\$\{\{\s*github\.event_name\s*==\s*'pull_request'\s*&&\s*needs\.runner-evidence\.result\s*==\s*'success'\s*\}\}/);
