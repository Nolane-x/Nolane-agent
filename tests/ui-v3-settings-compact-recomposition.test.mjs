...
test('final responsive authority structurally recomposes Settings at the 640 evidence viewport', async () => {
  const css = await readFile(new URL('../ui-v3/styles/responsive.css', import.meta.url), 'utf8');
  assert.match(css, /@media \(max-width:720px\)\{[\s\S]*?\.settings-center\{[^}]*grid-template-columns:minmax\(0,1fr\)[^}]*grid-template-rows:auto minmax\(0,1fr\)/);
