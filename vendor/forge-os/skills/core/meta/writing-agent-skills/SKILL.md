...
description: "Use when writing agent skills is required during meta work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **writing agent skills**. Its focus is writing agent skills within meta lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by writing agent skills.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to writing agent skills.
3. Apply writing agent skills only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if writing agent skills were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by writing agent skills.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to writing agent skills.
5. Apply writing agent skills only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if writing agent skills were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for writing agent skills explicit and bounded?
...
- `writing-agent-skills-decision-table`
- `writing-agent-skills-verification-report`
- `writing-agent-skills-handoff-envelope`
...
- The output directly and completely performs writing agent skills within its declared boundary.
- Does the artifact make the owned decision for writing agent skills explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating writing agent skills as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `writing-agent-skills-decision-table`, `writing-agent-skills-verification-report`, `writing-agent-skills-handoff-envelope`.
