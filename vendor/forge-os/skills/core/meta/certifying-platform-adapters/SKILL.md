---
name: certifying-platform-adapters
description: "Use when certifying platform adapters is required during meta work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Certifying Platform Adapters

## Overview

This skill owns one bounded responsibility: **certifying platform adapters**. Its focus is certifying platform adapters within meta lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by certifying platform adapters.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to certifying platform adapters.
3. Apply certifying platform adapters only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if certifying platform adapters were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by certifying platform adapters.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to certifying platform adapters.
5. Apply certifying platform adapters only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if certifying platform adapters were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for certifying platform adapters explicit and bounded?
...
- `certifying-platform-adapters-decision-table`
- `certifying-platform-adapters-verification-report`
- `certifying-platform-adapters-handoff-envelope`
...
- The output directly and completely performs certifying platform adapters within its declared boundary.
- Does the artifact make the owned decision for certifying platform adapters explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating certifying platform adapters as a naming exercise
...
- Next transition: the graph router selects a real consumer of `adapter-certification`.
- Required evidence: `contract-validation`, `independent-review`, `certifying-platform-adapters-decision-table`, `certifying-platform-adapters-verification-report`, `certifying-platform-adapters-handoff-envelope`.
