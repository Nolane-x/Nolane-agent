---
name: selecting-model-provider
description: "Use when selecting model provider is required during meta work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **selecting model provider**. Its focus is selecting model provider within meta lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by selecting model provider.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to selecting model provider.
3. Apply selecting model provider only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if selecting model provider were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by selecting model provider.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to selecting model provider.
5. Apply selecting model provider only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if selecting model provider were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for selecting model provider explicit and bounded?
...
- `selecting-model-provider-decision-table`
- `selecting-model-provider-verification-report`
- `selecting-model-provider-handoff-envelope`
...
- The output directly and completely performs selecting model provider within its declared boundary.
- Does the artifact make the owned decision for selecting model provider explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating selecting model provider as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `selecting-model-provider-decision-table`, `selecting-model-provider-verification-report`, `selecting-model-provider-handoff-envelope`.
