---
name: compressing-project-artifacts
description: "Use when compressing project artifacts is required during meta work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Compressing Project Artifacts

## Overview

This skill owns one bounded responsibility: **compressing project artifacts**. Its focus is compressing project artifacts within meta lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by compressing project artifacts.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to compressing project artifacts.
3. Apply compressing project artifacts only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if compressing project artifacts were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by compressing project artifacts.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to compressing project artifacts.
5. Apply compressing project artifacts only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if compressing project artifacts were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for compressing project artifacts explicit and bounded?
...
- `compressing-project-artifacts-decision-table`
- `compressing-project-artifacts-verification-report`
- `compressing-project-artifacts-handoff-envelope`
...
- The output directly and completely performs compressing project artifacts within its declared boundary.
- Does the artifact make the owned decision for compressing project artifacts explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating compressing project artifacts as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `compressing-project-artifacts-decision-table`, `compressing-project-artifacts-verification-report`, `compressing-project-artifacts-handoff-envelope`.
