---
name: preventing-duplicate-abstractions
description: "Use when preventing duplicate abstractions is required during implementation work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Preventing Duplicate Abstractions

## Overview

This skill owns one bounded responsibility: **preventing duplicate abstractions**. Its focus is preventing duplicate abstractions within implementation lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by preventing duplicate abstractions.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to preventing duplicate abstractions.
3. Apply preventing duplicate abstractions only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if preventing duplicate abstractions were incomplete or implemented incorrectly.

## Procedure

1. Load one task, its direct contracts, and its failing test.
2. Verify the test fails for the intended missing behavior.
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by preventing duplicate abstractions.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to preventing duplicate abstractions.
5. Apply preventing duplicate abstractions only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if preventing duplicate abstractions were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for preventing duplicate abstractions explicit and bounded?
...
- `preventing-duplicate-abstractions-decision-table`
- `preventing-duplicate-abstractions-verification-report`
- `preventing-duplicate-abstractions-handoff-envelope`
...
- The output directly and completely performs preventing duplicate abstractions within its declared boundary.
- Does the artifact make the owned decision for preventing duplicate abstractions explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating preventing duplicate abstractions as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `preventing-duplicate-abstractions-decision-table`, `preventing-duplicate-abstractions-verification-report`, `preventing-duplicate-abstractions-handoff-envelope`.
