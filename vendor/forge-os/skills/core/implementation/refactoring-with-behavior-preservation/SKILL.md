---
name: refactoring-with-behavior-preservation
description: "Use when refactoring with behavior preservation is required during implementation work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **refactoring with behavior preservation**. Its focus is refactoring with behavior preservation within implementation lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by refactoring with behavior preservation.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to refactoring with behavior preservation.
3. Apply refactoring with behavior preservation only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if refactoring with behavior preservation were incomplete or implemented incorrectly.

## Procedure

1. Load one task, its direct contracts, and its failing test.
2. Verify the test fails for the intended missing behavior.
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by refactoring with behavior preservation.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to refactoring with behavior preservation.
5. Apply refactoring with behavior preservation only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if refactoring with behavior preservation were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for refactoring with behavior preservation explicit and bounded?
...
- `refactoring-with-behavior-preservation-decision-table`
- `refactoring-with-behavior-preservation-verification-report`
- `refactoring-with-behavior-preservation-handoff-envelope`
...
- The output directly and completely performs refactoring with behavior preservation within its declared boundary.
- Does the artifact make the owned decision for refactoring with behavior preservation explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating refactoring with behavior preservation as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `refactoring-with-behavior-preservation-decision-table`, `refactoring-with-behavior-preservation-verification-report`, `refactoring-with-behavior-preservation-handoff-envelope`.
