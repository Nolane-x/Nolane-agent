...
description: "Use when controlling code complexity is required during implementation work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **controlling code complexity**. Its focus is controlling code complexity within implementation lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by controlling code complexity.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to controlling code complexity.
5. Apply controlling code complexity only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if controlling code complexity were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for controlling code complexity explicit and bounded?
...
- `controlling-code-complexity-decision-table`
- `controlling-code-complexity-verification-report`
- `controlling-code-complexity-handoff-envelope`
...
- The output directly and completely performs controlling code complexity within its declared boundary.
- Does the artifact make the owned decision for controlling code complexity explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating controlling code complexity as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `controlling-code-complexity-decision-table`, `controlling-code-complexity-verification-report`, `controlling-code-complexity-handoff-envelope`.
