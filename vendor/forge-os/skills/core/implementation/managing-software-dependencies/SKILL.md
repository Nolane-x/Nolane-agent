---
name: managing-software-dependencies
description: "Use when managing software dependencies is required during implementation work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **managing software dependencies**. Its focus is managing software dependencies within implementation lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by managing software dependencies.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to managing software dependencies.
5. Apply managing software dependencies only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if managing software dependencies were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for managing software dependencies explicit and bounded?
...
- `managing-software-dependencies-decision-table`
- `managing-software-dependencies-verification-report`
- `managing-software-dependencies-handoff-envelope`
...
- The output directly and completely performs managing software dependencies within its declared boundary.
- Does the artifact make the owned decision for managing software dependencies explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating managing software dependencies as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `managing-software-dependencies-decision-table`, `managing-software-dependencies-verification-report`, `managing-software-dependencies-handoff-envelope`.
