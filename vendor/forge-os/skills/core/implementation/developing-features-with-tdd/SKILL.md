...
description: "Use when developing features with tdd is required during implementation work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **developing features with tdd**. Its focus is implement one externally observable feature through a verified red-green-refactor cycle. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Write the smallest behavior test from the acceptance contract before production code.
2. Run it and confirm it fails for the intended missing behavior rather than setup or syntax.
3. Implement only enough behavior to pass, then run the focused and relevant regression suites.
4. Refactor only while all tests remain green and no behavior changes.
5. Record the red output, green output, diff review, and remaining risks.
...
- Was the failure observed before implementation?
- Does the test assert behavior rather than mocks or internals?
- Did implementation add anything not demanded by the test?
- Are warnings and unrelated regressions zero?
...
- The output directly and completely performs developing features with tdd within its declared boundary.
- Was the failure observed before implementation?
- Does the test assert behavior rather than mocks or internals?
- Did implementation add anything not demanded by the test?
- Are warnings and unrelated regressions zero?
...
- broad mocks hiding integration behavior
- speculative options and abstractions
...
- Required evidence: `contract-validation`, `independent-review`, `red test log`, `green test log`, `diff review`, `commit hash`.
