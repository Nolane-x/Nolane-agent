---
name: testing-rate-limits
description: "Use when testing rate limits is required during security work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **testing rate limits**. Its focus is testing rate limits within security lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing rate limits.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing rate limits.
3. Apply testing rate limits only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if testing rate limits were incomplete or implemented incorrectly.

## Procedure

1. Define assets, trust boundaries, actors, and abuse objectives.
2. Map the targeted control or attack surface.
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing rate limits.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing rate limits.
5. Apply testing rate limits only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if testing rate limits were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for testing rate limits explicit and bounded?
...
- `testing-rate-limits-decision-table`
- `testing-rate-limits-verification-report`
- `testing-rate-limits-handoff-envelope`
...
- The output directly and completely performs testing rate limits within its declared boundary.
- Does the artifact make the owned decision for testing rate limits explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating testing rate limits as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `testing-rate-limits-decision-table`, `testing-rate-limits-verification-report`, `testing-rate-limits-handoff-envelope`.
