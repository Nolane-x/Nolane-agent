---
name: testing-authentication-authorization
description: "Use when testing authentication authorization is required during security work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **testing authentication authorization**. Its focus is testing authentication authorization within security lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing authentication authorization.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing authentication authorization.
3. Apply testing authentication authorization only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if testing authentication authorization were incomplete or implemented incorrectly.

## Procedure

1. Define assets, trust boundaries, actors, and abuse objectives.
2. Map the targeted control or attack surface.
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing authentication authorization.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing authentication authorization.
5. Apply testing authentication authorization only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if testing authentication authorization were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for testing authentication authorization explicit and bounded?
...
- `testing-authentication-authorization-decision-table`
- `testing-authentication-authorization-verification-report`
- `testing-authentication-authorization-handoff-envelope`
...
- The output directly and completely performs testing authentication authorization within its declared boundary.
- Does the artifact make the owned decision for testing authentication authorization explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating testing authentication authorization as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `testing-authentication-authorization-decision-table`, `testing-authentication-authorization-verification-report`, `testing-authentication-authorization-handoff-envelope`.
