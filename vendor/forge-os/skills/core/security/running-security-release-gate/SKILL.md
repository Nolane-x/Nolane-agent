---
name: running-security-release-gate
description: "Use when running security release gate is required during security work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Running Security Release Gate

## Overview

This skill owns one bounded responsibility: **running security release gate**. Its focus is make a release decision from unresolved findings, required controls, test evidence, provenance, and explicit residual-risk ownership. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Resolve the assurance profile and enumerate mandatory security evidence.
2. Verify every critical and high finding against closure evidence rather than status labels.
3. Check dependency, secret, authorization, tenant, prompt injection, tool abuse, rollback, and incident readiness evidence.
4. Reject aggregate scores that hide a critical blocker.
5. Emit pass, fail, or blocked with exact rule IDs, remediation, owner, and evidence hashes.

## Procedure

1. Define assets, trust boundaries, actors, and abuse objectives.
2. Map the targeted control or attack surface.
3. Resolve the assurance profile and enumerate mandatory security evidence.
4. Verify every critical and high finding against closure evidence rather than status labels.
5. Check dependency, secret, authorization, tenant, prompt injection, tool abuse, rollback, and incident readiness evidence.
6. Reject aggregate scores that hide a critical blocker.
7. Emit pass, fail, or blocked with exact rule IDs, remediation, owner, and evidence hashes.
...
- Are any findings accepted without exact human confirmation?
- Do evidence hashes match the release candidate?
- Can the build be reproduced and rolled back?
- Are monitoring and incident owners ready before exposure?
...
- The output directly and completely performs running security release gate within its declared boundary.
- Are any findings accepted without exact human confirmation?
- Do evidence hashes match the release candidate?
- Can the build be reproduced and rolled back?
- Are monitoring and incident owners ready before exposure?
...
- waiving findings in chat
- using stale scans
- approving a different build than the one tested
...
- Next transition: the graph router selects a real consumer of `security-release-decision`.
- Required evidence: `contract-validation`, `independent-review`, `security gate result`, `finding closure matrix`, `release hash manifest`, `residual-risk acceptance`.
