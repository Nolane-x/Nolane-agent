---
name: maintaining-prior-art-map
description: "Use when maintaining prior art map is required during research work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Maintaining Prior Art Map

## Overview

This skill owns one bounded responsibility: **maintaining prior art map**. Its focus is maintaining prior art map within research lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by maintaining prior art map.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to maintaining prior art map.
3. Apply maintaining prior art map only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if maintaining prior art map were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by maintaining prior art map.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to maintaining prior art map.
5. Apply maintaining prior art map only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if maintaining prior art map were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for maintaining prior art map explicit and bounded?
...
- `maintaining-prior-art-map-decision-table`
- `maintaining-prior-art-map-verification-report`
- `maintaining-prior-art-map-handoff-envelope`
...
- The output directly and completely performs maintaining prior art map within its declared boundary.
- Does the artifact make the owned decision for maintaining prior art map explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating maintaining prior art map as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `maintaining-prior-art-map-decision-table`, `maintaining-prior-art-map-verification-report`, `maintaining-prior-art-map-handoff-envelope`.
