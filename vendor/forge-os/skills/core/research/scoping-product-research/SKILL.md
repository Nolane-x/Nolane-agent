---
name: scoping-product-research
description: "Use when scoping product research is required during research work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Scoping Product Research

## Overview

This skill owns one bounded responsibility: **scoping product research**. Its focus is scoping product research within research lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by scoping product research.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to scoping product research.
3. Apply scoping product research only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if scoping product research were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by scoping product research.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to scoping product research.
5. Apply scoping product research only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if scoping product research were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for scoping product research explicit and bounded?
...
- `scoping-product-research-decision-table`
- `scoping-product-research-verification-report`
- `scoping-product-research-handoff-envelope`
...
- The output directly and completely performs scoping product research within its declared boundary.
- Does the artifact make the owned decision for scoping product research explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating scoping product research as a naming exercise
...
- Next transition: the graph router selects a real consumer of `research-questions`, `problem-discovery`.
- Required evidence: `contract-validation`, `independent-review`, `scoping-product-research-decision-table`, `scoping-product-research-verification-report`, `scoping-product-research-handoff-envelope`.
