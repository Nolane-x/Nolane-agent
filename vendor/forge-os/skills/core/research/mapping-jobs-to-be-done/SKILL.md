---
name: mapping-jobs-to-be-done
description: "Use when mapping jobs to be done is required during research work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **mapping jobs to be done**. Its focus is mapping jobs to be done within research lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by mapping jobs to be done.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to mapping jobs to be done.
3. Apply mapping jobs to be done only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if mapping jobs to be done were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by mapping jobs to be done.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to mapping jobs to be done.
5. Apply mapping jobs to be done only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if mapping jobs to be done were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for mapping jobs to be done explicit and bounded?
...
- `mapping-jobs-to-be-done-decision-table`
- `mapping-jobs-to-be-done-verification-report`
- `mapping-jobs-to-be-done-handoff-envelope`
...
- The output directly and completely performs mapping jobs to be done within its declared boundary.
- Does the artifact make the owned decision for mapping jobs to be done explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating mapping jobs to be done as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `mapping-jobs-to-be-done-decision-table`, `mapping-jobs-to-be-done-verification-report`, `mapping-jobs-to-be-done-handoff-envelope`.
