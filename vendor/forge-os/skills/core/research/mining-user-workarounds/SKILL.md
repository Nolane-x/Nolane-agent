---
name: mining-user-workarounds
description: "Use when mining user workarounds is required during research work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Mining User Workarounds

## Overview

This skill owns one bounded responsibility: **mining user workarounds**. Its focus is mining user workarounds within research lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by mining user workarounds.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to mining user workarounds.
3. Apply mining user workarounds only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if mining user workarounds were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by mining user workarounds.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to mining user workarounds.
5. Apply mining user workarounds only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if mining user workarounds were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for mining user workarounds explicit and bounded?
...
- `mining-user-workarounds-decision-table`
- `mining-user-workarounds-verification-report`
- `mining-user-workarounds-handoff-envelope`
...
- The output directly and completely performs mining user workarounds within its declared boundary.
- Does the artifact make the owned decision for mining user workarounds explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating mining user workarounds as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `mining-user-workarounds-decision-table`, `mining-user-workarounds-verification-report`, `mining-user-workarounds-handoff-envelope`.
