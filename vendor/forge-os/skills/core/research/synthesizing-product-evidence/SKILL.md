---
name: synthesizing-product-evidence
description: "Use when synthesizing product evidence is required during research work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Synthesizing Product Evidence

## Overview

This skill owns one bounded responsibility: **synthesizing product evidence**. Its focus is synthesizing product evidence within research lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by synthesizing product evidence.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to synthesizing product evidence.
3. Apply synthesizing product evidence only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if synthesizing product evidence were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by synthesizing product evidence.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to synthesizing product evidence.
5. Apply synthesizing product evidence only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if synthesizing product evidence were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for synthesizing product evidence explicit and bounded?
...
- `synthesizing-product-evidence-decision-table`
- `synthesizing-product-evidence-verification-report`
- `synthesizing-product-evidence-handoff-envelope`
...
- The output directly and completely performs synthesizing product evidence within its declared boundary.
- Does the artifact make the owned decision for synthesizing product evidence explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating synthesizing product evidence as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `synthesizing-product-evidence-decision-table`, `synthesizing-product-evidence-verification-report`, `synthesizing-product-evidence-handoff-envelope`.
