---
name: identifying-market-shifts
description: "Use when identifying market shifts is required during research work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **identifying market shifts**. Its focus is identifying market shifts within research lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by identifying market shifts.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to identifying market shifts.
3. Apply identifying market shifts only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if identifying market shifts were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by identifying market shifts.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to identifying market shifts.
5. Apply identifying market shifts only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if identifying market shifts were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for identifying market shifts explicit and bounded?
...
- `identifying-market-shifts-decision-table`
- `identifying-market-shifts-verification-report`
- `identifying-market-shifts-handoff-envelope`
...
- The output directly and completely performs identifying market shifts within its declared boundary.
- Does the artifact make the owned decision for identifying market shifts explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating identifying market shifts as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `identifying-market-shifts-decision-table`, `identifying-market-shifts-verification-report`, `identifying-market-shifts-handoff-envelope`.
