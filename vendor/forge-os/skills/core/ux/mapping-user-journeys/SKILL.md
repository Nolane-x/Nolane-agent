...
description: "Use when mapping user journeys is required during ux work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **mapping user journeys**. Its focus is mapping user journeys within ux lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by mapping user journeys.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to mapping user journeys.
3. Apply mapping user journeys only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if mapping user journeys were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by mapping user journeys.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to mapping user journeys.
5. Apply mapping user journeys only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if mapping user journeys were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for mapping user journeys explicit and bounded?
...
- `mapping-user-journeys-decision-table`
- `mapping-user-journeys-verification-report`
- `mapping-user-journeys-handoff-envelope`
...
- The output directly and completely performs mapping user journeys within its declared boundary.
- Does the artifact make the owned decision for mapping user journeys explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating mapping user journeys as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `mapping-user-journeys-decision-table`, `mapping-user-journeys-verification-report`, `mapping-user-journeys-handoff-envelope`.
