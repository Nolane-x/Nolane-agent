---
name: designing-interface-system
description: "Use when designing interface system is required during ux work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Designing Interface System

## Overview

This skill owns one bounded responsibility: **designing interface system**. Its focus is designing interface system within ux lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing interface system.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing interface system.
3. Apply designing interface system only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing interface system were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing interface system.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing interface system.
5. Apply designing interface system only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing interface system were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing interface system explicit and bounded?
...
- `designing-interface-system-decision-table`
- `designing-interface-system-verification-report`
- `designing-interface-system-handoff-envelope`
...
- The output directly and completely performs designing interface system within its declared boundary.
- Does the artifact make the owned decision for designing interface system explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing interface system as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-interface-system-decision-table`, `designing-interface-system-verification-report`, `designing-interface-system-handoff-envelope`.
