---
name: building-evidence-backed-personas
description: "Use when building evidence backed personas is required during ux work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **building evidence backed personas**. Its focus is building evidence backed personas within ux lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by building evidence backed personas.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to building evidence backed personas.
3. Apply building evidence backed personas only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if building evidence backed personas were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by building evidence backed personas.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to building evidence backed personas.
5. Apply building evidence backed personas only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if building evidence backed personas were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for building evidence backed personas explicit and bounded?
...
- `building-evidence-backed-personas-decision-table`
- `building-evidence-backed-personas-verification-report`
- `building-evidence-backed-personas-handoff-envelope`
...
- The output directly and completely performs building evidence backed personas within its declared boundary.
- Does the artifact make the owned decision for building evidence backed personas explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating building evidence backed personas as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `building-evidence-backed-personas-decision-table`, `building-evidence-backed-personas-verification-report`, `building-evidence-backed-personas-handoff-envelope`.
