---
name: designing-responsive-experiences
description: "Use when designing responsive experiences is required during ux work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **designing responsive experiences**. Its focus is designing responsive experiences within ux lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing responsive experiences.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing responsive experiences.
3. Apply designing responsive experiences only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing responsive experiences were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing responsive experiences.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing responsive experiences.
5. Apply designing responsive experiences only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing responsive experiences were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing responsive experiences explicit and bounded?
...
- `designing-responsive-experiences-decision-table`
- `designing-responsive-experiences-verification-report`
- `designing-responsive-experiences-handoff-envelope`
...
- The output directly and completely performs designing responsive experiences within its declared boundary.
- Does the artifact make the owned decision for designing responsive experiences explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing responsive experiences as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-responsive-experiences-decision-table`, `designing-responsive-experiences-verification-report`, `designing-responsive-experiences-handoff-envelope`.
