---
name: managing-change-impact
description: "Use when managing change impact is required during planning work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **managing change impact**. Its focus is managing change impact within planning lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by managing change impact.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to managing change impact.
3. Apply managing change impact only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if managing change impact were incomplete or implemented incorrectly.

## Procedure

1. Read verified artifacts and acceptance contracts.
2. Slice work into independently reviewable vertical increments.
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by managing change impact.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to managing change impact.
5. Apply managing change impact only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if managing change impact were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for managing change impact explicit and bounded?
...
- `managing-change-impact-decision-table`
- `managing-change-impact-verification-report`
- `managing-change-impact-handoff-envelope`
...
- The output directly and completely performs managing change impact within its declared boundary.
- Does the artifact make the owned decision for managing change impact explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating managing change impact as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `managing-change-impact-decision-table`, `managing-change-impact-verification-report`, `managing-change-impact-handoff-envelope`.
