---
name: assigning-agent-work
description: "Use when assigning agent work is required during planning work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Assigning Agent Work

## Overview

This skill owns one bounded responsibility: **assigning agent work**. Its focus is assigning agent work within planning lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by assigning agent work.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to assigning agent work.
5. Apply assigning agent work only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if assigning agent work were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for assigning agent work explicit and bounded?
...
- `assigning-agent-work-decision-table`
- `assigning-agent-work-verification-report`
- `assigning-agent-work-handoff-envelope`
...
- The output directly and completely performs assigning agent work within its declared boundary.
- Does the artifact make the owned decision for assigning agent work explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating assigning agent work as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `assigning-agent-work-decision-table`, `assigning-agent-work-verification-report`, `assigning-agent-work-handoff-envelope`.
