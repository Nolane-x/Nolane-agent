---
name: writing-executable-plans
description: "Use when writing executable plans is required during planning work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **writing executable plans**. Its focus is produce a dependency-aware sequence of vertical increments that a fresh agent can execute and independently verify. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Map verified requirements and acceptance contracts to concrete files, interfaces, migrations, and tests.
2. Split work where a reviewer could approve one deliverable and reject the next.
3. For each task, write a failing test, expected failure, minimal implementation, focused verification, regression command, and commit boundary.
4. Declare exact consumed and produced interfaces so tasks can be executed out of conversational order.
5. Run a coverage review against the specification and remove placeholders, speculative work, and inconsistent names.
...
- Can an agent with no prior chat execute each task?
- Does every requirement map to a task and acceptance test?
- Does every task end in an independently testable artifact?
- Are exact paths, signatures, commands, and expected outputs present?
...
- The output directly and completely performs writing executable plans within its declared boundary.
- Can an agent with no prior chat execute each task?
- Does every requirement map to a task and acceptance test?
- Does every task end in an independently testable artifact?
- Are exact paths, signatures, commands, and expected outputs present?
...
- “implement as needed”
- horizontal layer tasks with no user value
- tests deferred to the end
...
- Required evidence: `contract-validation`, `independent-review`, `implementation plan`, `requirement-to-task matrix`, `dependency DAG`, `plan self-review`.
