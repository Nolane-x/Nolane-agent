---
name: tracking-evidence-backed-progress
description: "Use when tracking evidence backed progress is required during planning work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Tracking Evidence Backed Progress

## Overview

This skill owns one bounded responsibility: **tracking evidence backed progress**. Its focus is tracking evidence backed progress within planning lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by tracking evidence backed progress.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to tracking evidence backed progress.
3. Apply tracking evidence backed progress only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if tracking evidence backed progress were incomplete or implemented incorrectly.

## Procedure

1. Read verified artifacts and acceptance contracts.
2. Slice work into independently reviewable vertical increments.
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by tracking evidence backed progress.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to tracking evidence backed progress.
5. Apply tracking evidence backed progress only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if tracking evidence backed progress were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for tracking evidence backed progress explicit and bounded?
...
- `tracking-evidence-backed-progress-decision-table`
- `tracking-evidence-backed-progress-verification-report`
- `tracking-evidence-backed-progress-handoff-envelope`
...
- The output directly and completely performs tracking evidence backed progress within its declared boundary.
- Does the artifact make the owned decision for tracking evidence backed progress explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating tracking evidence backed progress as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `tracking-evidence-backed-progress-decision-table`, `tracking-evidence-backed-progress-verification-report`, `tracking-evidence-backed-progress-handoff-envelope`.
