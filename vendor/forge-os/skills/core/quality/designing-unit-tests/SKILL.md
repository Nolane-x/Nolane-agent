...
description: "Use when designing unit tests is required during quality work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **designing unit tests**. Its focus is designing unit tests within quality lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing unit tests.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing unit tests.
3. Apply designing unit tests only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing unit tests were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing unit tests.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing unit tests.
5. Apply designing unit tests only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing unit tests were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing unit tests explicit and bounded?
...
- `designing-unit-tests-decision-table`
- `designing-unit-tests-verification-report`
- `designing-unit-tests-handoff-envelope`
...
- The output directly and completely performs designing unit tests within its declared boundary.
- Does the artifact make the owned decision for designing unit tests explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing unit tests as a naming exercise
...
- Next transition: the graph router selects a real consumer of `test-plan`.
- Required evidence: `contract-validation`, `independent-review`, `designing-unit-tests-decision-table`, `designing-unit-tests-verification-report`, `designing-unit-tests-handoff-envelope`.
