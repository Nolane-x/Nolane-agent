---
name: testing-performance-budgets
description: "Use when testing performance budgets is required during quality work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **testing performance budgets**. Its focus is testing performance budgets within quality lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing performance budgets.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing performance budgets.
3. Apply testing performance budgets only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if testing performance budgets were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing performance budgets.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing performance budgets.
5. Apply testing performance budgets only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if testing performance budgets were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for testing performance budgets explicit and bounded?
...
- `testing-performance-budgets-decision-table`
- `testing-performance-budgets-verification-report`
- `testing-performance-budgets-handoff-envelope`
...
- The output directly and completely performs testing performance budgets within its declared boundary.
- Does the artifact make the owned decision for testing performance budgets explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating testing performance budgets as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `testing-performance-budgets-decision-table`, `testing-performance-budgets-verification-report`, `testing-performance-budgets-handoff-envelope`.
