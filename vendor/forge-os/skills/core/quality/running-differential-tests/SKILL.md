...
description: "Use when running differential tests is required during quality work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **running differential tests**. Its focus is running differential tests within quality lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by running differential tests.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to running differential tests.
3. Apply running differential tests only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if running differential tests were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by running differential tests.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to running differential tests.
5. Apply running differential tests only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if running differential tests were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for running differential tests explicit and bounded?
...
- `running-differential-tests-decision-table`
- `running-differential-tests-verification-report`
- `running-differential-tests-handoff-envelope`
...
- The output directly and completely performs running differential tests within its declared boundary.
- Does the artifact make the owned decision for running differential tests explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating running differential tests as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `running-differential-tests-decision-table`, `running-differential-tests-verification-report`, `running-differential-tests-handoff-envelope`.
