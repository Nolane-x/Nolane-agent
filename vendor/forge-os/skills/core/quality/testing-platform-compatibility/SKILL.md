---
name: testing-platform-compatibility
description: "Use when testing platform compatibility is required during quality work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **testing platform compatibility**. Its focus is testing platform compatibility within quality lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing platform compatibility.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing platform compatibility.
3. Apply testing platform compatibility only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if testing platform compatibility were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing platform compatibility.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing platform compatibility.
5. Apply testing platform compatibility only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if testing platform compatibility were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for testing platform compatibility explicit and bounded?
...
- `testing-platform-compatibility-decision-table`
- `testing-platform-compatibility-verification-report`
- `testing-platform-compatibility-handoff-envelope`
...
- The output directly and completely performs testing platform compatibility within its declared boundary.
- Does the artifact make the owned decision for testing platform compatibility explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating testing platform compatibility as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `testing-platform-compatibility-decision-table`, `testing-platform-compatibility-verification-report`, `testing-platform-compatibility-handoff-envelope`.
