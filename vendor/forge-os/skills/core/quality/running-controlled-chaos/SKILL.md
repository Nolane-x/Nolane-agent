---
name: running-controlled-chaos
description: "Use when running controlled chaos is required during quality work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Running Controlled Chaos

## Overview

This skill owns one bounded responsibility: **running controlled chaos**. Its focus is running controlled chaos within quality lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by running controlled chaos.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to running controlled chaos.
3. Apply running controlled chaos only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if running controlled chaos were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by running controlled chaos.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to running controlled chaos.
5. Apply running controlled chaos only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if running controlled chaos were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for running controlled chaos explicit and bounded?
...
- `running-controlled-chaos-decision-table`
- `running-controlled-chaos-verification-report`
- `running-controlled-chaos-handoff-envelope`
...
- The output directly and completely performs running controlled chaos within its declared boundary.
- Does the artifact make the owned decision for running controlled chaos explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating running controlled chaos as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `running-controlled-chaos-decision-table`, `running-controlled-chaos-verification-report`, `running-controlled-chaos-handoff-envelope`.
