---
name: triaging-production-incidents
description: "Use when triaging production incidents is required during operations work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **triaging production incidents**. Its focus is triaging production incidents within operations lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by triaging production incidents.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to triaging production incidents.
3. Apply triaging production incidents only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if triaging production incidents were incomplete or implemented incorrectly.

## Procedure

1. Read deployment topology, service objectives, and rollback constraints.
2. Define observable success and failure signals.
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by triaging production incidents.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to triaging production incidents.
5. Apply triaging production incidents only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if triaging production incidents were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for triaging production incidents explicit and bounded?
...
- `triaging-production-incidents-decision-table`
- `triaging-production-incidents-verification-report`
- `triaging-production-incidents-handoff-envelope`
...
- The output directly and completely performs triaging production incidents within its declared boundary.
- Does the artifact make the owned decision for triaging production incidents explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating triaging production incidents as a naming exercise
...
- Next transition: the graph router selects a real consumer of `incident-report`.
- Required evidence: `contract-validation`, `independent-review`, `triaging-production-incidents-decision-table`, `triaging-production-incidents-verification-report`, `triaging-production-incidents-handoff-envelope`.
