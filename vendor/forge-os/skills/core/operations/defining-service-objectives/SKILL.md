...
description: "Use when defining service objectives is required during operations work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **defining service objectives**. Its focus is defining service objectives within operations lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by defining service objectives.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to defining service objectives.
5. Apply defining service objectives only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if defining service objectives were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for defining service objectives explicit and bounded?
...
- `defining-service-objectives-decision-table`
- `defining-service-objectives-verification-report`
- `defining-service-objectives-handoff-envelope`
...
- The output directly and completely performs defining service objectives within its declared boundary.
- Does the artifact make the owned decision for defining service objectives explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating defining service objectives as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `defining-service-objectives-decision-table`, `defining-service-objectives-verification-report`, `defining-service-objectives-handoff-envelope`.
