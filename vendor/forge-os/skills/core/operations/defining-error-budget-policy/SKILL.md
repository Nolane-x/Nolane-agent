...
description: "Use when defining error budget policy is required during operations work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **defining error budget policy**. Its focus is defining error budget policy within operations lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by defining error budget policy.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to defining error budget policy.
3. Apply defining error budget policy only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if defining error budget policy were incomplete or implemented incorrectly.

## Procedure

1. Read deployment topology, service objectives, and rollback constraints.
2. Define observable success and failure signals.
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by defining error budget policy.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to defining error budget policy.
5. Apply defining error budget policy only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if defining error budget policy were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for defining error budget policy explicit and bounded?
...
- `defining-error-budget-policy-decision-table`
- `defining-error-budget-policy-verification-report`
- `defining-error-budget-policy-handoff-envelope`
...
- The output directly and completely performs defining error budget policy within its declared boundary.
- Does the artifact make the owned decision for defining error budget policy explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating defining error budget policy as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `defining-error-budget-policy-decision-table`, `defining-error-budget-policy-verification-report`, `defining-error-budget-policy-handoff-envelope`.
