...
description: "Use when validating observability is required during operations work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **validating observability**. Its focus is validating observability within operations lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by validating observability.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to validating observability.
3. Apply validating observability only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if validating observability were incomplete or implemented incorrectly.

## Procedure

1. Read deployment topology, service objectives, and rollback constraints.
2. Define observable success and failure signals.
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by validating observability.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to validating observability.
5. Apply validating observability only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if validating observability were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for validating observability explicit and bounded?
...
- `validating-observability-decision-table`
- `validating-observability-verification-report`
- `validating-observability-handoff-envelope`
...
- The output directly and completely performs validating observability within its declared boundary.
- Does the artifact make the owned decision for validating observability explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating validating observability as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `validating-observability-decision-table`, `validating-observability-verification-report`, `validating-observability-handoff-envelope`.
