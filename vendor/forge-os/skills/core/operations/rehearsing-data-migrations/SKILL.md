---
name: rehearsing-data-migrations
description: "Use when rehearsing data migrations is required during operations work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Rehearsing Data Migrations

## Overview

This skill owns one bounded responsibility: **rehearsing data migrations**. Its focus is rehearsing data migrations within operations lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by rehearsing data migrations.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to rehearsing data migrations.
3. Apply rehearsing data migrations only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if rehearsing data migrations were incomplete or implemented incorrectly.

## Procedure

1. Read deployment topology, service objectives, and rollback constraints.
2. Define observable success and failure signals.
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by rehearsing data migrations.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to rehearsing data migrations.
5. Apply rehearsing data migrations only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if rehearsing data migrations were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for rehearsing data migrations explicit and bounded?
...
- `rehearsing-data-migrations-decision-table`
- `rehearsing-data-migrations-verification-report`
- `rehearsing-data-migrations-handoff-envelope`
...
- The output directly and completely performs rehearsing data migrations within its declared boundary.
- Does the artifact make the owned decision for rehearsing data migrations explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating rehearsing data migrations as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `rehearsing-data-migrations-decision-table`, `rehearsing-data-migrations-verification-report`, `rehearsing-data-migrations-handoff-envelope`.
