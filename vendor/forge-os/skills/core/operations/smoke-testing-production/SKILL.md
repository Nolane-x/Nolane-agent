---
name: smoke-testing-production
description: "Use when smoke testing production is required during operations work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **smoke testing production**. Its focus is smoke testing production within operations lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by smoke testing production.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to smoke testing production.
3. Apply smoke testing production only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if smoke testing production were incomplete or implemented incorrectly.

## Procedure

1. Read deployment topology, service objectives, and rollback constraints.
2. Define observable success and failure signals.
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by smoke testing production.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to smoke testing production.
5. Apply smoke testing production only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if smoke testing production were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for smoke testing production explicit and bounded?
...
- `smoke-testing-production-decision-table`
- `smoke-testing-production-verification-report`
- `smoke-testing-production-handoff-envelope`
...
- The output directly and completely performs smoke testing production within its declared boundary.
- Does the artifact make the owned decision for smoke testing production explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating smoke testing production as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `smoke-testing-production-decision-table`, `smoke-testing-production-verification-report`, `smoke-testing-production-handoff-envelope`.
