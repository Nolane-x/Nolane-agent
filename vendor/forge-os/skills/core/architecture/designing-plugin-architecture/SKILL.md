---
name: designing-plugin-architecture
description: "Use when designing plugin architecture is required during architecture work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **designing plugin architecture**. Its focus is designing plugin architecture within architecture lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing plugin architecture.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing plugin architecture.
3. Apply designing plugin architecture only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing plugin architecture were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing plugin architecture.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing plugin architecture.
5. Apply designing plugin architecture only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing plugin architecture were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing plugin architecture explicit and bounded?
...
- `designing-plugin-architecture-decision-table`
- `designing-plugin-architecture-verification-report`
- `designing-plugin-architecture-handoff-envelope`
...
- The output directly and completely performs designing plugin architecture within its declared boundary.
- Does the artifact make the owned decision for designing plugin architecture explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing plugin architecture as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-plugin-architecture-decision-table`, `designing-plugin-architecture-verification-report`, `designing-plugin-architecture-handoff-envelope`.
