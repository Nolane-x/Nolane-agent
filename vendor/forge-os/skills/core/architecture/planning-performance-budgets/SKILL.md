---
name: planning-performance-budgets
description: "Use when planning performance budgets is required during architecture work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **planning performance budgets**. Its focus is planning performance budgets within architecture lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by planning performance budgets.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to planning performance budgets.
3. Apply planning performance budgets only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if planning performance budgets were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by planning performance budgets.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to planning performance budgets.
5. Apply planning performance budgets only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if planning performance budgets were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for planning performance budgets explicit and bounded?
...
- `planning-performance-budgets-decision-table`
- `planning-performance-budgets-verification-report`
- `planning-performance-budgets-handoff-envelope`
...
- The output directly and completely performs planning performance budgets within its declared boundary.
- Does the artifact make the owned decision for planning performance budgets explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating planning performance budgets as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `planning-performance-budgets-decision-table`, `planning-performance-budgets-verification-report`, `planning-performance-budgets-handoff-envelope`.
