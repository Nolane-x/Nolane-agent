...
description: "Use when recovering interrupted runs is required during kernel work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **recovering interrupted runs**. Its focus is recovering interrupted runs within kernel lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by recovering interrupted runs.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to recovering interrupted runs.
3. Apply recovering interrupted runs only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if recovering interrupted runs were incomplete or implemented incorrectly.

## Procedure

1. Read the current project state and active gate.
2. Resolve only the minimum missing state required for routing.
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by recovering interrupted runs.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to recovering interrupted runs.
5. Apply recovering interrupted runs only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if recovering interrupted runs were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for recovering interrupted runs explicit and bounded?
...
- `recovering-interrupted-runs-decision-table`
- `recovering-interrupted-runs-verification-report`
- `recovering-interrupted-runs-handoff-envelope`
...
- The output directly and completely performs recovering interrupted runs within its declared boundary.
- Does the artifact make the owned decision for recovering interrupted runs explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating recovering interrupted runs as a naming exercise
...
- Next transition: the graph router selects a real consumer of `recovery-report`.
- Required evidence: `contract-validation`, `independent-review`, `recovering-interrupted-runs-decision-table`, `recovering-interrupted-runs-verification-report`, `recovering-interrupted-runs-handoff-envelope`.
