...
description: "Use when requesting human decisions is required during kernel work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **requesting human decisions**. Its focus is requesting human decisions within kernel lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by requesting human decisions.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to requesting human decisions.
5. Apply requesting human decisions only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if requesting human decisions were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for requesting human decisions explicit and bounded?
...
- `requesting-human-decisions-decision-table`
- `requesting-human-decisions-verification-report`
- `requesting-human-decisions-handoff-envelope`
...
- The output directly and completely performs requesting human decisions within its declared boundary.
- Does the artifact make the owned decision for requesting human decisions explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating requesting human decisions as a naming exercise
...
- Next transition: the graph router selects a real consumer of `approval-request`.
- Required evidence: `contract-validation`, `independent-review`, `requesting-human-decisions-decision-table`, `requesting-human-decisions-verification-report`, `requesting-human-decisions-handoff-envelope`.
