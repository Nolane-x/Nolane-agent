...
description: "Use when auditing agent actions is required during kernel work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **auditing agent actions**. Its focus is auditing agent actions within kernel lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by auditing agent actions.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to auditing agent actions.
3. Apply auditing agent actions only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if auditing agent actions were incomplete or implemented incorrectly.

## Procedure

1. Read the current project state and active gate.
2. Resolve only the minimum missing state required for routing.
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by auditing agent actions.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to auditing agent actions.
5. Apply auditing agent actions only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if auditing agent actions were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for auditing agent actions explicit and bounded?
...
- `auditing-agent-actions-decision-table`
- `auditing-agent-actions-verification-report`
- `auditing-agent-actions-handoff-envelope`
...
- The output directly and completely performs auditing agent actions within its declared boundary.
- Does the artifact make the owned decision for auditing agent actions explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating auditing agent actions as a naming exercise
...
- Next transition: the graph router selects a real consumer of `audit-record`.
- Required evidence: `contract-validation`, `independent-review`, `auditing-agent-actions-decision-table`, `auditing-agent-actions-verification-report`, `auditing-agent-actions-handoff-envelope`.
