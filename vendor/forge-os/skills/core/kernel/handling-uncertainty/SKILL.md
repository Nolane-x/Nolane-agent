...
description: "Use when handling uncertainty is required during kernel work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **handling uncertainty**. Its focus is handling uncertainty within kernel lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by handling uncertainty.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to handling uncertainty.
5. Apply handling uncertainty only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if handling uncertainty were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for handling uncertainty explicit and bounded?
...
- `handling-uncertainty-decision-table`
- `handling-uncertainty-verification-report`
- `handling-uncertainty-handoff-envelope`
...
- The output directly and completely performs handling uncertainty within its declared boundary.
- Does the artifact make the owned decision for handling uncertainty explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating handling uncertainty as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `handling-uncertainty-decision-table`, `handling-uncertainty-verification-report`, `handling-uncertainty-handoff-envelope`.
