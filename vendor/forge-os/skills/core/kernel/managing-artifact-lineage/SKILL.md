...
description: "Use when managing artifact lineage is required during kernel work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **managing artifact lineage**. Its focus is preserve immutable provenance, dependency edges, supersession, verification, and downstream invalidation. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Assign stable project-scoped IDs and canonical content hashes.
2. Record producing skill, agent identity, consumed artifacts, decisions, evidence, and residual risks.
3. Append new versions; never mutate previously verified content in place.
4. When upstream content changes, compute descendants and mark only affected artifacts invalidated.
5. Require independent verification before changing state to verified.
...
- Does the same canonical content produce the same hash?
- Can every output reconstruct its input lineage?
- Are unaffected branches preserved during invalidation?
- Can a worker verify its own artifact?
...
- The output directly and completely performs managing artifact lineage within its declared boundary.
- Does the same canonical content produce the same hash?
- Can every output reconstruct its input lineage?
- Are unaffected branches preserved during invalidation?
- Can a worker verify its own artifact?
...
- overwriting verified artifacts
- global invalidation
...
- Required evidence: `contract-validation`, `independent-review`, `lineage graph`, `hash manifest`, `invalidation report`, `verification record`.
