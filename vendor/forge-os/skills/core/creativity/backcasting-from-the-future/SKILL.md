---
name: backcasting-from-the-future
description: "Use when backcasting from the future is required during creativity work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **backcasting from the future**. Its focus is backcasting from the future within creativity lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by backcasting from the future.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to backcasting from the future.
3. Apply backcasting from the future only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if backcasting from the future were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by backcasting from the future.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to backcasting from the future.
5. Apply backcasting from the future only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if backcasting from the future were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for backcasting from the future explicit and bounded?
...
- `backcasting-from-the-future-decision-table`
- `backcasting-from-the-future-verification-report`
- `backcasting-from-the-future-handoff-envelope`
...
- The output directly and completely performs backcasting from the future within its declared boundary.
- Does the artifact make the owned decision for backcasting from the future explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating backcasting from the future as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `backcasting-from-the-future-decision-table`, `backcasting-from-the-future-verification-report`, `backcasting-from-the-future-handoff-envelope`.
