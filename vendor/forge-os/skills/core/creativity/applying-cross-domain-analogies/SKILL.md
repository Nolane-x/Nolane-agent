---
name: applying-cross-domain-analogies
description: "Use when applying cross domain analogies is required during creativity work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Applying Cross Domain Analogies

## Overview

This skill owns one bounded responsibility: **applying cross domain analogies**. Its focus is applying cross domain analogies within creativity lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by applying cross domain analogies.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to applying cross domain analogies.
3. Apply applying cross domain analogies only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if applying cross domain analogies were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by applying cross domain analogies.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to applying cross domain analogies.
5. Apply applying cross domain analogies only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if applying cross domain analogies were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for applying cross domain analogies explicit and bounded?
...
- `applying-cross-domain-analogies-decision-table`
- `applying-cross-domain-analogies-verification-report`
- `applying-cross-domain-analogies-handoff-envelope`
...
- The output directly and completely performs applying cross domain analogies within its declared boundary.
- Does the artifact make the owned decision for applying cross domain analogies explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating applying cross domain analogies as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `applying-cross-domain-analogies-decision-table`, `applying-cross-domain-analogies-verification-report`, `applying-cross-domain-analogies-handoff-envelope`.
