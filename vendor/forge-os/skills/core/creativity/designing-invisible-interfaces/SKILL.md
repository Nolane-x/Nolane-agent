---
name: designing-invisible-interfaces
description: "Use when designing invisible interfaces is required during creativity work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **designing invisible interfaces**. Its focus is designing invisible interfaces within creativity lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing invisible interfaces.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing invisible interfaces.
5. Apply designing invisible interfaces only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing invisible interfaces were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing invisible interfaces explicit and bounded?
...
- `designing-invisible-interfaces-decision-table`
- `designing-invisible-interfaces-verification-report`
- `designing-invisible-interfaces-handoff-envelope`
...
- The output directly and completely performs designing invisible interfaces within its declared boundary.
- Does the artifact make the owned decision for designing invisible interfaces explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing invisible interfaces as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-invisible-interfaces-decision-table`, `designing-invisible-interfaces-verification-report`, `designing-invisible-interfaces-handoff-envelope`.
