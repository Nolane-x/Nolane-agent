---
name: exploring-edge-users
description: "Use when exploring edge users is required during creativity work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Exploring Edge Users

## Overview

This skill owns one bounded responsibility: **exploring edge users**. Its focus is exploring edge users within creativity lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by exploring edge users.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to exploring edge users.
3. Apply exploring edge users only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if exploring edge users were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by exploring edge users.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to exploring edge users.
5. Apply exploring edge users only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if exploring edge users were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for exploring edge users explicit and bounded?
...
- `exploring-edge-users-decision-table`
- `exploring-edge-users-verification-report`
- `exploring-edge-users-handoff-envelope`
...
- The output directly and completely performs exploring edge users within its declared boundary.
- Does the artifact make the owned decision for exploring edge users explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating exploring edge users as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `exploring-edge-users-decision-table`, `exploring-edge-users-verification-report`, `exploring-edge-users-handoff-envelope`.
