---
name: planning-product-experiments
description: "Use when planning product experiments is required during product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **planning product experiments**. Its focus is planning product experiments within product lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by planning product experiments.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to planning product experiments.
3. Apply planning product experiments only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if planning product experiments were incomplete or implemented incorrectly.

## Procedure

1. Read confirmed evidence, selected concept, and unresolved assumptions.
2. Define the product decision this skill owns.
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by planning product experiments.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to planning product experiments.
5. Apply planning product experiments only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if planning product experiments were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for planning product experiments explicit and bounded?
...
- `planning-product-experiments-decision-table`
- `planning-product-experiments-verification-report`
- `planning-product-experiments-handoff-envelope`
...
- The output directly and completely performs planning product experiments within its declared boundary.
- Does the artifact make the owned decision for planning product experiments explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating planning product experiments as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `planning-product-experiments-decision-table`, `planning-product-experiments-verification-report`, `planning-product-experiments-handoff-envelope`.
