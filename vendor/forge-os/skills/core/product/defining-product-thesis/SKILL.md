---
name: defining-product-thesis
description: "Use when defining product thesis is required during product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Defining Product Thesis

## Overview

This skill owns one bounded responsibility: **defining product thesis**. Its focus is defining product thesis within product lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by defining product thesis.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to defining product thesis.
3. Apply defining product thesis only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if defining product thesis were incomplete or implemented incorrectly.

## Procedure

1. Read confirmed evidence, selected concept, and unresolved assumptions.
2. Define the product decision this skill owns.
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by defining product thesis.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to defining product thesis.
5. Apply defining product thesis only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if defining product thesis were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for defining product thesis explicit and bounded?
...
- `defining-product-thesis-decision-table`
- `defining-product-thesis-verification-report`
- `defining-product-thesis-handoff-envelope`
...
- The output directly and completely performs defining product thesis within its declared boundary.
- Does the artifact make the owned decision for defining product thesis explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating defining product thesis as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `defining-product-thesis-decision-table`, `defining-product-thesis-verification-report`, `defining-product-thesis-handoff-envelope`.
