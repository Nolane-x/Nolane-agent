---
name: mapping-business-model
description: "Use when mapping business model is required during product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **mapping business model**. Its focus is mapping business model within product lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by mapping business model.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to mapping business model.
5. Apply mapping business model only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if mapping business model were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for mapping business model explicit and bounded?
...
- `mapping-business-model-decision-table`
- `mapping-business-model-verification-report`
- `mapping-business-model-handoff-envelope`
...
- The output directly and completely performs mapping business model within its declared boundary.
- Does the artifact make the owned decision for mapping business model explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating mapping business model as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `mapping-business-model-decision-table`, `mapping-business-model-verification-report`, `mapping-business-model-handoff-envelope`.
