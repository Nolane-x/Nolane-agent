---
name: running-product-go-no-go
description: "Use when running product go no go is required during product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Running Product Go No Go

## Overview

This skill owns one bounded responsibility: **running product go no go**. Its focus is running product go no go within product lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by running product go no go.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to running product go no go.
3. Apply running product go no go only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if running product go no go were incomplete or implemented incorrectly.

## Procedure

1. Read confirmed evidence, selected concept, and unresolved assumptions.
2. Define the product decision this skill owns.
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by running product go no go.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to running product go no go.
5. Apply running product go no go only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if running product go no go were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for running product go no go explicit and bounded?
...
- `running-product-go-no-go-decision-table`
- `running-product-go-no-go-verification-report`
- `running-product-go-no-go-handoff-envelope`
...
- The output directly and completely performs running product go no go within its declared boundary.
- Does the artifact make the owned decision for running product go no go explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating running product go no go as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `running-product-go-no-go-decision-table`, `running-product-go-no-go-verification-report`, `running-product-go-no-go-handoff-envelope`.
