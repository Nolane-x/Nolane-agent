---
name: defining-product-north-star
description: "Use when defining product north star is required during product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Defining Product North Star

## Overview

This skill owns one bounded responsibility: **defining product north star**. Its focus is defining product north star within product lifecycle boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by defining product north star.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to defining product north star.
3. Apply defining product north star only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if defining product north star were incomplete or implemented incorrectly.

## Procedure

1. Read confirmed evidence, selected concept, and unresolved assumptions.
2. Define the product decision this skill owns.
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by defining product north star.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to defining product north star.
5. Apply defining product north star only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if defining product north star were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for defining product north star explicit and bounded?
...
- `defining-product-north-star-decision-table`
- `defining-product-north-star-verification-report`
- `defining-product-north-star-handoff-envelope`
...
- The output directly and completely performs defining product north star within its declared boundary.
- Does the artifact make the owned decision for defining product north star explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating defining product north star as a naming exercise
...
- Next transition: the graph router selects a real consumer of `product-definition`, `product-thesis`, `capability-map`.
- Required evidence: `contract-validation`, `independent-review`, `defining-product-north-star-decision-table`, `defining-product-north-star-verification-report`, `defining-product-north-star-handoff-envelope`.
