---
name: designing-content-scripts
description: "Use when designing content scripts is required during browser-extensions product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Designing Content Scripts

## Overview

This skill owns one bounded responsibility: **designing content scripts**. Its focus is designing content scripts within browser-extensions product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing content scripts.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing content scripts.
3. Apply designing content scripts only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing content scripts were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing content scripts.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing content scripts.
5. Apply designing content scripts only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing content scripts were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing content scripts explicit and bounded?
...
- `designing-content-scripts-decision-table`
- `designing-content-scripts-verification-report`
- `designing-content-scripts-handoff-envelope`
...
- The output directly and completely performs designing content scripts within its declared boundary.
- Does the artifact make the owned decision for designing content scripts explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing content scripts as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-content-scripts-decision-table`, `designing-content-scripts-verification-report`, `designing-content-scripts-handoff-envelope`.
