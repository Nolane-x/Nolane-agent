---
name: minimizing-extension-permissions
description: "Use when minimizing extension permissions is required during browser-extensions product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Minimizing Extension Permissions

## Overview

This skill owns one bounded responsibility: **minimizing extension permissions**. Its focus is minimizing extension permissions within browser-extensions product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by minimizing extension permissions.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to minimizing extension permissions.
3. Apply minimizing extension permissions only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if minimizing extension permissions were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by minimizing extension permissions.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to minimizing extension permissions.
5. Apply minimizing extension permissions only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if minimizing extension permissions were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for minimizing extension permissions explicit and bounded?
...
- `minimizing-extension-permissions-decision-table`
- `minimizing-extension-permissions-verification-report`
- `minimizing-extension-permissions-handoff-envelope`
...
Reviewer: `browser-extensions-reviewer`

- The output directly and completely performs minimizing extension permissions within its declared boundary.
- Does the artifact make the owned decision for minimizing extension permissions explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating minimizing extension permissions as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `minimizing-extension-permissions-decision-table`, `minimizing-extension-permissions-verification-report`, `minimizing-extension-permissions-handoff-envelope`.
...
Load [skills/references/domains/browser-extensions.md](../../../references/domains/browser-extensions.md) only when this skill needs pack-wide decision tables, evidence patterns, or cross-skill handoff rules.
