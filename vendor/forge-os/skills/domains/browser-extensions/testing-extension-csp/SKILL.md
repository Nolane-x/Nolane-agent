---
name: testing-extension-csp
description: "Use when testing extension csp is required during browser-extensions product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Testing Extension Csp

## Overview

This skill owns one bounded responsibility: **testing extension csp**. Its focus is testing extension csp within browser-extensions product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing extension csp.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing extension csp.
3. Apply testing extension csp only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if testing extension csp were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing extension csp.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing extension csp.
5. Apply testing extension csp only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if testing extension csp were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for testing extension csp explicit and bounded?
...
- `testing-extension-csp-decision-table`
- `testing-extension-csp-verification-report`
- `testing-extension-csp-handoff-envelope`
...
- The output directly and completely performs testing extension csp within its declared boundary.
- Does the artifact make the owned decision for testing extension csp explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating testing extension csp as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `testing-extension-csp-decision-table`, `testing-extension-csp-verification-report`, `testing-extension-csp-handoff-envelope`.
