---
name: designing-enterprise-integrations
description: "Use when designing enterprise integrations is required during enterprise product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **designing enterprise integrations**. Its focus is designing enterprise integrations within enterprise product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing enterprise integrations.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing enterprise integrations.
3. Apply designing enterprise integrations only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing enterprise integrations were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing enterprise integrations.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing enterprise integrations.
5. Apply designing enterprise integrations only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing enterprise integrations were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing enterprise integrations explicit and bounded?
...
- `designing-enterprise-integrations-decision-table`
- `designing-enterprise-integrations-verification-report`
- `designing-enterprise-integrations-handoff-envelope`
...
- The output directly and completely performs designing enterprise integrations within its declared boundary.
- Does the artifact make the owned decision for designing enterprise integrations explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing enterprise integrations as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-enterprise-integrations-decision-table`, `designing-enterprise-integrations-verification-report`, `designing-enterprise-integrations-handoff-envelope`.
...
Load [skills/references/domains/enterprise.md](../../../references/domains/enterprise.md) only when this skill needs pack-wide decision tables, evidence patterns, or cross-skill handoff rules.
