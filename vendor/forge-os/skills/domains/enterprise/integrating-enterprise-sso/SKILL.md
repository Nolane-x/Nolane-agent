---
name: integrating-enterprise-sso
description: "Use when integrating enterprise sso is required during enterprise product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **integrating enterprise sso**. Its focus is integrating enterprise sso within enterprise product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by integrating enterprise sso.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to integrating enterprise sso.
3. Apply integrating enterprise sso only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if integrating enterprise sso were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by integrating enterprise sso.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to integrating enterprise sso.
5. Apply integrating enterprise sso only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if integrating enterprise sso were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for integrating enterprise sso explicit and bounded?
...
- `integrating-enterprise-sso-decision-table`
- `integrating-enterprise-sso-verification-report`
- `integrating-enterprise-sso-handoff-envelope`
...
- The output directly and completely performs integrating enterprise sso within its declared boundary.
- Does the artifact make the owned decision for integrating enterprise sso explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating integrating enterprise sso as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `integrating-enterprise-sso-decision-table`, `integrating-enterprise-sso-verification-report`, `integrating-enterprise-sso-handoff-envelope`.
