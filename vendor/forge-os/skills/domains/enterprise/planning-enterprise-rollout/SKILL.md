---
name: planning-enterprise-rollout
description: "Use when planning enterprise rollout is required during enterprise product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **planning enterprise rollout**. Its focus is planning enterprise rollout within enterprise product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by planning enterprise rollout.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to planning enterprise rollout.
3. Apply planning enterprise rollout only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if planning enterprise rollout were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by planning enterprise rollout.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to planning enterprise rollout.
5. Apply planning enterprise rollout only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if planning enterprise rollout were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for planning enterprise rollout explicit and bounded?
...
- `planning-enterprise-rollout-decision-table`
- `planning-enterprise-rollout-verification-report`
- `planning-enterprise-rollout-handoff-envelope`
...
- The output directly and completely performs planning enterprise rollout within its declared boundary.
- Does the artifact make the owned decision for planning enterprise rollout explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating planning enterprise rollout as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `planning-enterprise-rollout-decision-table`, `planning-enterprise-rollout-verification-report`, `planning-enterprise-rollout-handoff-envelope`.
