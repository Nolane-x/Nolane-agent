---
name: operating-enterprise-support
description: "Use when operating enterprise support is required during enterprise product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Operating Enterprise Support

## Overview

This skill owns one bounded responsibility: **operating enterprise support**. Its focus is operating enterprise support within enterprise product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by operating enterprise support.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to operating enterprise support.
3. Apply operating enterprise support only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if operating enterprise support were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by operating enterprise support.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to operating enterprise support.
5. Apply operating enterprise support only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if operating enterprise support were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for operating enterprise support explicit and bounded?
...
- `operating-enterprise-support-decision-table`
- `operating-enterprise-support-verification-report`
- `operating-enterprise-support-handoff-envelope`
...
- The output directly and completely performs operating enterprise support within its declared boundary.
- Does the artifact make the owned decision for operating enterprise support explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating operating enterprise support as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `operating-enterprise-support-decision-table`, `operating-enterprise-support-verification-report`, `operating-enterprise-support-handoff-envelope`.
