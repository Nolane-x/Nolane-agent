---
name: designing-game-content-pipeline
description: "Use when designing game content pipeline is required during games product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Designing Game Content Pipeline

## Overview

This skill owns one bounded responsibility: **designing game content pipeline**. Its focus is designing game content pipeline within games product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing game content pipeline.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing game content pipeline.
3. Apply designing game content pipeline only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing game content pipeline were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing game content pipeline.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing game content pipeline.
5. Apply designing game content pipeline only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing game content pipeline were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing game content pipeline explicit and bounded?
...
- `designing-game-content-pipeline-decision-table`
- `designing-game-content-pipeline-verification-report`
- `designing-game-content-pipeline-handoff-envelope`
...
Reviewer: `games-reviewer`

- The output directly and completely performs designing game content pipeline within its declared boundary.
- Does the artifact make the owned decision for designing game content pipeline explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing game content pipeline as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-game-content-pipeline-decision-table`, `designing-game-content-pipeline-verification-report`, `designing-game-content-pipeline-handoff-envelope`.
...
Load [skills/references/domains/games.md](../../../references/domains/games.md) only when this skill needs pack-wide decision tables, evidence patterns, or cross-skill handoff rules.
