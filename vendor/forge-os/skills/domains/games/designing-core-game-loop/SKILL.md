---
name: designing-core-game-loop
description: "Use when designing core game loop is required during games product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **designing core game loop**. Its focus is designing core game loop within games product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing core game loop.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing core game loop.
3. Apply designing core game loop only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing core game loop were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing core game loop.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing core game loop.
5. Apply designing core game loop only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing core game loop were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing core game loop explicit and bounded?
...
- `designing-core-game-loop-decision-table`
- `designing-core-game-loop-verification-report`
- `designing-core-game-loop-handoff-envelope`
...
- The output directly and completely performs designing core game loop within its declared boundary.
- Does the artifact make the owned decision for designing core game loop explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing core game loop as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-core-game-loop-decision-table`, `designing-core-game-loop-verification-report`, `designing-core-game-loop-handoff-envelope`.
