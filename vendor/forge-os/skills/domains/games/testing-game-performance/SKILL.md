---
name: testing-game-performance
description: "Use when testing game performance is required during games product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **testing game performance**. Its focus is testing game performance within games product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing game performance.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing game performance.
3. Apply testing game performance only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if testing game performance were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing game performance.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing game performance.
5. Apply testing game performance only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if testing game performance were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for testing game performance explicit and bounded?
...
- `testing-game-performance-decision-table`
- `testing-game-performance-verification-report`
- `testing-game-performance-handoff-envelope`
...
- The output directly and completely performs testing game performance within its declared boundary.
- Does the artifact make the owned decision for testing game performance explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating testing game performance as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `testing-game-performance-decision-table`, `testing-game-performance-verification-report`, `testing-game-performance-handoff-envelope`.
