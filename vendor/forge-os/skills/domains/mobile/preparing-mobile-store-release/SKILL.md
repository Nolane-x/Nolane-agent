---
name: preparing-mobile-store-release
description: "Use when preparing mobile store release is required during mobile product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Preparing Mobile Store Release

## Overview

This skill owns one bounded responsibility: **preparing mobile store release**. Its focus is preparing mobile store release within mobile product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by preparing mobile store release.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to preparing mobile store release.
3. Apply preparing mobile store release only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if preparing mobile store release were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by preparing mobile store release.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to preparing mobile store release.
5. Apply preparing mobile store release only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if preparing mobile store release were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for preparing mobile store release explicit and bounded?
...
- `preparing-mobile-store-release-decision-table`
- `preparing-mobile-store-release-verification-report`
- `preparing-mobile-store-release-handoff-envelope`
...
- The output directly and completely performs preparing mobile store release within its declared boundary.
- Does the artifact make the owned decision for preparing mobile store release explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating preparing mobile store release as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `preparing-mobile-store-release-decision-table`, `preparing-mobile-store-release-verification-report`, `preparing-mobile-store-release-handoff-envelope`.
...
Load [skills/references/domains/mobile.md](../../../references/domains/mobile.md) only when this skill needs pack-wide decision tables, evidence patterns, or cross-skill handoff rules.
