---
name: designing-mobile-sync
description: "Use when designing mobile sync is required during mobile product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Designing Mobile Sync

## Overview

This skill owns one bounded responsibility: **designing mobile sync**. Its focus is designing mobile sync within mobile product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing mobile sync.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing mobile sync.
3. Apply designing mobile sync only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing mobile sync were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing mobile sync.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing mobile sync.
5. Apply designing mobile sync only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing mobile sync were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing mobile sync explicit and bounded?
...
- `designing-mobile-sync-decision-table`
- `designing-mobile-sync-verification-report`
- `designing-mobile-sync-handoff-envelope`
...
- The output directly and completely performs designing mobile sync within its declared boundary.
- Does the artifact make the owned decision for designing mobile sync explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing mobile sync as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-mobile-sync-decision-table`, `designing-mobile-sync-verification-report`, `designing-mobile-sync-handoff-envelope`.
