---
name: optimizing-mobile-battery
description: "Use when optimizing mobile battery is required during mobile product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Optimizing Mobile Battery

## Overview

This skill owns one bounded responsibility: **optimizing mobile battery**. Its focus is optimizing mobile battery within mobile product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by optimizing mobile battery.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to optimizing mobile battery.
3. Apply optimizing mobile battery only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if optimizing mobile battery were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by optimizing mobile battery.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to optimizing mobile battery.
5. Apply optimizing mobile battery only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if optimizing mobile battery were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for optimizing mobile battery explicit and bounded?
...
- `optimizing-mobile-battery-decision-table`
- `optimizing-mobile-battery-verification-report`
- `optimizing-mobile-battery-handoff-envelope`
...
- The output directly and completely performs optimizing mobile battery within its declared boundary.
- Does the artifact make the owned decision for optimizing mobile battery explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating optimizing mobile battery as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `optimizing-mobile-battery-decision-table`, `optimizing-mobile-battery-verification-report`, `optimizing-mobile-battery-handoff-envelope`.
