---
name: designing-mobile-permissions
description: "Use when designing mobile permissions is required during mobile product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Designing Mobile Permissions

## Overview

This skill owns one bounded responsibility: **designing mobile permissions**. Its focus is designing mobile permissions within mobile product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing mobile permissions.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing mobile permissions.
3. Apply designing mobile permissions only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing mobile permissions were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing mobile permissions.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing mobile permissions.
5. Apply designing mobile permissions only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing mobile permissions were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing mobile permissions explicit and bounded?
...
- `designing-mobile-permissions-decision-table`
- `designing-mobile-permissions-verification-report`
- `designing-mobile-permissions-handoff-envelope`
...
- The output directly and completely performs designing mobile permissions within its declared boundary.
- Does the artifact make the owned decision for designing mobile permissions explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing mobile permissions as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-mobile-permissions-decision-table`, `designing-mobile-permissions-verification-report`, `designing-mobile-permissions-handoff-envelope`.
