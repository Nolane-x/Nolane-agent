---
name: measuring-api-reliability
description: "Use when measuring api reliability is required during api-products product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **measuring api reliability**. Its focus is measuring api reliability within api-products product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by measuring api reliability.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to measuring api reliability.
3. Apply measuring api reliability only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if measuring api reliability were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by measuring api reliability.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to measuring api reliability.
5. Apply measuring api reliability only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if measuring api reliability were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for measuring api reliability explicit and bounded?
...
- `measuring-api-reliability-decision-table`
- `measuring-api-reliability-verification-report`
- `measuring-api-reliability-handoff-envelope`
...
- The output directly and completely performs measuring api reliability within its declared boundary.
- Does the artifact make the owned decision for measuring api reliability explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating measuring api reliability as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `measuring-api-reliability-decision-table`, `measuring-api-reliability-verification-report`, `measuring-api-reliability-handoff-envelope`.
