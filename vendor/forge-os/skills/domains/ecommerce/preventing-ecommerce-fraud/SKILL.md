---
name: preventing-ecommerce-fraud
description: "Use when preventing ecommerce fraud is required during ecommerce product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **preventing ecommerce fraud**. Its focus is preventing ecommerce fraud within ecommerce product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by preventing ecommerce fraud.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to preventing ecommerce fraud.
3. Apply preventing ecommerce fraud only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if preventing ecommerce fraud were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by preventing ecommerce fraud.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to preventing ecommerce fraud.
5. Apply preventing ecommerce fraud only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if preventing ecommerce fraud were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for preventing ecommerce fraud explicit and bounded?
...
- `preventing-ecommerce-fraud-decision-table`
- `preventing-ecommerce-fraud-verification-report`
- `preventing-ecommerce-fraud-handoff-envelope`
...
- The output directly and completely performs preventing ecommerce fraud within its declared boundary.
- Does the artifact make the owned decision for preventing ecommerce fraud explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating preventing ecommerce fraud as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `preventing-ecommerce-fraud-decision-table`, `preventing-ecommerce-fraud-verification-report`, `preventing-ecommerce-fraud-handoff-envelope`.
