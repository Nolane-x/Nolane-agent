---
name: designing-ecommerce-catalog
description: "Use when designing ecommerce catalog is required during ecommerce product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Designing Ecommerce Catalog

## Overview

This skill owns one bounded responsibility: **designing ecommerce catalog**. Its focus is designing ecommerce catalog within ecommerce product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing ecommerce catalog.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing ecommerce catalog.
3. Apply designing ecommerce catalog only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing ecommerce catalog were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing ecommerce catalog.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing ecommerce catalog.
5. Apply designing ecommerce catalog only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing ecommerce catalog were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing ecommerce catalog explicit and bounded?
...
- `designing-ecommerce-catalog-decision-table`
- `designing-ecommerce-catalog-verification-report`
- `designing-ecommerce-catalog-handoff-envelope`
...
- The output directly and completely performs designing ecommerce catalog within its declared boundary.
- Does the artifact make the owned decision for designing ecommerce catalog explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing ecommerce catalog as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-ecommerce-catalog-decision-table`, `designing-ecommerce-catalog-verification-report`, `designing-ecommerce-catalog-handoff-envelope`.
