---
name: designing-shopping-cart
description: "Use when designing shopping cart is required during ecommerce product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **designing shopping cart**. Its focus is designing shopping cart within ecommerce product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing shopping cart.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing shopping cart.
3. Apply designing shopping cart only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing shopping cart were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing shopping cart.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing shopping cart.
5. Apply designing shopping cart only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing shopping cart were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing shopping cart explicit and bounded?
...
- `designing-shopping-cart-decision-table`
- `designing-shopping-cart-verification-report`
- `designing-shopping-cart-handoff-envelope`
...
- The output directly and completely performs designing shopping cart within its declared boundary.
- Does the artifact make the owned decision for designing shopping cart explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing shopping cart as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-shopping-cart-decision-table`, `designing-shopping-cart-verification-report`, `designing-shopping-cart-handoff-envelope`.
