---
name: testing-ecommerce-tax
description: "Use when testing ecommerce tax is required during ecommerce product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Testing Ecommerce Tax

## Overview

This skill owns one bounded responsibility: **testing ecommerce tax**. Its focus is testing ecommerce tax within ecommerce product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing ecommerce tax.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing ecommerce tax.
3. Apply testing ecommerce tax only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if testing ecommerce tax were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing ecommerce tax.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing ecommerce tax.
5. Apply testing ecommerce tax only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if testing ecommerce tax were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for testing ecommerce tax explicit and bounded?
...
- `testing-ecommerce-tax-decision-table`
- `testing-ecommerce-tax-verification-report`
- `testing-ecommerce-tax-handoff-envelope`
...
- The output directly and completely performs testing ecommerce tax within its declared boundary.
- Does the artifact make the owned decision for testing ecommerce tax explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating testing ecommerce tax as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `testing-ecommerce-tax-decision-table`, `testing-ecommerce-tax-verification-report`, `testing-ecommerce-tax-handoff-envelope`.
