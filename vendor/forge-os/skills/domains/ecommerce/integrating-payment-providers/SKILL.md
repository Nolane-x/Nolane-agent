---
name: integrating-payment-providers
description: "Use when integrating payment providers is required during ecommerce product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **integrating payment providers**. Its focus is integrating payment providers within ecommerce product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by integrating payment providers.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to integrating payment providers.
3. Apply integrating payment providers only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if integrating payment providers were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by integrating payment providers.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to integrating payment providers.
5. Apply integrating payment providers only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if integrating payment providers were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for integrating payment providers explicit and bounded?
...
- `integrating-payment-providers-decision-table`
- `integrating-payment-providers-verification-report`
- `integrating-payment-providers-handoff-envelope`
...
- The output directly and completely performs integrating payment providers within its declared boundary.
- Does the artifact make the owned decision for integrating payment providers explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating integrating payment providers as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `integrating-payment-providers-decision-table`, `integrating-payment-providers-verification-report`, `integrating-payment-providers-handoff-envelope`.
...
Load [skills/references/domains/ecommerce.md](../../../references/domains/ecommerce.md) only when this skill needs pack-wide decision tables, evidence patterns, or cross-skill handoff rules.
