---
name: designing-os-integrations
description: "Use when designing os integrations is required during desktop product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Designing Os Integrations

## Overview

This skill owns one bounded responsibility: **designing os integrations**. Its focus is designing os integrations within desktop product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing os integrations.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing os integrations.
3. Apply designing os integrations only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing os integrations were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing os integrations.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing os integrations.
5. Apply designing os integrations only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing os integrations were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing os integrations explicit and bounded?
...
- `designing-os-integrations-decision-table`
- `designing-os-integrations-verification-report`
- `designing-os-integrations-handoff-envelope`
...
- The output directly and completely performs designing os integrations within its declared boundary.
- Does the artifact make the owned decision for designing os integrations explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing os integrations as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-os-integrations-decision-table`, `designing-os-integrations-verification-report`, `designing-os-integrations-handoff-envelope`.
