---
name: testing-desktop-file-associations
description: "Use when testing desktop file associations is required during desktop product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Testing Desktop File Associations

## Overview

This skill owns one bounded responsibility: **testing desktop file associations**. Its focus is testing desktop file associations within desktop product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing desktop file associations.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing desktop file associations.
3. Apply testing desktop file associations only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if testing desktop file associations were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing desktop file associations.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing desktop file associations.
5. Apply testing desktop file associations only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if testing desktop file associations were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for testing desktop file associations explicit and bounded?
...
- `testing-desktop-file-associations-decision-table`
- `testing-desktop-file-associations-verification-report`
- `testing-desktop-file-associations-handoff-envelope`
...
- The output directly and completely performs testing desktop file associations within its declared boundary.
- Does the artifact make the owned decision for testing desktop file associations explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating testing desktop file associations as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `testing-desktop-file-associations-decision-table`, `testing-desktop-file-associations-verification-report`, `testing-desktop-file-associations-handoff-envelope`.
...
Load [skills/references/domains/desktop.md](../../../references/domains/desktop.md) only when this skill needs pack-wide decision tables, evidence patterns, or cross-skill handoff rules.
