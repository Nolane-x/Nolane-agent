---
name: designing-desktop-updates
description: "Use when designing desktop updates is required during desktop product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Designing Desktop Updates

## Overview

This skill owns one bounded responsibility: **designing desktop updates**. Its focus is designing desktop updates within desktop product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing desktop updates.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing desktop updates.
3. Apply designing desktop updates only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing desktop updates were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing desktop updates.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing desktop updates.
5. Apply designing desktop updates only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing desktop updates were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing desktop updates explicit and bounded?
...
- `designing-desktop-updates-decision-table`
- `designing-desktop-updates-verification-report`
- `designing-desktop-updates-handoff-envelope`
...
- The output directly and completely performs designing desktop updates within its declared boundary.
- Does the artifact make the owned decision for designing desktop updates explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing desktop updates as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-desktop-updates-decision-table`, `designing-desktop-updates-verification-report`, `designing-desktop-updates-handoff-envelope`.
