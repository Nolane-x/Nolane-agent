---
name: designing-desktop-packaging
description: "Use when designing desktop packaging is required during desktop product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Designing Desktop Packaging

## Overview

This skill owns one bounded responsibility: **designing desktop packaging**. Its focus is designing desktop packaging within desktop product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing desktop packaging.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing desktop packaging.
3. Apply designing desktop packaging only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing desktop packaging were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing desktop packaging.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing desktop packaging.
5. Apply designing desktop packaging only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing desktop packaging were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing desktop packaging explicit and bounded?
...
- `designing-desktop-packaging-decision-table`
- `designing-desktop-packaging-verification-report`
- `designing-desktop-packaging-handoff-envelope`
...
- The output directly and completely performs designing desktop packaging within its declared boundary.
- Does the artifact make the owned decision for designing desktop packaging explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing desktop packaging as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-desktop-packaging-decision-table`, `designing-desktop-packaging-verification-report`, `designing-desktop-packaging-handoff-envelope`.
