---
name: sandboxing-desktop-features
description: "Use when sandboxing desktop features is required during desktop product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Sandboxing Desktop Features

## Overview

This skill owns one bounded responsibility: **sandboxing desktop features**. Its focus is sandboxing desktop features within desktop product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by sandboxing desktop features.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to sandboxing desktop features.
3. Apply sandboxing desktop features only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if sandboxing desktop features were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by sandboxing desktop features.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to sandboxing desktop features.
5. Apply sandboxing desktop features only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if sandboxing desktop features were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for sandboxing desktop features explicit and bounded?
...
- `sandboxing-desktop-features-decision-table`
- `sandboxing-desktop-features-verification-report`
- `sandboxing-desktop-features-handoff-envelope`
...
- The output directly and completely performs sandboxing desktop features within its declared boundary.
- Does the artifact make the owned decision for sandboxing desktop features explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating sandboxing desktop features as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `sandboxing-desktop-features-decision-table`, `sandboxing-desktop-features-verification-report`, `sandboxing-desktop-features-handoff-envelope`.
