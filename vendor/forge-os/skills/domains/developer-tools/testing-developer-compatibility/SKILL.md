---
name: testing-developer-compatibility
description: "Use when testing developer compatibility is required during developer-tools product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Testing Developer Compatibility

## Overview

This skill owns one bounded responsibility: **testing developer compatibility**. Its focus is testing developer compatibility within developer-tools product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing developer compatibility.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing developer compatibility.
3. Apply testing developer compatibility only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if testing developer compatibility were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing developer compatibility.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing developer compatibility.
5. Apply testing developer compatibility only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if testing developer compatibility were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for testing developer compatibility explicit and bounded?
...
- `testing-developer-compatibility-decision-table`
- `testing-developer-compatibility-verification-report`
- `testing-developer-compatibility-handoff-envelope`
...
Reviewer: `developer-tools-reviewer`

- The output directly and completely performs testing developer compatibility within its declared boundary.
- Does the artifact make the owned decision for testing developer compatibility explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating testing developer compatibility as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `testing-developer-compatibility-decision-table`, `testing-developer-compatibility-verification-report`, `testing-developer-compatibility-handoff-envelope`.
...
Load [skills/references/domains/developer-tools.md](../../../references/domains/developer-tools.md) only when this skill needs pack-wide decision tables, evidence patterns, or cross-skill handoff rules.
