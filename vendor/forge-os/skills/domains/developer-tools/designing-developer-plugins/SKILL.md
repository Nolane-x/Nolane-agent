---
name: designing-developer-plugins
description: "Use when designing developer plugins is required during developer-tools product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Designing Developer Plugins

## Overview

This skill owns one bounded responsibility: **designing developer plugins**. Its focus is designing developer plugins within developer-tools product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing developer plugins.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing developer plugins.
3. Apply designing developer plugins only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing developer plugins were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing developer plugins.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing developer plugins.
5. Apply designing developer plugins only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing developer plugins were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing developer plugins explicit and bounded?
...
- `designing-developer-plugins-decision-table`
- `designing-developer-plugins-verification-report`
- `designing-developer-plugins-handoff-envelope`
...
- The output directly and completely performs designing developer plugins within its declared boundary.
- Does the artifact make the owned decision for designing developer plugins explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing developer plugins as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-developer-plugins-decision-table`, `designing-developer-plugins-verification-report`, `designing-developer-plugins-handoff-envelope`.
