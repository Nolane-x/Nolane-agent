...
description: "Use when measuring developer experience is required during developer-tools product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **measuring developer experience**. Its focus is measuring developer experience within developer-tools product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by measuring developer experience.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to measuring developer experience.
3. Apply measuring developer experience only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if measuring developer experience were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by measuring developer experience.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to measuring developer experience.
5. Apply measuring developer experience only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if measuring developer experience were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for measuring developer experience explicit and bounded?
...
- `measuring-developer-experience-decision-table`
- `measuring-developer-experience-verification-report`
- `measuring-developer-experience-handoff-envelope`
...
- The output directly and completely performs measuring developer experience within its declared boundary.
- Does the artifact make the owned decision for measuring developer experience explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating measuring developer experience as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `measuring-developer-experience-decision-table`, `measuring-developer-experience-verification-report`, `measuring-developer-experience-handoff-envelope`.
