---
name: packaging-developer-binaries
description: "Use when packaging developer binaries is required during developer-tools product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Packaging Developer Binaries

## Overview

This skill owns one bounded responsibility: **packaging developer binaries**. Its focus is packaging developer binaries within developer-tools product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by packaging developer binaries.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to packaging developer binaries.
3. Apply packaging developer binaries only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if packaging developer binaries were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by packaging developer binaries.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to packaging developer binaries.
5. Apply packaging developer binaries only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if packaging developer binaries were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for packaging developer binaries explicit and bounded?
...
- `packaging-developer-binaries-decision-table`
- `packaging-developer-binaries-verification-report`
- `packaging-developer-binaries-handoff-envelope`
...
- The output directly and completely performs packaging developer binaries within its declared boundary.
- Does the artifact make the owned decision for packaging developer binaries explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating packaging developer binaries as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `packaging-developer-binaries-decision-table`, `packaging-developer-binaries-verification-report`, `packaging-developer-binaries-handoff-envelope`.
