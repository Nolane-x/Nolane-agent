---
name: designing-filesystem-tools
description: "Use when designing filesystem tools is required during developer-tools product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Designing Filesystem Tools

## Overview

This skill owns one bounded responsibility: **designing filesystem tools**. Its focus is designing filesystem tools within developer-tools product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing filesystem tools.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing filesystem tools.
3. Apply designing filesystem tools only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing filesystem tools were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing filesystem tools.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing filesystem tools.
5. Apply designing filesystem tools only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing filesystem tools were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing filesystem tools explicit and bounded?
...
- `designing-filesystem-tools-decision-table`
- `designing-filesystem-tools-verification-report`
- `designing-filesystem-tools-handoff-envelope`
...
- The output directly and completely performs designing filesystem tools within its declared boundary.
- Does the artifact make the owned decision for designing filesystem tools explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing filesystem tools as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-filesystem-tools-decision-table`, `designing-filesystem-tools-verification-report`, `designing-filesystem-tools-handoff-envelope`.
