---
name: enforcing-saas-quotas
description: "Use when enforcing saas quotas is required during saas product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Enforcing Saas Quotas

## Overview

This skill owns one bounded responsibility: **enforcing saas quotas**. Its focus is enforcing saas quotas within saas product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by enforcing saas quotas.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to enforcing saas quotas.
3. Apply enforcing saas quotas only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if enforcing saas quotas were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by enforcing saas quotas.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to enforcing saas quotas.
5. Apply enforcing saas quotas only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if enforcing saas quotas were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for enforcing saas quotas explicit and bounded?
...
- `enforcing-saas-quotas-decision-table`
- `enforcing-saas-quotas-verification-report`
- `enforcing-saas-quotas-handoff-envelope`
...
- The output directly and completely performs enforcing saas quotas within its declared boundary.
- Does the artifact make the owned decision for enforcing saas quotas explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating enforcing saas quotas as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `enforcing-saas-quotas-decision-table`, `enforcing-saas-quotas-verification-report`, `enforcing-saas-quotas-handoff-envelope`.
