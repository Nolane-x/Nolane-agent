---
name: designing-saas-billing
description: "Use when designing saas billing is required during saas product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Designing Saas Billing

## Overview

This skill owns one bounded responsibility: **designing saas billing**. Its focus is designing saas billing within saas product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing saas billing.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing saas billing.
3. Apply designing saas billing only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing saas billing were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing saas billing.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing saas billing.
5. Apply designing saas billing only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing saas billing were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing saas billing explicit and bounded?
...
- `designing-saas-billing-decision-table`
- `designing-saas-billing-verification-report`
- `designing-saas-billing-handoff-envelope`
...
- The output directly and completely performs designing saas billing within its declared boundary.
- Does the artifact make the owned decision for designing saas billing explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing saas billing as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-saas-billing-decision-table`, `designing-saas-billing-verification-report`, `designing-saas-billing-handoff-envelope`.
