---
name: measuring-saas-activation
description: "Use when measuring saas activation is required during saas product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Measuring Saas Activation

## Overview

This skill owns one bounded responsibility: **measuring saas activation**. Its focus is measuring saas activation within saas product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by measuring saas activation.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to measuring saas activation.
3. Apply measuring saas activation only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if measuring saas activation were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by measuring saas activation.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to measuring saas activation.
5. Apply measuring saas activation only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if measuring saas activation were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for measuring saas activation explicit and bounded?
...
- `measuring-saas-activation-decision-table`
- `measuring-saas-activation-verification-report`
- `measuring-saas-activation-handoff-envelope`
...
- The output directly and completely performs measuring saas activation within its declared boundary.
- Does the artifact make the owned decision for measuring saas activation explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating measuring saas activation as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `measuring-saas-activation-decision-table`, `measuring-saas-activation-verification-report`, `measuring-saas-activation-handoff-envelope`.
