---
name: securing-data-access
description: "Use when securing data access is required during data-platforms product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **securing data access**. Its focus is securing data access within data-platforms product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by securing data access.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to securing data access.
3. Apply securing data access only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if securing data access were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by securing data access.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to securing data access.
5. Apply securing data access only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if securing data access were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for securing data access explicit and bounded?
...
- `securing-data-access-decision-table`
- `securing-data-access-verification-report`
- `securing-data-access-handoff-envelope`
...
- The output directly and completely performs securing data access within its declared boundary.
- Does the artifact make the owned decision for securing data access explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating securing data access as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `securing-data-access-decision-table`, `securing-data-access-verification-report`, `securing-data-access-handoff-envelope`.
