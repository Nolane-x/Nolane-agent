---
name: designing-data-partitions
description: "Use when designing data partitions is required during data-platforms product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Designing Data Partitions

## Overview

This skill owns one bounded responsibility: **designing data partitions**. Its focus is designing data partitions within data-platforms product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing data partitions.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing data partitions.
3. Apply designing data partitions only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing data partitions were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing data partitions.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing data partitions.
5. Apply designing data partitions only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing data partitions were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing data partitions explicit and bounded?
...
- `designing-data-partitions-decision-table`
- `designing-data-partitions-verification-report`
- `designing-data-partitions-handoff-envelope`
...
Reviewer: `data-platforms-reviewer`

- The output directly and completely performs designing data partitions within its declared boundary.
- Does the artifact make the owned decision for designing data partitions explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing data partitions as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-data-partitions-decision-table`, `designing-data-partitions-verification-report`, `designing-data-partitions-handoff-envelope`.
...
Load [skills/references/domains/data-platforms.md](../../../references/domains/data-platforms.md) only when this skill needs pack-wide decision tables, evidence patterns, or cross-skill handoff rules.
