---
name: designing-data-lineage
description: "Use when designing data lineage is required during data-platforms product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **designing data lineage**. Its focus is designing data lineage within data-platforms product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing data lineage.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing data lineage.
3. Apply designing data lineage only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing data lineage were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing data lineage.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing data lineage.
5. Apply designing data lineage only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing data lineage were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing data lineage explicit and bounded?
...
- `designing-data-lineage-decision-table`
- `designing-data-lineage-verification-report`
- `designing-data-lineage-handoff-envelope`
...
- The output directly and completely performs designing data lineage within its declared boundary.
- Does the artifact make the owned decision for designing data lineage explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing data lineage as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-data-lineage-decision-table`, `designing-data-lineage-verification-report`, `designing-data-lineage-handoff-envelope`.
