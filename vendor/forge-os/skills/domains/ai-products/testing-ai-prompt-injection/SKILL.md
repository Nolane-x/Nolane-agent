---
name: testing-ai-prompt-injection
description: "Use when testing ai prompt injection is required during ai-products product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Testing Ai Prompt Injection

## Overview

This skill owns one bounded responsibility: **testing ai prompt injection**. Its focus is testing ai prompt injection within ai-products product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing ai prompt injection.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing ai prompt injection.
3. Apply testing ai prompt injection only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if testing ai prompt injection were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing ai prompt injection.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing ai prompt injection.
5. Apply testing ai prompt injection only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if testing ai prompt injection were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for testing ai prompt injection explicit and bounded?
...
- `testing-ai-prompt-injection-decision-table`
- `testing-ai-prompt-injection-verification-report`
- `testing-ai-prompt-injection-handoff-envelope`
...
- The output directly and completely performs testing ai prompt injection within its declared boundary.
- Does the artifact make the owned decision for testing ai prompt injection explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating testing ai prompt injection as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `testing-ai-prompt-injection-decision-table`, `testing-ai-prompt-injection-verification-report`, `testing-ai-prompt-injection-handoff-envelope`.
