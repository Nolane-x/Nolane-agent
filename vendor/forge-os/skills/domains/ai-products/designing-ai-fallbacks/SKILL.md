---
name: designing-ai-fallbacks
description: "Use when designing ai fallbacks is required during ai-products product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **designing ai fallbacks**. Its focus is designing ai fallbacks within ai-products product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing ai fallbacks.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing ai fallbacks.
3. Apply designing ai fallbacks only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing ai fallbacks were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing ai fallbacks.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing ai fallbacks.
5. Apply designing ai fallbacks only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing ai fallbacks were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing ai fallbacks explicit and bounded?
...
- `designing-ai-fallbacks-decision-table`
- `designing-ai-fallbacks-verification-report`
- `designing-ai-fallbacks-handoff-envelope`
...
- The output directly and completely performs designing ai fallbacks within its declared boundary.
- Does the artifact make the owned decision for designing ai fallbacks explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing ai fallbacks as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-ai-fallbacks-decision-table`, `designing-ai-fallbacks-verification-report`, `designing-ai-fallbacks-handoff-envelope`.
