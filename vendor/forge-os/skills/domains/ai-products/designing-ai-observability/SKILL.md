---
name: designing-ai-observability
description: "Use when designing ai observability is required during ai-products product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Designing Ai Observability

## Overview

This skill owns one bounded responsibility: **designing ai observability**. Its focus is designing ai observability within ai-products product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing ai observability.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing ai observability.
3. Apply designing ai observability only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if designing ai observability were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by designing ai observability.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to designing ai observability.
5. Apply designing ai observability only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if designing ai observability were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for designing ai observability explicit and bounded?
...
- `designing-ai-observability-decision-table`
- `designing-ai-observability-verification-report`
- `designing-ai-observability-handoff-envelope`
...
- The output directly and completely performs designing ai observability within its declared boundary.
- Does the artifact make the owned decision for designing ai observability explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating designing ai observability as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `designing-ai-observability-decision-table`, `designing-ai-observability-verification-report`, `designing-ai-observability-handoff-envelope`.
