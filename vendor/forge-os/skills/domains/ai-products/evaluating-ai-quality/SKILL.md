---
name: evaluating-ai-quality
description: "Use when evaluating ai quality is required during ai-products product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **evaluating ai quality**. Its focus is evaluating ai quality within ai-products product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by evaluating ai quality.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to evaluating ai quality.
3. Apply evaluating ai quality only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if evaluating ai quality were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by evaluating ai quality.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to evaluating ai quality.
5. Apply evaluating ai quality only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if evaluating ai quality were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for evaluating ai quality explicit and bounded?
...
- `evaluating-ai-quality-decision-table`
- `evaluating-ai-quality-verification-report`
- `evaluating-ai-quality-handoff-envelope`
...
- The output directly and completely performs evaluating ai quality within its declared boundary.
- Does the artifact make the owned decision for evaluating ai quality explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating evaluating ai quality as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `evaluating-ai-quality-decision-table`, `evaluating-ai-quality-verification-report`, `evaluating-ai-quality-handoff-envelope`.
