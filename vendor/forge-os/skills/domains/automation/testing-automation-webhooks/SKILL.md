---
name: testing-automation-webhooks
description: "Use when testing automation webhooks is required during automation product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Testing Automation Webhooks

## Overview

This skill owns one bounded responsibility: **testing automation webhooks**. Its focus is testing automation webhooks within automation product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing automation webhooks.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing automation webhooks.
3. Apply testing automation webhooks only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if testing automation webhooks were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing automation webhooks.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing automation webhooks.
5. Apply testing automation webhooks only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if testing automation webhooks were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for testing automation webhooks explicit and bounded?
...
- `testing-automation-webhooks-decision-table`
- `testing-automation-webhooks-verification-report`
- `testing-automation-webhooks-handoff-envelope`
...
- The output directly and completely performs testing automation webhooks within its declared boundary.
- Does the artifact make the owned decision for testing automation webhooks explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating testing automation webhooks as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `testing-automation-webhooks-decision-table`, `testing-automation-webhooks-verification-report`, `testing-automation-webhooks-handoff-envelope`.
