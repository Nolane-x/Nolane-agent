---
name: modeling-automation-side-effects
description: "Use when modeling automation side effects is required during automation product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Modeling Automation Side Effects

## Overview

This skill owns one bounded responsibility: **modeling automation side effects**. Its focus is modeling automation side effects within automation product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by modeling automation side effects.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to modeling automation side effects.
3. Apply modeling automation side effects only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if modeling automation side effects were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by modeling automation side effects.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to modeling automation side effects.
5. Apply modeling automation side effects only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if modeling automation side effects were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for modeling automation side effects explicit and bounded?
...
- `modeling-automation-side-effects-decision-table`
- `modeling-automation-side-effects-verification-report`
- `modeling-automation-side-effects-handoff-envelope`
...
- The output directly and completely performs modeling automation side effects within its declared boundary.
- Does the artifact make the owned decision for modeling automation side effects explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating modeling automation side effects as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `modeling-automation-side-effects-decision-table`, `modeling-automation-side-effects-verification-report`, `modeling-automation-side-effects-handoff-envelope`.
...
Load [skills/references/domains/automation.md](../../../references/domains/automation.md) only when this skill needs pack-wide decision tables, evidence patterns, or cross-skill handoff rules.
