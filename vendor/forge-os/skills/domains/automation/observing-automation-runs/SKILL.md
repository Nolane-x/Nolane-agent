---
name: observing-automation-runs
description: "Use when observing automation runs is required during automation product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Observing Automation Runs

## Overview

This skill owns one bounded responsibility: **observing automation runs**. Its focus is observing automation runs within automation product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by observing automation runs.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to observing automation runs.
3. Apply observing automation runs only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if observing automation runs were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by observing automation runs.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to observing automation runs.
5. Apply observing automation runs only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if observing automation runs were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for observing automation runs explicit and bounded?
...
- `observing-automation-runs-decision-table`
- `observing-automation-runs-verification-report`
- `observing-automation-runs-handoff-envelope`
...
- The output directly and completely performs observing automation runs within its declared boundary.
- Does the artifact make the owned decision for observing automation runs explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating observing automation runs as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `observing-automation-runs-decision-table`, `observing-automation-runs-verification-report`, `observing-automation-runs-handoff-envelope`.
