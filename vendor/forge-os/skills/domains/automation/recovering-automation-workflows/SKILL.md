---
name: recovering-automation-workflows
description: "Use when recovering automation workflows is required during automation product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Recovering Automation Workflows

## Overview

This skill owns one bounded responsibility: **recovering automation workflows**. Its focus is recovering automation workflows within automation product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by recovering automation workflows.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to recovering automation workflows.
3. Apply recovering automation workflows only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if recovering automation workflows were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by recovering automation workflows.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to recovering automation workflows.
5. Apply recovering automation workflows only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if recovering automation workflows were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for recovering automation workflows explicit and bounded?
...
- `recovering-automation-workflows-decision-table`
- `recovering-automation-workflows-verification-report`
- `recovering-automation-workflows-handoff-envelope`
...
- The output directly and completely performs recovering automation workflows within its declared boundary.
- Does the artifact make the owned decision for recovering automation workflows explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating recovering automation workflows as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `recovering-automation-workflows-decision-table`, `recovering-automation-workflows-verification-report`, `recovering-automation-workflows-handoff-envelope`.
