---
name: testing-automation-schedules
description: "Use when testing automation schedules is required during automation product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
# Testing Automation Schedules

## Overview

This skill owns one bounded responsibility: **testing automation schedules**. Its focus is testing automation schedules within automation product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing automation schedules.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing automation schedules.
3. Apply testing automation schedules only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if testing automation schedules were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by testing automation schedules.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to testing automation schedules.
5. Apply testing automation schedules only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if testing automation schedules were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for testing automation schedules explicit and bounded?
...
- `testing-automation-schedules-decision-table`
- `testing-automation-schedules-verification-report`
- `testing-automation-schedules-handoff-envelope`
...
- The output directly and completely performs testing automation schedules within its declared boundary.
- Does the artifact make the owned decision for testing automation schedules explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating testing automation schedules as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `testing-automation-schedules-decision-table`, `testing-automation-schedules-verification-report`, `testing-automation-schedules-handoff-envelope`.
