---
name: securing-automation-credentials
description: "Use when securing automation credentials is required during automation product work, especially when the result must be traceable, independently reviewable, and safe to hand to another agent."
license: MIT
...
This skill owns one bounded responsibility: **securing automation credentials**. Its focus is securing automation credentials within automation product boundaries. It converts declared inputs into typed artifacts and reproducible evidence without silently changing product scope.
...
1. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by securing automation credentials.
2. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to securing automation credentials.
3. Apply securing automation credentials only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
4. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
5. Create reproducible checks that would fail if securing automation credentials were incomplete or implemented incorrectly.
...
3. Define the exact decision, actors, objects, states, invariants, side effects, and non-goals owned by securing automation credentials.
4. Build a decision table for normal, boundary, invalid, permission, failure, retry, recovery, concurrency, migration, and abuse conditions relevant to securing automation credentials.
5. Apply securing automation credentials only to direct input artifacts; record assumptions, rejected alternatives, and any human decision still required.
6. Trace the resulting contract to user value, security, reliability, cost, operability, and downstream consumers.
7. Create reproducible checks that would fail if securing automation credentials were incomplete or implemented incorrectly.
...
- Does the artifact make the owned decision for securing automation credentials explicit and bounded?
...
- `securing-automation-credentials-decision-table`
- `securing-automation-credentials-verification-report`
- `securing-automation-credentials-handoff-envelope`
...
- The output directly and completely performs securing automation credentials within its declared boundary.
- Does the artifact make the owned decision for securing automation credentials explicit and bounded?
- Are failure, recovery, permissions, concurrency, and irreversible side effects addressed where applicable?
- Can every load-bearing claim be traced to a confirmed fact, direct artifact, executable check, or declared assumption?
- Would a downstream agent know exactly what changed, what remains open, and what must be invalidated?
...
- treating securing automation credentials as a naming exercise
...
- Required evidence: `contract-validation`, `independent-review`, `securing-automation-credentials-decision-table`, `securing-automation-credentials-verification-report`, `securing-automation-credentials-handoff-envelope`.
