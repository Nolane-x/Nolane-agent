# Operations Core Pack Playbook

This reference is loaded on demand by ForgeOS skills. It does not replace the selected skill's contract.

## Pack boundary

- Lifecycle coverage: implementation → verification → release-readiness → released
- Skills: `designing-ci-cd`, `choosing-deployment-strategy`, `rehearsing-data-migrations`, `testing-production-rollback`, `validating-observability`, `defining-service-objectives`, `defining-error-budget-policy`, `running-canary-release`, `smoke-testing-production`, `monitoring-post-release`, `triaging-production-incidents`, `packaging-release-evidence`
