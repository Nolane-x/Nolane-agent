# Creativity Core Pack Playbook

This reference is loaded on demand by ForgeOS skills. It does not replace the selected skill's contract.

## Pack boundary

- Lifecycle coverage: divergence → synthesis
- Skills: `framing-creative-challenge`, `breaking-hidden-assumptions`, `generating-divergent-concepts`, `applying-cross-domain-analogies`, `constructing-idea-genomes`, `mutating-idea-genomes`, `using-denial-ladders`, `exploring-edge-users`, `exploring-zero-resource-designs`, `designing-invisible-interfaces`, `backcasting-from-the-future`, `clustering-semantic-ideas`, `detecting-fake-novelty`, `scoring-contextual-creativity`
