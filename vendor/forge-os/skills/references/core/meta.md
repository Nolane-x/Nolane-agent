# Meta Core Pack Playbook

This reference is loaded on demand by ForgeOS skills. It does not replace the selected skill's contract.

## Pack boundary

- Lifecycle coverage: discovery → planning → implementation → verification
- Skills: `writing-agent-skills`, `testing-skill-behavior`, `benchmarking-skill-utility`, `detecting-skill-regression`, `selecting-model-provider`, `optimizing-context-budget`, `compressing-project-artifacts`, `learning-from-agent-failures`, `evolving-skill-packs`, `certifying-platform-adapters`
