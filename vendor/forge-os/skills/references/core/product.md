# Product Core Pack Playbook

This reference is loaded on demand by ForgeOS skills. It does not replace the selected skill's contract.

## Pack boundary

- Lifecycle coverage: selection → product-definition
- Skills: `selecting-winning-concept`, `defining-product-thesis`, `defining-product-north-star`, `defining-product-non-goals`, `mapping-business-model`, `modeling-unit-economics`, `designing-product-pricing`, `defining-minimum-viable-product`, `planning-product-experiments`, `running-product-go-no-go`
