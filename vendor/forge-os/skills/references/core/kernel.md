# Kernel Core Pack Playbook

This reference is loaded on demand by ForgeOS skills. It does not replace the selected skill's contract.

## Pack boundary

- Lifecycle coverage: intent → discovery → research → divergence → synthesis → selection → product-definition → ux-design → architecture → planning → implementation → verification → release-readiness
- Skills: `using-forge-os`, `resolving-user-intent`, `routing-skill-graph`, `compiling-context-pack`, `managing-project-state`, `managing-artifact-lineage`, `handling-uncertainty`, `requesting-human-decisions`, `recovering-interrupted-runs`, `auditing-agent-actions`
