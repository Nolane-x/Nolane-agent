...
export function inferHarnessFamily(provider = {}) {
  ...
  if (id === 'codex' || id === 'codex-app-server' || kind === 'codex-app-server') return 'codex-cli';
...
export function createBuiltInCliProviders(overrides = {}) {
  const definitions = [
    { id: 'codex', label: 'OpenAI Codex CLI', executable: 'codex', harnessFamily: 'codex-cli', baseArgs: ['exec', '--json', '--ephemeral', '--sandbox', 'read-only', '-'], promptMode: 'stdin', profile: { capabilities: ['coding', 'structured-output', 'subscription-auth', 'governed-actions'], qualityTier: 4, costTier: 0, latencyTier: 2 } },
    ...
    { id: 'opencode', label: 'OpenCode', executable: 'opencode', harnessFamily: 'generic-local', baseArgs: ['run'], promptMode: 'arg', profile: { capabilities: ['coding', 'structured-output', 'governed-actions'], qualityTier: 3, costTier: 1, latencyTier: 2 } },
  ];
