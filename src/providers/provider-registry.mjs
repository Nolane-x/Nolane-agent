...
export function inferHarnessFamily(provider = {}) {
  ...
  if (id === 'grok-build') return 'grok-build-cli';
...
export function createBuiltInCliProviders(overrides = {}) {
  const definitions = [
    { id: 'codex', label: 'OpenAI Codex CLI', executable: 'codex', harnessFamily: 'codex-cli', baseArgs: ['exec', '--json', '--ephemeral', '--skip-git-repo-check', '--sandbox', 'read-only', '-'], promptMode: 'stdin', effortConfigKey: 'model_reasoning_effort', secretEnvKeys: ['OPENAI_API_KEY', 'CODEX_HOME'], profile: { capabilities: ['coding', 'structured-output', 'subscription-auth', 'governed-actions'], qualityTier: 4, costTier: 0, latencyTier: 2 } },
    { id: 'claude', label: 'Anthropic Claude Code', executable: 'claude', harnessFamily: 'claude-code', baseArgs: ['-p', '--output-format', 'json', '--permission-mode', 'plan'], promptMode: 'stdin', secretEnvKeys: ['ANTHROPIC_API_KEY', 'ANTHROPIC_AUTH_TOKEN', 'ANTHROPIC_BASE_URL'], modelCatalog: ['sonnet', 'opus'], effortFlag: '--effort', effortLevels: ['low', 'medium', 'high'], effortByModel: { opus: ['low', 'medium', 'high', 'xhigh', 'max'] }, profile: { capabilities: ['coding', 'structured-output', 'subscription-auth', 'long-context', 'governed-actions'], qualityTier: 4, costTier: 0, latencyTier: 2 } },
    { id: 'gemini', label: 'Google Gemini CLI', executable: 'gemini', harnessFamily: 'gemini-cli', baseArgs: ['--output-format', 'json', '--approval-mode', 'plan'], promptMode: 'arg', promptFlag: '-p', secretEnvKeys: ['GEMINI_API_KEY', 'GOOGLE_API_KEY', 'GOOGLE_GENAI_USE_VERTEXAI', 'GOOGLE_CLOUD_PROJECT', 'GOOGLE_CLOUD_LOCATION', 'GOOGLE_APPLICATION_CREDENTIALS'], modelCatalog: ['auto', 'pro', 'flash', 'flash-lite'], profile: { capabilities: ['coding', 'structured-output', 'subscription-auth', 'long-context', 'governed-actions'], qualityTier: 3, costTier: 0, latencyTier: 2 } },
    { id: 'opencode', label: 'OpenCode', executable: 'opencode', harnessFamily: 'opencode-cli', baseArgs: ['run'], promptMode: 'arg', modelDiscoveryArgs: ['models', '--refresh', '--verbose', '--pure'], effortFlag: '--variant', profile: { capabilities: ['coding', 'structured-output', 'governed-actions'], qualityTier: 3, costTier: 1, latencyTier: 2 } },
    { id: 'github-copilot', label: 'GitHub Copilot CLI', executable: 'copilot', harnessFamily: 'github-copilot-cli', baseArgs: ['--mode', 'plan', '--sandbox', 'on', '--allow-tool', 'read', '--output-format', 'json', '--no-remote', '--no-remote-export', '--no-ask-user'], promptMode: 'arg', promptFlag: '--prompt', effortFlag: '--effort', effortLevels: ['none', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max'], modelCatalog: ['claude-sonnet-4.6', 'gpt-5.4', 'claude-haiku-4.5', 'gpt-5.3-codex', 'gemini-3.1-pro-preview', 'gemini-3.5-flash', 'gemini-3.6-flash', 'mai-code-1-flash'], profile: { capabilities: ['coding', 'structured-output', 'subscription-auth', 'governed-actions'], qualityTier: 4, costTier: 0, latencyTier: 2 } },
    { id: 'cursor-agent', label: 'Cursor Agent CLI', executable: 'cursor-agent', harnessFamily: 'cursor-agent-cli', baseArgs: ['-p', '--output-format', 'json'], promptMode: 'arg', modelFlag: '--model', profile: { capabilities: ['coding', 'structured-output', 'subscription-auth'], qualityTier: 4, costTier: 0, latencyTier: 2 } },
    { id: 'grok-build', label: 'xAI Grok Build', executable: 'agent', harnessFamily: 'grok-build-cli', baseArgs: ['--permission-mode', 'plan', '--output-format', 'json'], promptMode: 'arg', promptFlag: '-p', modelFlag: '--model', modelDiscoveryArgs: ['models'], effortFlag: '--reasoning-effort', effortLevels: ['low', 'medium', 'high', 'xhigh'], profile: { capabilities: ['coding', 'structured-output', 'subscription-auth', 'governed-actions'], qualityTier: 4, costTier: 0, latencyTier: 2 } },
    ...
    { id: 'auggie', label: 'Augment Auggie CLI', executable: 'auggie', harnessFamily: 'auggie-cli', baseArgs: ['--print', '--quiet', '--output-format', 'json', '--dont-save-session', '--ask'], promptMode: 'arg', modelFlag: '--model', modelDiscoveryArgs: ['models', 'list', '--json'], profile: { capabilities: ['coding', 'structured-output', 'subscription-auth'], qualityTier: 4, costTier: 0, latencyTier: 2 } },
    { id: 'amp', label: 'Sourcegraph Amp', executable: 'amp', harnessFamily: 'amp-cli', baseArgs: ['--execute', '--stream-json'], promptMode: 'arg', modelFlag: null, modelSelection: 'cli-config', executionSafety: 'external-plan-config-required', profile: { capabilities: ['coding', 'structured-output', 'subscription-auth'], qualityTier: 4, costTier: 0, latencyTier: 2 } },
    { id: 'amazon-q', label: 'Amazon Q Developer CLI', executable: 'q', harnessFamily: 'amazon-q-cli', baseArgs: ['chat', '--no-interactive'], promptMode: 'arg', modelFlag: null, modelSelection: 'cli-config', executionSafety: 'external-plan-config-required', profile: { capabilities: ['coding', 'structured-output', 'subscription-auth'], qualityTier: 3, costTier: 0, latencyTier: 2 } },
    { id: 'crush', label: 'Charm Crush', executable: 'crush', harnessFamily: 'charm-crush-cli', baseArgs: ['run'], promptMode: 'arg', modelFlag: null, modelSelection: 'cli-config', executionSafety: 'external-plan-config-required', profile: { capabilities: ['coding', 'structured-output'], qualityTier: 3, costTier: 1, latencyTier: 2 } },
    { id: 'roo-code', label: 'Roo Code CLI', executable: 'roo', harnessFamily: 'roo-code-cli', baseArgs: [], promptMode: 'arg', modelFlag: null, modelSelection: 'cli-config', executionSafety: 'external-plan-config-required', profile: { capabilities: ['coding', 'structured-output'], qualityTier: 3, costTier: 1, latencyTier: 2 } },
    ...
    { id: 'aider', label: 'Aider', executable: 'aider', harnessFamily: 'aider-cli', baseArgs: ['--no-auto-commits'], promptMode: 'arg', promptFlag: '--message', effortFlag: '--reasoning-effort', effortLevels: ['low', 'medium', 'high'], executionSafety: 'external-plan-config-required', profile: { capabilities: ['coding'], qualityTier: 3, costTier: 1, latencyTier: 2 } },
    ...
    { id: 'qoder', label: 'Qoder CLI', executable: 'qodercli', harnessFamily: 'qoder-cli', baseArgs: ['--permission-mode', 'dont_ask', '--tools', 'Read', 'Grep', 'Glob', '--allowed-tools', 'Read,Grep,Glob'], promptMode: 'arg', promptFlag: '-p', modelFlag: '--model', modelDiscoveryArgs: ['--list-models'], profile: { capabilities: ['coding', 'structured-output', 'subscription-auth', 'long-context', 'governed-actions'], qualityTier: 4, costTier: 0, latencyTier: 2 } },
    { id: 'pi', label: 'Pi Coding Agent', executable: 'pi', harnessFamily: 'pi-coding-agent-cli', baseArgs: ['--print', '--no-approve', '--tools', 'read,grep,find,ls', '--no-extensions', '--no-skills'], promptMode: 'arg', modelFlag: '--model', profile: { capabilities: ['coding', 'structured-output', 'subscription-auth', 'governed-actions'], qualityTier: 3, costTier: 1, latencyTier: 2 } },
    { id: 'kilo', label: 'Kilo Code CLI', executable: 'kilo', harnessFamily: 'kilo-code-cli', baseArgs: ['run'], promptMode: 'arg', modelFlag: null, modelSelection: 'cli-config', modelDiscoveryArgs: ['models'], executionSafety: 'external-plan-config-required', profile: { capabilities: ['coding', 'structured-output', 'subscription-auth'], qualityTier: 3, costTier: 1, latencyTier: 2 } },
