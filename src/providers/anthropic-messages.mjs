...
export class AnthropicMessagesProvider {
  ...
  async complete({ messages = [], tools = [], signal = null, model = this.model } = {}) {
    ...
    const payload = await postJson({ url: this.url, headers: { 'x-api-key': key, 'anthropic-version': '2023-06-01' }, body: { model, max_tokens: this.maxTokens, ...(system ? { system } : {}), messages: conversation, ...(tools.length ? { tools: tools.map((tool) => { const value = functionTool(tool); return { name: value.name, description: value.description, input_schema: value.parameters }; }) } : {}) }, timeoutMs: this.timeoutMs, signal, fetchImpl: this.fetchImpl, secretValues: [key] });
