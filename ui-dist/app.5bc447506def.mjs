import { createRouter } from './core/router.a2bca650e1d3.mjs';
import { createUiStore } from './core/ui-store.5cb40f3ff000.mjs';
import { createUiBus } from './core/ui-bus.4734624cfff0.mjs';
import { createApiClient } from './core/api-client.9af948e12f1b.mjs';
import { createLayoutStore } from './core/layout-store.1e93374a35e7.mjs';
import { createResizableRegionController } from './core/resizable-region.a2cc6935386f.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.fe0479d40bef.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.6bd43f4335a4.mjs';
import { normalizeExperience } from './core/experience-policy.a68521ed8baf.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.3f197d3eaee7.mjs';
import { createViewStateBridge } from './core/view-state-bridge.6207485c18ac.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.e222428670f0.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.a07bbd3907ee.mjs';
import { createUpdateStateController } from './core/update-state-controller.aa04340c956d.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.63af5726a46b.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.1ee2e6e72753.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.65064629536f.mjs';
import { projectNameFromPath } from './shell/project-picker.e6ef02859065.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.765a5a155de4.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.4beb5b2aa4ab.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.ee64e816aa24.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.df535f3afc82.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.820acb25efc2.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.c309cf8cc11f.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.ff873a21ad19.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.8d820c78e49b.mjs'), import('./views/workroom/terminal-client.4ead92cc2a5b.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.730cc922bd69.mjs'), import('./control-plane/route-registry.3474e826d49b.mjs'), import('./control-plane/live-domain-workspace.1fcc61d7c37c.mjs'), import('./views/browser/browser-view.fa6d27c04668.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.2aa9ba202d9a.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.bdf5dd1e1036.mjs'), import('./views/onboarding/onboarding-view.21e591ae3a0a.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.7d8ae2660cd8.mjs'), import('./views/settings/settings-view.ee18f9c5cc7a.mjs')]);
