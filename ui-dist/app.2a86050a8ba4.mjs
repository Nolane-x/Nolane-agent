import { createRouter } from './core/router.190587fb4fbd.mjs';
import { createUiStore } from './core/ui-store.154cadc5ca68.mjs';
import { createUiBus } from './core/ui-bus.69713cf09432.mjs';
import { createApiClient } from './core/api-client.d4dbf3bbb9cf.mjs';
import { createLayoutStore } from './core/layout-store.4db7d32e6252.mjs';
import { createResizableRegionController } from './core/resizable-region.a23d44ea20e3.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.5ba0c57f9e91.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.9dda913a4c2a.mjs';
import { normalizeExperience } from './core/experience-policy.bc2b9958fe00.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.bd45230a6ce9.mjs';
import { createViewStateBridge } from './core/view-state-bridge.207020f3df4e.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.3df8512ed2cc.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.0949e1533f90.mjs';
import { createUpdateStateController } from './core/update-state-controller.9baa40cc4386.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.a35d4c89a8dc.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.90ad9e9dc97b.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.237385d7f9d7.mjs';
import { projectNameFromPath } from './shell/project-picker.24fa710949df.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.ca9263c25278.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.a1e7b6b44066.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.27e54d9b9030.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.15e5cba63cfc.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.083bebc36300.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.3752b42fa8ca.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.c56b01d5fda7.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.b7083572e2dc.mjs'), import('./control-plane/route-registry.453c1b814115.mjs'), import('./control-plane/live-domain-workspace.4bbac674da80.mjs'), import('./views/browser/browser-view.dcd05c9e45a2.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.90b35bee50d7.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.a7bb66322591.mjs'), import('./views/onboarding/onboarding-view.73a924ccd188.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.f12dd82544f7.mjs'), import('./views/settings/settings-view.aa8faff2d980.mjs')]);
