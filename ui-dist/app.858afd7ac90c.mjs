import { createRouter } from './core/router.72846177d72d.mjs';
import { createUiStore } from './core/ui-store.d1b008b4fa46.mjs';
import { createUiBus } from './core/ui-bus.b9cc4f097974.mjs';
import { createApiClient } from './core/api-client.8e68cfa3fdf6.mjs';
import { createLayoutStore } from './core/layout-store.7923b407bca2.mjs';
import { createResizableRegionController } from './core/resizable-region.43b9abd257b2.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.2d6e4acf801a.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.1d9464042dca.mjs';
import { normalizeExperience } from './core/experience-policy.b6b67032d1d9.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.fab59bcbd5d3.mjs';
import { createViewStateBridge } from './core/view-state-bridge.1bfdee80154e.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.ca6b3950b5e1.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.490c0dc3a10a.mjs';
import { createUpdateStateController } from './core/update-state-controller.80fec89a0c94.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.c024edbbe1dd.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.d2637c3d3a2e.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.c201261368f9.mjs';
import { projectNameFromPath } from './shell/project-picker.24bd145b1d6a.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.eb5c6ace4177.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.daeb377d2bf7.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.2c04174a262b.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.0932ef7d35a4.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.063284a03f1d.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.c1bfb9d130b4.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.bbcb76578016.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.baeb6073173d.mjs'), import('./views/workroom/terminal-client.e9b467475475.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.36a807092f67.mjs'), import('./control-plane/route-registry.f078aa861eca.mjs'), import('./control-plane/live-domain-workspace.f4f5d61d7d24.mjs'), import('./views/browser/browser-view.e6bb06153fdb.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.d8d7f2e68015.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.053940faf692.mjs'), import('./views/onboarding/onboarding-view.f85370fff4b5.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.b658c1e5aa89.mjs'), import('./views/settings/settings-view.62b00905c3bd.mjs')]);
