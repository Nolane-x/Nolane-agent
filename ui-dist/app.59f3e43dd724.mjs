import { createRouter } from './core/router.ba6a6bca0d69.mjs';
import { createUiStore } from './core/ui-store.710c41a3e6db.mjs';
import { createUiBus } from './core/ui-bus.daccd9fc7a6d.mjs';
import { createApiClient } from './core/api-client.5230999e6109.mjs';
import { createLayoutStore } from './core/layout-store.95a3861f993a.mjs';
import { createResizableRegionController } from './core/resizable-region.525ba2f22f47.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.034715a2fd34.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.d876766907da.mjs';
import { normalizeExperience } from './core/experience-policy.90d7183c14d8.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.46fdc08e4956.mjs';
import { createViewStateBridge } from './core/view-state-bridge.486304f9dcd6.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.f3eecbb44b50.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.f831eecd0b69.mjs';
import { createUpdateStateController } from './core/update-state-controller.f950a0e5fda0.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.a9dc7ac97c26.mjs';
import { settingsSectionFromRoute } from './core/settings-route.ac660b3cfd27.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.f9bdc96823d1.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.3bf0908b9537.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.f6d28b12ad8a.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.40ec6d237d71.mjs';
import { projectNameFromPath } from './shell/project-picker.dcc08c47fb36.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.419c9a3f3d39.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.98037ffccd69.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.19bd691fc58d.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.c5decc9e6aca.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.d63daa8bc395.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.c7b3096b20b6.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.ed1eb06a8e79.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.396766968260.mjs'), import('./views/workroom/terminal-client.1704b9eb3f5c.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.43fbc29a94f7.mjs'), import('./control-plane/route-registry.09356449a575.mjs'), import('./control-plane/live-domain-workspace.22f733931d3b.mjs'), import('./views/browser/browser-view.44487f2171c9.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.a169f9608b0b.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.d1175d1ed711.mjs'), import('./views/onboarding/onboarding-view.b1cc18e14b12.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.240fed9e42d9.mjs'), import('./views/settings/settings-view.765bdf6dd3dd.mjs')]);
