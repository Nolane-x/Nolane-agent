import { createRouter } from './core/router.de7d600bd182.mjs';
import { createUiStore } from './core/ui-store.8396d369c104.mjs';
import { createUiBus } from './core/ui-bus.80475d0eb8f7.mjs';
import { createApiClient } from './core/api-client.a95426aeaba8.mjs';
import { createLayoutStore } from './core/layout-store.3d43f6e1c80d.mjs';
import { createResizableRegionController } from './core/resizable-region.49cf558206bc.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.73d22b3d23ee.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.93b540a3df42.mjs';
import { normalizeExperience } from './core/experience-policy.4587c76f4e4a.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.1bff6a956cc9.mjs';
import { createViewStateBridge } from './core/view-state-bridge.181709a68f76.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.dad49f6156c6.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.4b37a176c0b5.mjs';
import { createUpdateStateController } from './core/update-state-controller.57393f265fb1.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.991559ae3754.mjs';
import { settingsSectionFromRoute } from './core/settings-route.740c9d39f674.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.044ddd8901ed.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.34b8fe6b8351.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.cbbebd599e50.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.bf494744fa59.mjs';
import { projectNameFromPath } from './shell/project-picker.d1adbc61ea5e.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.15bcabe5b453.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.bfc38831b3a0.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.7bf0e6bd4fd0.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.f930982f284b.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.e2d2354c6fe8.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.daa3a444cce6.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.9b24b155c347.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.c2d30c60432a.mjs'), import('./views/workroom/terminal-client.4e7c2ca08d04.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.7a6315098d4d.mjs'), import('./control-plane/route-registry.f2438561e8b2.mjs'), import('./control-plane/live-domain-workspace.7787892bbcc6.mjs'), import('./views/browser/browser-view.8ff852778b22.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.386a082fffaf.mjs'), import('./views/settings/settings-view.75ed49368903.mjs')]);
