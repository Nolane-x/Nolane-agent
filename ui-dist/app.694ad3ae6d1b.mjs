import { createRouter } from './core/router.baf46a7df9b0.mjs';
import { createUiStore } from './core/ui-store.db95b5c3f957.mjs';
import { createUiBus } from './core/ui-bus.688a743a375f.mjs';
import { createApiClient } from './core/api-client.360078cd8810.mjs';
import { createLayoutStore } from './core/layout-store.c1400d3193d9.mjs';
import { createResizableRegionController } from './core/resizable-region.1885a4c111a5.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.08c306bc862e.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.3321c0bc2982.mjs';
import { normalizeExperience } from './core/experience-policy.6008dc729d7b.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.494c528d4f7a.mjs';
import { createViewStateBridge } from './core/view-state-bridge.c542cfb5d441.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.1c29353494c6.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.0af83527069f.mjs';
import { createUpdateStateController } from './core/update-state-controller.cbf6df282620.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.cbbdf6f7b17e.mjs';
import { settingsSectionFromRoute } from './core/settings-route.0f52ca728a3d.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.f97b33df67f7.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.08ca194c72f1.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.14fff2ba94e0.mjs';
import { projectNameFromPath } from './shell/project-picker.7588fcbb43aa.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.53acb89d6b30.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.87c1606d1420.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.43b3fdc933e6.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.8bca06efaff4.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.30bc1ad51a49.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.984513970fb8.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.a1f8cfa4ec75.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.c19db3b22eaa.mjs'), import('./views/workroom/terminal-client.ce73cffeb648.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.6dc6dab31287.mjs'), import('./control-plane/route-registry.b907952f0317.mjs'), import('./control-plane/live-domain-workspace.22b746261b77.mjs'), import('./views/browser/browser-view.f7e72df469eb.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.e76a644ec03a.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.1cad1465e917.mjs'), import('./views/onboarding/onboarding-view.18d801cb010f.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.2a6e16072a4e.mjs'), import('./views/settings/settings-view.5e3363aed8ef.mjs')]);
