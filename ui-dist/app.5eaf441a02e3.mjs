import { createRouter } from './core/router.3904acaea590.mjs';
import { createUiStore } from './core/ui-store.4db34baf9c8d.mjs';
import { createUiBus } from './core/ui-bus.619a1421ef52.mjs';
import { createApiClient } from './core/api-client.c5d6ef702d93.mjs';
import { createLayoutStore } from './core/layout-store.35919d62f286.mjs';
import { createResizableRegionController } from './core/resizable-region.d4a21ac5f9aa.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.0087fc8879d6.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.107e9b8e81d7.mjs';
import { normalizeExperience } from './core/experience-policy.ea4eaa867687.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.5611535823fa.mjs';
import { createViewStateBridge } from './core/view-state-bridge.14c15b6582f8.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.19d00372de20.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.55917d1a9939.mjs';
import { createUpdateStateController } from './core/update-state-controller.8d225c541c44.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.4f590f67a413.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.9a4fefc74d96.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.481b37f286f4.mjs';
import { projectNameFromPath } from './shell/project-picker.66ff5debd3f4.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.e343f805f9a3.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.7d5ccaa5c286.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.b8c77db216cc.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.b171827dac86.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.47b1ceb0981f.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.91963b5a64b4.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.35e791fd0133.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.9dc230a62b3d.mjs'), import('./control-plane/route-registry.9ba5fd0ae56c.mjs'), import('./control-plane/live-domain-workspace.9e02d235d3a5.mjs'), import('./views/browser/browser-view.c2228af4ccc8.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.b2bb4b8bfd7b.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.42f061d8c8ca.mjs'), import('./views/onboarding/onboarding-view.425f0b8d198b.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.ce113c2cdb97.mjs'), import('./views/settings/settings-view.25a0ce210198.mjs')]);
