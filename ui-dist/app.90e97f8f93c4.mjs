import { createRouter } from './core/router.713c80801fea.mjs';
import { createUiStore } from './core/ui-store.fa234caba1f3.mjs';
import { createUiBus } from './core/ui-bus.a808be1139d9.mjs';
import { createApiClient } from './core/api-client.e85e59051d76.mjs';
import { createLayoutStore } from './core/layout-store.96a9f2054c43.mjs';
import { createResizableRegionController } from './core/resizable-region.a469d0d4ce1c.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.fe74f15c1850.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.98ff72440b81.mjs';
import { normalizeExperience } from './core/experience-policy.8c30dc615ac2.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.23c7a8373f40.mjs';
import { createViewStateBridge } from './core/view-state-bridge.8a9e97c11c7c.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.5ed1833a6f15.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.35a5cc6bff23.mjs';
import { createUpdateStateController } from './core/update-state-controller.773ce5c76626.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.98de8e16c184.mjs';
import { settingsSectionFromRoute } from './core/settings-route.e0e27610c206.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.1efc2a6f79e9.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.57dae634e12d.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.070aed41c12c.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.eb9c98e09a0f.mjs';
import { projectNameFromPath } from './shell/project-picker.62c11cda7665.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.20f9ad5c4c8e.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.52753af74326.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.3a9ac1c699b3.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.442065f22a89.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.e1c928ea31d2.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.bd0af596b358.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.b54b8f93aec1.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.616c028777ec.mjs'), import('./views/workroom/terminal-client.bfac76cb83d9.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.03952f668526.mjs'), import('./control-plane/route-registry.44122748623c.mjs'), import('./control-plane/live-domain-workspace.8c2cdeceb682.mjs'), import('./views/browser/browser-view.ba12285d6d0a.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.6fc1690f2ee4.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.b20817e07942.mjs'), import('./views/onboarding/onboarding-view.5b4b84cc8655.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.25b489348cbb.mjs'), import('./views/settings/settings-view.c66ab842f1e2.mjs')]);
