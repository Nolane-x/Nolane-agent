import { createRouter } from './core/router.b9fc3fdc9843.mjs';
import { createUiStore } from './core/ui-store.6ae0ef9335c8.mjs';
import { createUiBus } from './core/ui-bus.693359d7e9f5.mjs';
import { createApiClient } from './core/api-client.81b3ddf32b9c.mjs';
import { createLayoutStore } from './core/layout-store.7b4dcef068f8.mjs';
import { createResizableRegionController } from './core/resizable-region.96580d9b80ce.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.34bf07f0bf9a.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.d6b4366914a4.mjs';
import { normalizeExperience } from './core/experience-policy.73173839b2ad.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.8c4b76d3a835.mjs';
import { createViewStateBridge } from './core/view-state-bridge.d14c9a0bdb3a.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.a0f685487b0c.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.c821da251bf7.mjs';
import { createUpdateStateController } from './core/update-state-controller.2c5339306a83.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.347c648c86e1.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.de4db0bfb7ed.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.15a595a9ac76.mjs';
import { projectNameFromPath } from './shell/project-picker.646e084d1b7a.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.1d3f04aa18df.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.001d2ba94628.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.68b7ccb96fc7.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.3f078ab7028b.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.369e709f0000.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.6733ac08d9bf.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.14bc44c781e5.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.8fe6d8c59506.mjs'), import('./control-plane/route-registry.c7c2e1b94875.mjs'), import('./control-plane/live-domain-workspace.2c4d34db9411.mjs'), import('./views/browser/browser-view.5655a4663243.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.403105ec4d08.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.5101df8cbf41.mjs'), import('./views/onboarding/onboarding-view.84fadb9d4139.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.17ea65f7d38e.mjs'), import('./views/settings/settings-view.a12606f190f3.mjs')]);
