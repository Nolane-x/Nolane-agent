import { createRouter } from './core/router.cfd3da547374.mjs';
import { createUiStore } from './core/ui-store.88ca14f34f7c.mjs';
import { createUiBus } from './core/ui-bus.b9da004c222c.mjs';
import { createApiClient } from './core/api-client.f400d63e4ac1.mjs';
import { createLayoutStore } from './core/layout-store.46f5de0f030c.mjs';
import { createResizableRegionController } from './core/resizable-region.0a81c9e25def.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.218174a98b02.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.1f795ba35989.mjs';
import { normalizeExperience } from './core/experience-policy.f2a3b8832166.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.6490d4aaa62e.mjs';
import { createViewStateBridge } from './core/view-state-bridge.66b7aeb8fc90.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.500392a29d35.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.d9a6d425e433.mjs';
import { createUpdateStateController } from './core/update-state-controller.ed5b643aa115.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.0596c47cb0a6.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.6f297bd44b50.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.830871bf661e.mjs';
import { projectNameFromPath } from './shell/project-picker.9f941a33840c.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.0765424d5d1e.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.caf7acb4dc8c.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.2c6f6dae86cb.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.74d3b6533685.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.5fcaad25a8f9.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.9adcd182879c.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.4f08f47c88ba.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.94535f78c8b7.mjs'), import('./views/workroom/terminal-client.dfc9952a13c0.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.2dec82b56800.mjs'), import('./control-plane/route-registry.cfa0502b2d27.mjs'), import('./control-plane/live-domain-workspace.8fafa0613981.mjs'), import('./views/browser/browser-view.126cb745fcda.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.8255cab43c87.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.020f380048ee.mjs'), import('./views/onboarding/onboarding-view.61a2df32251d.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.8ff1b5d44e59.mjs'), import('./views/settings/settings-view.0362d003a178.mjs')]);
