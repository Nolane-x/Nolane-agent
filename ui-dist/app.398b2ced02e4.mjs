import { createRouter } from './core/router.583b60e903d8.mjs';
import { createUiStore } from './core/ui-store.226cc5b6eade.mjs';
import { createUiBus } from './core/ui-bus.b2c30a63bfd3.mjs';
import { createApiClient } from './core/api-client.24ab63ae93bb.mjs';
import { createLayoutStore } from './core/layout-store.ac16ef9ccf39.mjs';
import { createResizableRegionController } from './core/resizable-region.ebc38c653d5a.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.0e43bba5cd65.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.1f427053ece0.mjs';
import { normalizeExperience } from './core/experience-policy.ab21d28cf20c.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.4d1f89916955.mjs';
import { createViewStateBridge } from './core/view-state-bridge.a758b706a4fc.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.d97a946d81a6.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.2eac7c5b4aab.mjs';
import { createUpdateStateController } from './core/update-state-controller.562b5179fbee.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.e9bbb3c91cd3.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.3e45b9cc939b.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.9e21fc3103de.mjs';
import { projectNameFromPath } from './shell/project-picker.6ac4c33f19eb.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.b59837a97815.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.620033276faf.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.f33fcdeec588.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.93f3d0342721.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.b8d79777da9b.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.c1ed659e6516.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.6fb0c01ea9ef.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.05c73192b7b3.mjs'), import('./views/workroom/terminal-client.75090ad4cddd.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.09f53c794182.mjs'), import('./control-plane/route-registry.b70ace9f3ac8.mjs'), import('./control-plane/live-domain-workspace.43a37beb95e1.mjs'), import('./views/browser/browser-view.93df9f046321.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.ec1706aae4de.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.1ec29472911f.mjs'), import('./views/onboarding/onboarding-view.73fe34cd956c.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.fe7af30f4a24.mjs'), import('./views/settings/settings-view.db650a7dff2e.mjs')]);
