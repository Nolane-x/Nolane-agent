import { createRouter } from './core/router.76a311789ef6.mjs';
import { createUiStore } from './core/ui-store.02e157d57cb5.mjs';
import { createUiBus } from './core/ui-bus.f513fe8961d6.mjs';
import { createApiClient } from './core/api-client.083b0734c3a9.mjs';
import { createLayoutStore } from './core/layout-store.b715e014b529.mjs';
import { createResizableRegionController } from './core/resizable-region.84e865ce9ea5.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.d09d3e49467e.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.1f73ac3b4caf.mjs';
import { normalizeExperience } from './core/experience-policy.9d0c5cdd3ff6.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.c8414c12d3cb.mjs';
import { createViewStateBridge } from './core/view-state-bridge.62b34adf80fc.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.292c4ecfed84.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.788a70159b94.mjs';
import { createUpdateStateController } from './core/update-state-controller.c6595746313d.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.bff5eaae3062.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.773675145ffd.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.61c8929c7a9b.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.00e9b880e0a1.mjs';
import { projectNameFromPath } from './shell/project-picker.71b2b68c7a69.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.1e73bc5629e8.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.5c017eeace85.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.42001cd96e5c.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.929a3ac719d1.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.06462fbe8218.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.a1be9bf39c5e.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.f9dba8e2e759.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.53342aed4194.mjs'), import('./views/workroom/terminal-client.47962ea0404b.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.ef652bd8a4a2.mjs'), import('./control-plane/route-registry.6b0cd3c58dbf.mjs'), import('./control-plane/live-domain-workspace.6821f79f46a2.mjs'), import('./views/browser/browser-view.d7a9d55f8bda.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.8c79c7291009.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.fc8f90fdea3d.mjs'), import('./views/onboarding/onboarding-view.74d3dc5f4104.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.abff693241f4.mjs'), import('./views/settings/settings-view.9f13a3a27b11.mjs')]);
