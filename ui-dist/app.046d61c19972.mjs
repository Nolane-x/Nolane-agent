import { createRouter } from './core/router.27ab3db91077.mjs';
import { createUiStore } from './core/ui-store.3fad77dd321d.mjs';
import { createUiBus } from './core/ui-bus.6b4f9ce664d4.mjs';
import { createApiClient } from './core/api-client.03c95bf5aec2.mjs';
import { createLayoutStore } from './core/layout-store.114626c17ebc.mjs';
import { createResizableRegionController } from './core/resizable-region.8c8b9fbf510d.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.cacc41f02642.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.30e3ebf720dd.mjs';
import { normalizeExperience } from './core/experience-policy.7f6fe4ef0cdf.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.ddf987d8b8f3.mjs';
import { createViewStateBridge } from './core/view-state-bridge.fa6573ba4a7a.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.68c8e12322ad.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.4822e42cb30f.mjs';
import { createUpdateStateController } from './core/update-state-controller.e8ec95bec417.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.c0c3c47faa99.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.926f8260d88c.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.7dfe28138e01.mjs';
import { projectNameFromPath } from './shell/project-picker.77ef9d994ea2.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.b1da484418b0.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.0ab2d6e42363.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.6f789984b4ec.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.3f08348b0dfd.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.eb56bdc6685d.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.dc9a1953404e.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.c9b66d204175.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.526661e406d3.mjs'), import('./views/workroom/terminal-client.c5d6f0e39a8d.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.549681bee866.mjs'), import('./control-plane/route-registry.ea186e8976e1.mjs'), import('./control-plane/live-domain-workspace.d0777f84c08a.mjs'), import('./views/browser/browser-view.7a8cb2ccc9b6.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.73b46f760885.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.99b9f7863ee0.mjs'), import('./views/onboarding/onboarding-view.834ba6890073.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.710222000f0a.mjs'), import('./views/settings/settings-view.f1beab748d06.mjs')]);
