import { createRouter } from './core/router.06dac4e2794c.mjs';
import { createUiStore } from './core/ui-store.beba83e68d0c.mjs';
import { createUiBus } from './core/ui-bus.fd3389b62f25.mjs';
import { createApiClient } from './core/api-client.9990c24b2882.mjs';
import { createLayoutStore } from './core/layout-store.4c3e5e058b5d.mjs';
import { createResizableRegionController } from './core/resizable-region.be37f4ad4d83.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.cf326f4d7fcb.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.792558449025.mjs';
import { normalizeExperience } from './core/experience-policy.2dfb7ff039a1.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.145bba1c4426.mjs';
import { createViewStateBridge } from './core/view-state-bridge.81afecced4d0.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.9f893d9bb3f4.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.949666749643.mjs';
import { createUpdateStateController } from './core/update-state-controller.77b2b0fa2644.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.41c933d00c37.mjs';
import { settingsSectionFromRoute } from './core/settings-route.62c72fdc4cfb.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.696258ca64c1.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.c0e41029a384.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.e774988e5972.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.f638978bb8f7.mjs';
import { projectNameFromPath } from './shell/project-picker.b4ed9d2f9268.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.3651d05f3107.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.7e616899804d.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.67c0b7071b98.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.e232a8a14286.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.9d08fb2425e5.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.f9015537ddca.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.e5be981acce2.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.296127407050.mjs'), import('./views/workroom/terminal-client.c8267179b7a7.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.ef83d0c3a0ab.mjs'), import('./control-plane/route-registry.e0c4eb03b44f.mjs'), import('./control-plane/live-domain-workspace.c95a6dc2e6d1.mjs'), import('./views/browser/browser-view.793343b0b721.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.5e9283e04d28.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.b5ab646122b1.mjs'), import('./views/onboarding/onboarding-view.a41a88b10244.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.d2c5484ee80b.mjs'), import('./views/settings/settings-view.03d184695a07.mjs')]);
