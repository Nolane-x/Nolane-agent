import { createRouter } from './core/router.434d638bb650.mjs';
import { createUiStore } from './core/ui-store.20c895e0d0f8.mjs';
import { createUiBus } from './core/ui-bus.596aa99d82b4.mjs';
import { createApiClient } from './core/api-client.424635647e1f.mjs';
import { createLayoutStore } from './core/layout-store.e9283f781aeb.mjs';
import { createResizableRegionController } from './core/resizable-region.7b273b5ca86f.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.795a44b8ef1e.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.f1130ed04138.mjs';
import { normalizeExperience } from './core/experience-policy.99fd45a79f06.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.943d6ac1a1dc.mjs';
import { createViewStateBridge } from './core/view-state-bridge.5db640fbf4a2.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.13eaa4349739.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.e941023ede24.mjs';
import { createUpdateStateController } from './core/update-state-controller.e79be8aa1bd6.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.8598a54ca6b7.mjs';
import { settingsSectionFromRoute } from './core/settings-route.8221ef7ee2d4.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.19302221d4fa.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.23e3faebc78e.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.4771878bcf81.mjs';
import { projectNameFromPath } from './shell/project-picker.361067648e4f.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.a6bb09f8e004.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.5f26cd0fdd28.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.ffaea1497adf.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.a952fe6373c3.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.640c16a1b284.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.969334194aa6.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.c720015818be.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.d2d109344a5b.mjs'), import('./views/workroom/terminal-client.6ae2bf074353.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.e33a467c7334.mjs'), import('./control-plane/route-registry.cf2a1c87a8ce.mjs'), import('./control-plane/live-domain-workspace.f3db8dfe16f6.mjs'), import('./views/browser/browser-view.e40e8b18ccd7.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.4794a4da213f.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.77d55e04cf55.mjs'), import('./views/onboarding/onboarding-view.71987b75b86b.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.406f29847056.mjs'), import('./views/settings/settings-view.ccd5c1eed31e.mjs')]);
