import { createRouter } from './core/router.6f4913f8e8b0.mjs';
import { createUiStore } from './core/ui-store.7d2245d9e61b.mjs';
import { createUiBus } from './core/ui-bus.6afb095655ac.mjs';
import { createApiClient } from './core/api-client.2a890a47b2c6.mjs';
import { createLayoutStore } from './core/layout-store.ce220b9d6cd4.mjs';
import { createResizableRegionController } from './core/resizable-region.3978628d2edc.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.4d17832115c9.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.3421666c3459.mjs';
import { normalizeExperience } from './core/experience-policy.92fe2fb9ff35.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.4d6267a64814.mjs';
import { createViewStateBridge } from './core/view-state-bridge.1389ce82baa0.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.c622fe5a8ecb.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.0ec7b73bb6eb.mjs';
import { createUpdateStateController } from './core/update-state-controller.592a246253d3.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.16f0e91bcf18.mjs';
import { settingsSectionFromRoute } from './core/settings-route.2e1234664325.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.22f0d1e1320d.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.e04472bf6bbd.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.ab0427fe5a0d.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.bf208d3d856a.mjs';
import { projectNameFromPath } from './shell/project-picker.563e8d5a8ab1.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.c0ffe641ac2e.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.9949c54f7fda.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.b4ce3b10a758.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.37b596d823ba.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.3c1c38d2caef.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.3cb5e8e79edb.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.d873f59461ec.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.2a7b366e2a07.mjs'), import('./views/workroom/terminal-client.7af9d6886b7d.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.48cde849515e.mjs'), import('./control-plane/route-registry.0a2a25f4985d.mjs'), import('./control-plane/live-domain-workspace.80b245bb8645.mjs'), import('./views/browser/browser-view.4cd25d65298e.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.4b256c8d5811.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.415ef092be5a.mjs'), import('./views/onboarding/onboarding-view.41410c9c0f1a.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.52c59cba25e8.mjs'), import('./views/settings/settings-view.d05825c6fd5d.mjs')]);
