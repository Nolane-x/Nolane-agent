import { createRouter } from './core/router.8a09f0c120ae.mjs';
import { createUiStore } from './core/ui-store.ea4e74a45673.mjs';
import { createUiBus } from './core/ui-bus.b918c83d5c50.mjs';
import { createApiClient } from './core/api-client.713419d35cdc.mjs';
import { createLayoutStore } from './core/layout-store.0cfbb0dbcac6.mjs';
import { createResizableRegionController } from './core/resizable-region.d3363b61678a.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.5308e2660c7c.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.f18e2c1cef03.mjs';
import { normalizeExperience } from './core/experience-policy.f08ff60361b3.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.c8fbcffc77b6.mjs';
import { createViewStateBridge } from './core/view-state-bridge.be5e7b78d391.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.b3bcc7ff56dc.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.75b8d695b47b.mjs';
import { createUpdateStateController } from './core/update-state-controller.2a527c398e53.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.b86fced00463.mjs';
import { settingsSectionFromRoute } from './core/settings-route.e877d569d7b2.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.257ba021c0ea.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.74c641d359e2.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.a1b867f57edd.mjs';
import { closeComposerPickers, isComposerPickerInteraction } from './components/composer-picker.ed8f99c27432.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.072006aaf1b2.mjs';
import { projectNameFromPath } from './shell/project-picker.dfed82c1994f.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.b70fefde205d.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.3e6a48d465fa.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.79dab22ae995.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.19a9e97d948f.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.cc7c4657ae66.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.5c6d8ae9bdc8.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.3ba7beb22664.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.e35f7cedc586.mjs'), import('./views/workroom/terminal-client.c92f5273a6b2.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.799d002c3488.mjs'), import('./control-plane/route-registry.44a5201051c1.mjs'), import('./control-plane/live-domain-workspace.ce88bc62f3fe.mjs'), import('./views/browser/browser-view.9e814e23032e.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.82421052380d.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.afc049875b09.mjs'), import('./views/onboarding/onboarding-view.4721735bb467.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.0219947a49cd.mjs'), import('./views/settings/settings-view.740345fd59ab.mjs')]);
