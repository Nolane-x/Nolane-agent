import { createRouter } from './core/router.784267ecbe3f.mjs';
import { createUiStore } from './core/ui-store.b045bedf480e.mjs';
import { createUiBus } from './core/ui-bus.ad12dc5f10de.mjs';
import { createApiClient } from './core/api-client.b1e2f576ac4c.mjs';
import { createLayoutStore } from './core/layout-store.0438ade43aee.mjs';
import { createResizableRegionController } from './core/resizable-region.9dc853f06805.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.d0701c4e45b6.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.c690b2961616.mjs';
import { normalizeExperience } from './core/experience-policy.f1f147a3534f.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.b061ce4be53f.mjs';
import { createViewStateBridge } from './core/view-state-bridge.b37acd581093.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.48c369df8bdb.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.46110a5d00d2.mjs';
import { createUpdateStateController } from './core/update-state-controller.31bcae6acb5e.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.6cb1b87973c0.mjs';
import { settingsSectionFromRoute } from './core/settings-route.7cd647908fb8.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.4a4b563c5beb.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.b3e841785298.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.422aab885a87.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.dbf5856320a4.mjs';
import { projectNameFromPath } from './shell/project-picker.07c7bb0c0ee7.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.3924894e99b8.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.825ebdb141a6.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.6ec0811e0708.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.a4c19e7372dc.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.549efff120f5.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.5d02e35b508f.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.0dc152535fbb.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.a18a29e4f90c.mjs'), import('./views/workroom/terminal-client.be076c9c7015.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.367d4ca1ec86.mjs'), import('./control-plane/route-registry.2383dca92729.mjs'), import('./control-plane/live-domain-workspace.cca131483c77.mjs'), import('./views/browser/browser-view.8c625d49c994.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.03ea63dd4c2e.mjs'), import('./views/settings/settings-view.965d7f26c5c5.mjs')]);
