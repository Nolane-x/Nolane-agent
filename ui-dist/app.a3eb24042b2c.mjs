import { createRouter } from './core/router.868f542b62ec.mjs';
import { createUiStore } from './core/ui-store.006a79499030.mjs';
import { createUiBus } from './core/ui-bus.be4d3e96d042.mjs';
import { createApiClient } from './core/api-client.a1fa66d03da1.mjs';
import { createLayoutStore } from './core/layout-store.db2899ce379b.mjs';
import { createResizableRegionController } from './core/resizable-region.476eeb2f7e1c.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.def84ced0b11.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.10f10cf648ed.mjs';
import { normalizeExperience } from './core/experience-policy.550b575e0ded.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.4e00f1a8465e.mjs';
import { createViewStateBridge } from './core/view-state-bridge.5e32ddce3831.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.fddf7b8da3b2.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.c6d08367e16b.mjs';
import { createUpdateStateController } from './core/update-state-controller.38ff86561c22.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.6ab30771a559.mjs';
import { settingsSectionFromRoute } from './core/settings-route.9a683e89fbcb.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.da7c5fd0b39c.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.869f48526426.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.3e3bc410578d.mjs';
import { projectNameFromPath } from './shell/project-picker.c6d25cdc3ef3.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.bdc23deb0a55.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.1148b5527023.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.9bb189a1a7c3.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.5c0cfb062d86.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.247717d5a4df.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.80ceb33b3159.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.18fbe20afd74.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.b61aa8545277.mjs'), import('./views/workroom/terminal-client.7f196cd89613.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.1483004f4fa7.mjs'), import('./control-plane/route-registry.e4abe8ef8fe3.mjs'), import('./control-plane/live-domain-workspace.1f9c86b332d2.mjs'), import('./views/browser/browser-view.efdd489e0041.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.534665a12ef6.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.1e280de6ce72.mjs'), import('./views/onboarding/onboarding-view.1d8d7676758b.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.26481dbd787b.mjs'), import('./views/settings/settings-view.3bbfa9d99f81.mjs')]);
