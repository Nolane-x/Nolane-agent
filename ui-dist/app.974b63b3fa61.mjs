import { createRouter } from './core/router.c0ba0e50fc2b.mjs';
import { createUiStore } from './core/ui-store.fcb2a195fb4e.mjs';
import { createUiBus } from './core/ui-bus.9414774a9918.mjs';
import { createApiClient } from './core/api-client.9ddf20e5133b.mjs';
import { createLayoutStore } from './core/layout-store.fb400ec12800.mjs';
import { createResizableRegionController } from './core/resizable-region.7f4475f9a6e7.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.67e3f592ebb1.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.034f7224aa9e.mjs';
import { normalizeExperience } from './core/experience-policy.1b058eb3c581.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.d9b8423e9362.mjs';
import { createViewStateBridge } from './core/view-state-bridge.cc26ac243506.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.2221676df3cf.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.a890ab6d4281.mjs';
import { createUpdateStateController } from './core/update-state-controller.4c032b61e1ce.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.8009ff937931.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.fcab1f639e47.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.ede831a515de.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.d6e5c24225b2.mjs';
import { projectNameFromPath } from './shell/project-picker.095871019958.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.b32cbb2e99d3.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.2db2c937a2b5.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.76fdb0e71f16.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.ea189c470dc0.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.1ec5519b3274.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.3f9e70419625.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.abcd692fcbf8.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.441e68d125fc.mjs'), import('./views/workroom/terminal-client.40f607215b8b.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.51c565ed5b89.mjs'), import('./control-plane/route-registry.2d684f491660.mjs'), import('./control-plane/live-domain-workspace.0e09cb312424.mjs'), import('./views/browser/browser-view.44c8f4a40af1.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.04d4f4f3584b.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.abef33f23bcd.mjs'), import('./views/onboarding/onboarding-view.6c149e55929c.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.e148d25ac865.mjs'), import('./views/settings/settings-view.481d5e7816d1.mjs')]);
