import { createRouter } from './core/router.69b9cb538864.mjs';
import { createUiStore } from './core/ui-store.3fe7c8a91146.mjs';
import { createUiBus } from './core/ui-bus.6f338cc2e18b.mjs';
import { createApiClient } from './core/api-client.7133fccc7393.mjs';
import { createLayoutStore } from './core/layout-store.88aeab1d3474.mjs';
import { createResizableRegionController } from './core/resizable-region.db49ac0e9999.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.388bee047241.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.0d25ac42f82d.mjs';
import { normalizeExperience } from './core/experience-policy.5cdcaa39bc94.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.be61213291fd.mjs';
import { createViewStateBridge } from './core/view-state-bridge.4719b910bae3.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.cd253edefb89.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.50c77a7763f5.mjs';
import { createUpdateStateController } from './core/update-state-controller.ab5f2fd9fa20.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.87cf784ed190.mjs';
import { settingsSectionFromRoute } from './core/settings-route.d42abc22d84b.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.b8a5b2b19de6.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.60b8bdeb3b29.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.8541132a042f.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.9dbd83f3b903.mjs';
import { projectNameFromPath } from './shell/project-picker.3ef70fcee6d8.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.900b1429319b.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.5fae0938cd02.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.b93c05d74eee.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.17de3f46dfae.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.4736deccaffc.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.2139bd5dc1e7.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.bc444a4a5bd6.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.0d21e0c04e5d.mjs'), import('./views/workroom/terminal-client.6f5923450d61.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.751629ec9c6c.mjs'), import('./control-plane/route-registry.d4e9eb54c8c6.mjs'), import('./control-plane/live-domain-workspace.5645f1484010.mjs'), import('./views/browser/browser-view.21b4b393a8f5.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.500e5e02ab67.mjs'), import('./views/settings/settings-view.b94fba4f686f.mjs')]);
