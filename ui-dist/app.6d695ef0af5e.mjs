import { createRouter } from './core/router.0fa1a121a764.mjs';
import { createUiStore } from './core/ui-store.0f4ab3621c53.mjs';
import { createUiBus } from './core/ui-bus.cdd9c2d5c0ce.mjs';
import { createApiClient } from './core/api-client.693e94e59359.mjs';
import { createLayoutStore } from './core/layout-store.d9d8117233ce.mjs';
import { createResizableRegionController } from './core/resizable-region.da05284de03c.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.bc55883e6da1.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.4cb2003879a3.mjs';
import { normalizeExperience } from './core/experience-policy.2a8e6c2a2c3a.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.a51e5c2bdbf5.mjs';
import { createViewStateBridge } from './core/view-state-bridge.42c0315975b6.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.4da114835990.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.8cf68f56ca20.mjs';
import { createUpdateStateController } from './core/update-state-controller.40940d52b348.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.7b7e28b220d1.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.cf98e852f8d6.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.728c55f30633.mjs';
import { projectNameFromPath } from './shell/project-picker.8953bfa46283.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.e41db5aa3679.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.6ec1527700d7.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.d6aeca3d81e4.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.a91d4a81e877.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.660e66249a4d.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.848a585b139b.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.158facc0cdb2.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.224343ba5dae.mjs'), import('./control-plane/route-registry.e8802f01beee.mjs'), import('./control-plane/live-domain-workspace.3d64a0ae371c.mjs'), import('./views/browser/browser-view.279eb59178da.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.b28d91a76342.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.ab20a9cb2fa8.mjs'), import('./views/onboarding/onboarding-view.ac62f38b5817.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.bab819273cf7.mjs'), import('./views/settings/settings-view.d44c21fd4629.mjs')]);
