import { createRouter } from './core/router.5b63f3b54acb.mjs';
import { createUiStore } from './core/ui-store.4d06cd6f9b5c.mjs';
import { createUiBus } from './core/ui-bus.8a3291b8d96d.mjs';
import { createApiClient } from './core/api-client.203211207aa5.mjs';
import { createLayoutStore } from './core/layout-store.4808257cd5c8.mjs';
import { createResizableRegionController } from './core/resizable-region.ddd3deefddff.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.ef499834734e.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.788a052ca677.mjs';
import { normalizeExperience } from './core/experience-policy.677fb8f3159b.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.070591ad79a4.mjs';
import { createViewStateBridge } from './core/view-state-bridge.e200f40bb4e8.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.be451a6d6f0c.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.a046f90dad1b.mjs';
import { createUpdateStateController } from './core/update-state-controller.ee5b7dc1a116.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.b9eb5348b973.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.34d840aa66d5.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.1999ad5f7aa1.mjs';
import { projectNameFromPath } from './shell/project-picker.463fbcbc696a.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.ed536c05a92f.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.c83fd7ad510d.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.91b7921b87ef.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.c4885d03e827.mjs');
...
router.register({ id: 'projects', pattern: /^\/projects(?:\?.*)?$/, title: 'Projects', load: async () => {
  const { createProjectsController, renderProjectsView } = await import('./views/projects/project-view.94ee96fa26cc.mjs');const controller=createProjectsController({api,language:cachedPreferences.language});await controller.load();let root=null;const view={render:()=>renderProjectsView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-project-search]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{if(e.target.closest('[data-project-action="add"]')){window.dispatchEvent(new CustomEvent('nolane:project-create-requested',{detail:{source:'projects-view'}}));}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view;
} });

router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.76c5444bc0a5.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.2ddf73a8c4a4.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.f2106ce01dfb.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.e926e36f13d9.mjs'), import('./control-plane/route-registry.02288def0edf.mjs'), import('./control-plane/live-domain-workspace.c5412ffc9f94.mjs'), import('./views/browser/browser-view.f9fe7431b6eb.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.946331147a4c.mjs'), import('./views/settings/settings-view.ea845f66c316.mjs')]);
