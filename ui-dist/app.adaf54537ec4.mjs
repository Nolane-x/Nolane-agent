import { createRouter } from './core/router.d555de131be2.mjs';
import { createUiStore } from './core/ui-store.0500b68cf9e9.mjs';
import { createUiBus } from './core/ui-bus.12b29a03b6bf.mjs';
import { createApiClient } from './core/api-client.abac7ab5b203.mjs';
import { createLayoutStore } from './core/layout-store.471fdd534d86.mjs';
import { createResizableRegionController } from './core/resizable-region.758edc679c87.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.bd1b26215030.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.cd4aff459d64.mjs';
import { normalizeExperience } from './core/experience-policy.97d047c99bd0.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.01853181dd6f.mjs';
import { createViewStateBridge } from './core/view-state-bridge.713da272939f.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.90b91ffffc6d.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.cf415760ce6e.mjs';
import { createUpdateStateController } from './core/update-state-controller.be58aed6afb0.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.cf12d0d5911a.mjs';
import { settingsSectionFromRoute } from './core/settings-route.da40ea3811a7.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.7f0765f42772.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.f5cdd4b1d36f.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.4eb9dc65a8df.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.5c056ca07329.mjs';
import { projectNameFromPath } from './shell/project-picker.a0a63a151bf6.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.ab95ec0976d5.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.60e7100fb5e4.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.ad47c812af9a.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.46dffde2af99.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.74125367709a.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.3111d2d3f854.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.6cabfdad3751.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.cd7563fd2555.mjs'), import('./views/workroom/terminal-client.94cebea68102.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.66f7750f1a44.mjs'), import('./control-plane/route-registry.c39b913c4b30.mjs'), import('./control-plane/live-domain-workspace.cfb3b29cbab4.mjs'), import('./views/browser/browser-view.ac43c61ccf60.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.7b0e9ef21751.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.9339054508d3.mjs'), import('./views/onboarding/onboarding-view.04a35cb41c2a.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.fe1778d9472c.mjs'), import('./views/settings/settings-view.2ca8a311dea0.mjs')]);
