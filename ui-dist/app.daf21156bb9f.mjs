import { createRouter } from './core/router.611fcbc04aca.mjs';
import { createUiStore } from './core/ui-store.f44478f225ce.mjs';
import { createUiBus } from './core/ui-bus.ba942755cf16.mjs';
import { createApiClient } from './core/api-client.01b2cdbca87e.mjs';
import { createLayoutStore } from './core/layout-store.c734c6d4c09a.mjs';
import { createResizableRegionController } from './core/resizable-region.26c23c91dd91.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.72c6b995220d.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.45b66c66f09f.mjs';
import { normalizeExperience } from './core/experience-policy.935b7dc22c7c.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.5d821b67a71b.mjs';
import { createViewStateBridge } from './core/view-state-bridge.f6a7fc800ce6.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.a02fb5b996a6.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.b3a1f78cadf2.mjs';
import { createUpdateStateController } from './core/update-state-controller.d7ca71f2ce17.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.d97d01316428.mjs';
import { settingsSectionFromRoute } from './core/settings-route.2a66f1cf7103.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.19af07cd2506.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.9017de051e1f.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.31e84e58cf8a.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.3da36cb37719.mjs';
import { projectNameFromPath } from './shell/project-picker.fe376d1ef72d.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.92959db7aea3.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.d6e398e9b75c.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.87bbe28d5b08.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.94114d19eefb.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.14d43aa42308.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.41e5e5343df2.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.d333be4560ac.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.73b43920dbbe.mjs'), import('./views/workroom/terminal-client.2a9acc4c68b1.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.f02340452ba2.mjs'), import('./control-plane/route-registry.c5d26a1662b8.mjs'), import('./control-plane/live-domain-workspace.ece35557f1f8.mjs'), import('./views/browser/browser-view.97c811a2b26f.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.d0f565492f11.mjs'), import('./views/settings/settings-view.6e84f4b6cc7b.mjs')]);
