import { createRouter } from './core/router.7be56f579b40.mjs';
import { createUiStore } from './core/ui-store.b5ff287c7ee2.mjs';
import { createUiBus } from './core/ui-bus.62b848a3d392.mjs';
import { createApiClient } from './core/api-client.ac9650a047ca.mjs';
import { createLayoutStore } from './core/layout-store.d85c8a1df51d.mjs';
import { createResizableRegionController } from './core/resizable-region.d71d4ead53bf.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.2f66a09bdeea.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.436d9809157c.mjs';
import { normalizeExperience } from './core/experience-policy.6562de87fcbb.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.e2cc27ac5639.mjs';
import { createViewStateBridge } from './core/view-state-bridge.adaa076efd7c.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.11c68e7441d4.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.ac548ad3cba8.mjs';
import { createUpdateStateController } from './core/update-state-controller.289cf3edff36.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.da783b849782.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.713f599af0ee.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.024d6948b4e7.mjs';
import { projectNameFromPath } from './shell/project-picker.d0b2764729c3.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.50921f855c5c.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.6a93e7ab26f9.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.45e00ce60bda.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.51538909f381.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.c6f6a8f7dc99.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.43f770bb4688.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.0bb3dcfb5781.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.29c04156a192.mjs'), import('./control-plane/route-registry.63244e22a7f1.mjs'), import('./control-plane/live-domain-workspace.2ba20ef54c74.mjs'), import('./views/browser/browser-view.043cc10d6cd2.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.a212e38fd7df.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.86167c43d183.mjs'), import('./views/onboarding/onboarding-view.d09b604be59d.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.3ae75cdaf591.mjs'), import('./views/settings/settings-view.e60893132cb8.mjs')]);
