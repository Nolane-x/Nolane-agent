import { createRouter } from './core/router.e0ed750a5d53.mjs';
import { createUiStore } from './core/ui-store.69044be0c53e.mjs';
import { createUiBus } from './core/ui-bus.d31e30d5bd73.mjs';
import { createApiClient } from './core/api-client.75da8817b958.mjs';
import { createLayoutStore } from './core/layout-store.7907305daeff.mjs';
import { createResizableRegionController } from './core/resizable-region.38b484ba64d6.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.16fff9a20d89.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.fc2655c08d7f.mjs';
import { normalizeExperience } from './core/experience-policy.51a1dc109c37.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.1023f68d97e8.mjs';
import { createViewStateBridge } from './core/view-state-bridge.f3e3c08a311c.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.1190bbbbeff8.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.9f01977041be.mjs';
import { createUpdateStateController } from './core/update-state-controller.151720816f37.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.74977e920a03.mjs';
import { settingsSectionFromRoute } from './core/settings-route.0750cca170bc.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.1fb518856b25.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.d49a5d33820b.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.ee1dccc9e41f.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.bbd8f974a044.mjs';
import { projectNameFromPath } from './shell/project-picker.c2848bc18cea.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.071b19628217.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.c01a3c9b8129.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.0817d0cf23ad.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.75170a9a53a2.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.f87a897d19cf.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.fcbee2cd4db1.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.2f088899281a.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.fa6f23b98b2b.mjs'), import('./views/workroom/terminal-client.3958eacd5e50.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.d0eaeb4e3f86.mjs'), import('./control-plane/route-registry.4827dfa7d09a.mjs'), import('./control-plane/live-domain-workspace.f41b9633781b.mjs'), import('./views/browser/browser-view.453d337cc1fd.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.5941ef78b97b.mjs'), import('./views/settings/settings-view.c932569bb44b.mjs')]);
