import { createRouter } from './core/router.07e1079e4941.mjs';
import { createUiStore } from './core/ui-store.253987211a10.mjs';
import { createUiBus } from './core/ui-bus.874e69268ce4.mjs';
import { createApiClient } from './core/api-client.8f24a9095983.mjs';
import { createLayoutStore } from './core/layout-store.2ae38b4b0d47.mjs';
import { createResizableRegionController } from './core/resizable-region.6324914ce89f.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.257974f5c811.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.039b97385122.mjs';
import { normalizeExperience } from './core/experience-policy.e91d2c689b7a.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.2a423e7c409d.mjs';
import { createViewStateBridge } from './core/view-state-bridge.b50f66e32a81.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.f5e1dff760ed.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.c3b3c4c69017.mjs';
import { createUpdateStateController } from './core/update-state-controller.727ace879ab1.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.3fdfb17fe02b.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.cb406c0f94e9.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.52027d4d40ba.mjs';
import { projectNameFromPath } from './shell/project-picker.3acb0c1666b1.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.22795fcdcabe.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.41fc67482d50.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.276f50fcca68.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.867e53052e83.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.92249fdf97a9.mjs'), import('./control-plane/route-registry.fe4f740a86cd.mjs'), import('./control-plane/live-domain-workspace.70555c3051ec.mjs'), import('./views/browser/browser-view.d8ff3b8a7b21.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.f3dd4441842b.mjs'), import('./views/settings/settings-view.8d27258769fa.mjs')]);
