import { createRouter } from './core/router.f80039c56862.mjs';
import { createUiStore } from './core/ui-store.90aead5d6892.mjs';
import { createUiBus } from './core/ui-bus.7ee03f8cc9ce.mjs';
import { createApiClient } from './core/api-client.b6093480b3c4.mjs';
import { createLayoutStore } from './core/layout-store.cdaccb218dae.mjs';
import { createResizableRegionController } from './core/resizable-region.08862c621694.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.c7485b623f3a.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.d7079881da53.mjs';
import { normalizeExperience } from './core/experience-policy.0907903f3420.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.c79f4d5b1606.mjs';
import { createViewStateBridge } from './core/view-state-bridge.759315d8c9b9.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.7bd7358fed8b.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.907a781dde66.mjs';
import { createUpdateStateController } from './core/update-state-controller.d0fe9aff7781.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.576a586b35e1.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.de3b9ae0e9ce.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.9600abba0e80.mjs';
import { projectNameFromPath } from './shell/project-picker.4243ac46f8cc.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.7218959c82b0.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.14578559b0d3.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.125fd391354f.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.0d3adf1940e3.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.3caa966d0102.mjs'), import('./control-plane/route-registry.843d5fad4055.mjs'), import('./control-plane/live-domain-workspace.6fd1ae7377d6.mjs'), import('./views/browser/browser-view.d13994ae4157.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.098b3e193698.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.65333365e543.mjs'), import('./views/onboarding/onboarding-view.4970821ffb70.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.06ea04298b42.mjs'), import('./views/settings/settings-view.909b50b3f0b6.mjs')]);
