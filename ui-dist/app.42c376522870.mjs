import { createRouter } from './core/router.1fa56b00115c.mjs';
import { createUiStore } from './core/ui-store.ad1dd5bdca89.mjs';
import { createUiBus } from './core/ui-bus.74077ea22782.mjs';
import { createApiClient } from './core/api-client.73b6802a46b8.mjs';
import { createLayoutStore } from './core/layout-store.223f9b97db16.mjs';
import { createResizableRegionController } from './core/resizable-region.dc0a668abbd8.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.7d78a19eaee5.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.9055d738b874.mjs';
import { normalizeExperience } from './core/experience-policy.edffc1797d1d.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.8cb60a73dba1.mjs';
import { createViewStateBridge } from './core/view-state-bridge.1d25b29cdbf5.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.eff3446b485e.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.cf1954c815d0.mjs';
import { createUpdateStateController } from './core/update-state-controller.d89e0a5d4a68.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.9923b6a9a9ba.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.94f9ab4cfbd3.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.5e0c9b3aa770.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.fd541170cebe.mjs';
import { projectNameFromPath } from './shell/project-picker.8a0ed1a6cc5d.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.d50af2129da1.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.17376921a4b1.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.a28cc5193bf2.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.eebcac413f33.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.0001a35a9c86.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.91782c145201.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.e03bf69fed1e.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.2e1ac3132c0a.mjs'), import('./views/workroom/terminal-client.3c844a44f5f6.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.d93b6def41d0.mjs'), import('./control-plane/route-registry.b9b2bd2eb0bc.mjs'), import('./control-plane/live-domain-workspace.bc59fed9ed38.mjs'), import('./views/browser/browser-view.8be182835ba1.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.1067bc275ed0.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.22667592c06b.mjs'), import('./views/onboarding/onboarding-view.0560b3118a6f.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.ae2ff47a3473.mjs'), import('./views/settings/settings-view.5f1c04dce2b2.mjs')]);
