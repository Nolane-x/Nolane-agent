import { createRouter } from './core/router.28f50b4712a7.mjs';
import { createUiStore } from './core/ui-store.9721a873f0db.mjs';
import { createUiBus } from './core/ui-bus.ad62c1883196.mjs';
import { createApiClient } from './core/api-client.e240a4fad167.mjs';
import { createLayoutStore } from './core/layout-store.e2d5f9e68c2f.mjs';
import { createResizableRegionController } from './core/resizable-region.35fee85d419f.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.663f4a617d73.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.abd2a6d6f6c6.mjs';
import { normalizeExperience } from './core/experience-policy.e27b5cf7a2e1.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.d60c29103137.mjs';
import { createViewStateBridge } from './core/view-state-bridge.fe357eac1fff.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.1dfa2a8b2c39.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.cd11984cc6ea.mjs';
import { createUpdateStateController } from './core/update-state-controller.b2be55fdf905.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.b5afc5e317b7.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.0ebda9818175.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.8c6502e92c79.mjs';
import { projectNameFromPath } from './shell/project-picker.54e03f8a7c2e.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.0131853e6684.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.96a3f9d4aa82.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.bc04c904655f.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.5b04880f640b.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.a72a7b490063.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.1fd4563bb0e8.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.3cc4522dd4c5.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.14b529ea608c.mjs'), import('./control-plane/route-registry.621bf94f42d9.mjs'), import('./control-plane/live-domain-workspace.97325488a363.mjs'), import('./views/browser/browser-view.3748a9e7a61d.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.f55a08d772d1.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.7a6fc684bd91.mjs'), import('./views/onboarding/onboarding-view.4d3eb15e1f8c.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.518d27c9e2ce.mjs'), import('./views/settings/settings-view.85ea2116119c.mjs')]);
