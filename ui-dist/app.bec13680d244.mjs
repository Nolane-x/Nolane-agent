import { createRouter } from './core/router.d0a5e3f07aa6.mjs';
import { createUiStore } from './core/ui-store.b87cdd0d63f4.mjs';
import { createUiBus } from './core/ui-bus.dc1314ca0d62.mjs';
import { createApiClient } from './core/api-client.3f88be8088ac.mjs';
import { createLayoutStore } from './core/layout-store.5a9b22aac154.mjs';
import { createResizableRegionController } from './core/resizable-region.3f4af2fa73cd.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.096ba23d73fa.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.a4df09836a53.mjs';
import { normalizeExperience } from './core/experience-policy.d2e6e57b3456.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.124f16a49d9e.mjs';
import { createViewStateBridge } from './core/view-state-bridge.d62be11e884d.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.037649050182.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.95f9a048a2b2.mjs';
import { createUpdateStateController } from './core/update-state-controller.02206bbc14d1.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.ec9bd7fc36cc.mjs';
import { settingsSectionFromRoute } from './core/settings-route.81d2fffe9ab2.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.b53725ec161e.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.a02ef5ae09e7.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.aedce74e78da.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.caa1db4ba39b.mjs';
import { projectNameFromPath } from './shell/project-picker.87866937d184.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.30d913abc822.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.c151c0dc7de1.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.768362fb903a.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.d3ba0b5f451c.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.e700074ef01c.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.79b86e18e4d0.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.9583b3d53107.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.37cc2b6032a4.mjs'), import('./views/workroom/terminal-client.a3977aaa2b1d.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.76db20bfeddc.mjs'), import('./control-plane/route-registry.f7edf6824bc9.mjs'), import('./control-plane/live-domain-workspace.e33324bd9484.mjs'), import('./views/browser/browser-view.a1757942f75c.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.5476cbc255ef.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.7b8deca69bc8.mjs'), import('./views/onboarding/onboarding-view.42a5ba2fe0c1.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.dac8b494f180.mjs'), import('./views/settings/settings-view.cd7a555b1e91.mjs')]);
