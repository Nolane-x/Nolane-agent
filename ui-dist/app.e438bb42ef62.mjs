import { createRouter } from './core/router.0cbebe0abc12.mjs';
import { createUiStore } from './core/ui-store.65b208c60c05.mjs';
import { createUiBus } from './core/ui-bus.83f1bd481d84.mjs';
import { createApiClient } from './core/api-client.94332ca38ddb.mjs';
import { createLayoutStore } from './core/layout-store.05b809af59ee.mjs';
import { createResizableRegionController } from './core/resizable-region.792b98186ae1.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.79cd84eb1774.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.31b9ddaf3b1c.mjs';
import { normalizeExperience } from './core/experience-policy.e5a06b0c410c.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.9c7c4fb340e0.mjs';
import { createViewStateBridge } from './core/view-state-bridge.279bda1d42dd.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.ab192db4f227.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.2e8f7bbd1d12.mjs';
import { createUpdateStateController } from './core/update-state-controller.6418fa02c85d.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.ec4f371e5a78.mjs';
import { settingsSectionFromRoute } from './core/settings-route.1019ebcbad32.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.222daddce14b.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.06c614b8a47e.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.9bad9a539ddb.mjs';
import { closeComposerPickers, isComposerPickerInteraction } from './components/composer-picker.f674a8f3bbc4.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.51e541696a09.mjs';
import { projectNameFromPath } from './shell/project-picker.8a545b807f66.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.37fafe0ff4b8.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.7d5dea25588d.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.1af4c19e721c.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.13f95619d561.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.46ad3dba6fde.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.a3c700d0a9a6.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.b508455cab75.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.e50dda1f13f4.mjs'), import('./views/workroom/terminal-client.0bf3dd04007a.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.6a356a178f63.mjs'), import('./control-plane/route-registry.8542bf02dfc3.mjs'), import('./control-plane/live-domain-workspace.71b19e5122f1.mjs'), import('./views/browser/browser-view.48d83515c98f.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.e42afbeb92ff.mjs'), import('./views/settings/settings-view.6ac2820c67fa.mjs')]);
