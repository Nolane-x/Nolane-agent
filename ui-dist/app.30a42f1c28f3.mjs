import { createRouter } from './core/router.1a7a55a4db0d.mjs';
import { createUiStore } from './core/ui-store.a661966eb7b9.mjs';
import { createUiBus } from './core/ui-bus.2b66165f935d.mjs';
import { createApiClient } from './core/api-client.46b9a00f6e8f.mjs';
import { createLayoutStore } from './core/layout-store.4ad3ebe38c1d.mjs';
import { createResizableRegionController } from './core/resizable-region.ba62e3c6797e.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.8e9744daad17.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.60008b47b2a8.mjs';
import { normalizeExperience } from './core/experience-policy.b49c2eacf1be.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.1a2c9c16a380.mjs';
import { createViewStateBridge } from './core/view-state-bridge.c6200cb8c399.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.db4055a343fe.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.17056a9e2793.mjs';
import { createUpdateStateController } from './core/update-state-controller.969a93448720.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.7e77ccd1540e.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.0a4947ce716a.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.5e434b1424f5.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.5524b62f42ae.mjs';
import { projectNameFromPath } from './shell/project-picker.ae93c3e9a7ec.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.b85109b4e736.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.5312adeb346a.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.ad770db54729.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.d0336e9dcb4e.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.f2d2a5fe4919.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.5675912ac5e8.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.5b8fca32e0c9.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.4ee59c87313e.mjs'), import('./views/workroom/terminal-client.74e6f3a922ff.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.6871dfdd2200.mjs'), import('./control-plane/route-registry.975abfeaee8f.mjs'), import('./control-plane/live-domain-workspace.7dc447d75be6.mjs'), import('./views/browser/browser-view.062cae0d5448.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.508d6d6947c3.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.7af7d86e009b.mjs'), import('./views/onboarding/onboarding-view.fedfbffc6d4d.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.01ede0a6c339.mjs'), import('./views/settings/settings-view.3e1c07298fdc.mjs')]);
