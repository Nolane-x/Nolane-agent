import { createRouter } from './core/router.b7d137457c3c.mjs';
import { createUiStore } from './core/ui-store.0f3d4a99aafc.mjs';
import { createUiBus } from './core/ui-bus.a6a3db9a4cd3.mjs';
import { createApiClient } from './core/api-client.d3120a4b0879.mjs';
import { createLayoutStore } from './core/layout-store.d000dbbddf37.mjs';
import { createResizableRegionController } from './core/resizable-region.a11ad92a7d89.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.d03e3bb037ec.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.a9c0d0947eb9.mjs';
import { normalizeExperience } from './core/experience-policy.f3cc237ca2a6.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.a26bc3cf1743.mjs';
import { createViewStateBridge } from './core/view-state-bridge.125e68d408d3.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.d12cfdc86e4d.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.043629b0766c.mjs';
import { createUpdateStateController } from './core/update-state-controller.6257eb2d0304.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.c19ca72c8332.mjs';
import { settingsSectionFromRoute } from './core/settings-route.0e8e74b597f6.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.f0df7e6b1eae.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.ab6cde483b92.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.8a0813d917f5.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.0b9eb00f1f2c.mjs';
import { projectNameFromPath } from './shell/project-picker.76d06bde4254.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.fbae896dfd06.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.d262d61f337e.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.1a7f5b5c9441.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.8a02d3f91a31.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.4610a9d80566.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.e4a284f452eb.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.552a8ad3292d.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.d6c830ddaf78.mjs'), import('./views/workroom/terminal-client.7c406d29b550.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.e180e491d82c.mjs'), import('./control-plane/route-registry.dd9285138a64.mjs'), import('./control-plane/live-domain-workspace.b491168371df.mjs'), import('./views/browser/browser-view.ca08f51819b7.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.b3fbe700fa7f.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.02d816db4a63.mjs'), import('./views/onboarding/onboarding-view.23e4becb9e3f.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.933d9a0edb6d.mjs'), import('./views/settings/settings-view.ffe50d175b90.mjs')]);
