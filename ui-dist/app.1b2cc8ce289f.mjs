import { createRouter } from './core/router.602301f96f34.mjs';
import { createUiStore } from './core/ui-store.c0559c89ecd1.mjs';
import { createUiBus } from './core/ui-bus.f3ca0b233b42.mjs';
import { createApiClient } from './core/api-client.fe4c6e2c8536.mjs';
import { createLayoutStore } from './core/layout-store.7ca54e935e67.mjs';
import { createResizableRegionController } from './core/resizable-region.871be221a129.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.069461fe4d67.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.e8587a2b1133.mjs';
import { normalizeExperience } from './core/experience-policy.5834cff3b888.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.5c7d7b059f7e.mjs';
import { createViewStateBridge } from './core/view-state-bridge.26e23c8f6d5c.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.69c4416688a0.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.e4cd83ae8c6a.mjs';
import { createUpdateStateController } from './core/update-state-controller.f2cbd6198507.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.1daa4f9b6cef.mjs';
import { settingsSectionFromRoute } from './core/settings-route.aba810458569.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.683dbc8bb66c.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.58d88b6e15b2.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.58c9c3485358.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.837d8dd875c2.mjs';
import { projectNameFromPath } from './shell/project-picker.73d84c742130.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.a64805b6f919.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.cbbaa331f700.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.5c142ab9be05.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.b911585c6cd8.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.2651322b743d.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.1a0794e46b23.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.04735d0cfc3a.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.0b41b1b245f4.mjs'), import('./views/workroom/terminal-client.33808f313970.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.0716eca97af6.mjs'), import('./control-plane/route-registry.b256d582358a.mjs'), import('./control-plane/live-domain-workspace.2b1ac6702a7d.mjs'), import('./views/browser/browser-view.3a578a56cf07.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.d24539158047.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.f4cd8b61df1c.mjs'), import('./views/onboarding/onboarding-view.5e1e8aad3aa4.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.16248d82480c.mjs'), import('./views/settings/settings-view.a0973d392589.mjs')]);
