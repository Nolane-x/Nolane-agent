import { createRouter } from './core/router.0639ddcff331.mjs';
import { createUiStore } from './core/ui-store.f5eb08ad2e46.mjs';
import { createUiBus } from './core/ui-bus.ff0545bd5dcf.mjs';
import { createApiClient } from './core/api-client.8ba699e5919c.mjs';
import { createLayoutStore } from './core/layout-store.409a314c5496.mjs';
import { createResizableRegionController } from './core/resizable-region.fd5c4db9b089.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.6068f56a1cd8.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.02fd9fe766a4.mjs';
import { normalizeExperience } from './core/experience-policy.e1b11c48336c.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.503bce8a8081.mjs';
import { createViewStateBridge } from './core/view-state-bridge.b455dec7643b.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.c0a3d1f7cb4b.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.fec0a9b048b7.mjs';
import { createUpdateStateController } from './core/update-state-controller.ea4f6996fc66.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.48f4649b3928.mjs';
import { settingsSectionFromRoute } from './core/settings-route.c83cb670f473.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.e7cf55d0a77b.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.3a58ca04e2d3.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.9f5deef03ac9.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.f3f19ec1b516.mjs';
import { projectNameFromPath } from './shell/project-picker.eea473bfcc9e.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.3461f1d8cad4.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.72585608fc46.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.1e60b4f2dddf.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.0a76b2a06498.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.c02c073eb04c.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.87e79d672216.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.d53eb54983da.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.c79e2adbebf5.mjs'), import('./views/workroom/terminal-client.1ada2b669a32.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.bbdd3157e66f.mjs'), import('./control-plane/route-registry.f58640c6e2e0.mjs'), import('./control-plane/live-domain-workspace.b36a24bf8442.mjs'), import('./views/browser/browser-view.92cdd07bd14a.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.330eb54d8866.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.75b09d1d70e3.mjs'), import('./views/onboarding/onboarding-view.8af7205ce053.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.252153aed1f7.mjs'), import('./views/settings/settings-view.2e9edaaf45d6.mjs')]);
