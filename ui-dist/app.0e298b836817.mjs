import { createRouter } from './core/router.8edcfebe142b.mjs';
import { createUiStore } from './core/ui-store.886996721bdc.mjs';
import { createUiBus } from './core/ui-bus.7d570b50d8be.mjs';
import { createApiClient } from './core/api-client.52070eb324e8.mjs';
import { createLayoutStore } from './core/layout-store.0bb1f94b1686.mjs';
import { createResizableRegionController } from './core/resizable-region.8c7fe63dd944.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.645e6146ba33.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.c9031105f76e.mjs';
import { normalizeExperience } from './core/experience-policy.e988087d0e3b.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.dbecb75fdfae.mjs';
import { createViewStateBridge } from './core/view-state-bridge.8aa7df5560ed.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.29fb035bd3df.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.f83f6015b6e8.mjs';
import { createUpdateStateController } from './core/update-state-controller.f0626d5f14eb.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.6e58fc02b393.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.2dd8984191b1.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.1e52473bd570.mjs';
import { projectNameFromPath } from './shell/project-picker.b57bfd3c64f5.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.f8dc8201483b.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.ac9aee789d96.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.5a023506b763.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.53d4d92cbf08.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.93c614ace9b5.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.a18a652ba45d.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.25a16922c31d.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.5af2b356b3bb.mjs'), import('./control-plane/route-registry.86d4c045f994.mjs'), import('./control-plane/live-domain-workspace.7a11bacbf243.mjs'), import('./views/browser/browser-view.c5396e4c4dd3.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.008d2ecf259b.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.997f7c96a1d0.mjs'), import('./views/onboarding/onboarding-view.68e0c015f8d0.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.9a21b445d7f5.mjs'), import('./views/settings/settings-view.8acf50693a7c.mjs')]);
