import { createRouter } from './core/router.d4906a5987a4.mjs';
import { createUiStore } from './core/ui-store.763c2fddcf8e.mjs';
import { createUiBus } from './core/ui-bus.4f96648dcbfd.mjs';
import { createApiClient } from './core/api-client.766979495aaf.mjs';
import { createLayoutStore } from './core/layout-store.2048ff70fa8b.mjs';
import { createResizableRegionController } from './core/resizable-region.c2e03fba1359.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.18433d500467.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.d2d5b407a132.mjs';
import { normalizeExperience } from './core/experience-policy.2a36c61069b7.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.1fe84b8bc5bf.mjs';
import { createViewStateBridge } from './core/view-state-bridge.c2c3e8caa109.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.e423de18ad7d.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.150d41d8599b.mjs';
import { createUpdateStateController } from './core/update-state-controller.6a915335325b.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.2c0dd4fbaa20.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.6925fc5743e1.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.c3a958899acc.mjs';
import { projectNameFromPath } from './shell/project-picker.8637ef041a65.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.b2bc7189ef4d.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.acec23e2906b.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.c323de3715e4.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.bd84e5daa1e8.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.1d23c21ce255.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.e00a18144284.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.1dbfb60fddf6.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.c4b8ca12134e.mjs'), import('./views/workroom/terminal-client.97142b281d8c.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.b61cf5b3c48f.mjs'), import('./control-plane/route-registry.9c5a794caabb.mjs'), import('./control-plane/live-domain-workspace.9ff6e38d31d1.mjs'), import('./views/browser/browser-view.ae4c2a636dc8.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.a882c42d6e25.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.eb027523a487.mjs'), import('./views/onboarding/onboarding-view.16b3e86fcc3c.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.88bbd2af5f31.mjs'), import('./views/settings/settings-view.fbc24d93562b.mjs')]);
