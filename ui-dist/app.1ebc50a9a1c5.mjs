import { createRouter } from './core/router.f79d7ae12192.mjs';
import { createUiStore } from './core/ui-store.95617e60519f.mjs';
import { createUiBus } from './core/ui-bus.0a60894c34c5.mjs';
import { createApiClient } from './core/api-client.742287f44be6.mjs';
import { createLayoutStore } from './core/layout-store.1620afb0fb54.mjs';
import { createResizableRegionController } from './core/resizable-region.21bd0b429958.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.5baf352a795d.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.abedcf265fbc.mjs';
import { normalizeExperience } from './core/experience-policy.484817139b94.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.6d93808c6442.mjs';
import { createViewStateBridge } from './core/view-state-bridge.cd211b0a6ebd.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.8cc6b1c19740.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.3cd47ee4cb94.mjs';
import { createUpdateStateController } from './core/update-state-controller.390b7c33584b.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.2e95fd4ed9db.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.9dffdc478f9c.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.691680fa6d79.mjs';
import { projectNameFromPath } from './shell/project-picker.9097f530b371.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.b939e99c896e.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.bc29c806f7a0.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.e9065927c514.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.1843d6a05559.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.e536fcd5fb56.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.a199089b06f8.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.f79621705884.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.6699a36136b6.mjs'), import('./control-plane/route-registry.adb246b68072.mjs'), import('./control-plane/live-domain-workspace.9b2d77e08653.mjs'), import('./views/browser/browser-view.23b6308f2fc7.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.3fa87ed3cea9.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.03d73db835c8.mjs'), import('./views/onboarding/onboarding-view.c7291ebad8f6.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.0b4f91b2d4fc.mjs'), import('./views/settings/settings-view.812a102ef7e3.mjs')]);
