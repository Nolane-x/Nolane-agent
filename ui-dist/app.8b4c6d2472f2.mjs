import { createRouter } from './core/router.a24ce0c1fd48.mjs';
import { createUiStore } from './core/ui-store.d67f836a1100.mjs';
import { createUiBus } from './core/ui-bus.323e2dce8c80.mjs';
import { createApiClient } from './core/api-client.04611325d96e.mjs';
import { createLayoutStore } from './core/layout-store.c3099ff6259f.mjs';
import { createResizableRegionController } from './core/resizable-region.2f8d208495d7.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.14df3259f9ee.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.a6342f9ebf1c.mjs';
import { normalizeExperience } from './core/experience-policy.0179f9e97df8.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.33d60fb278fc.mjs';
import { createViewStateBridge } from './core/view-state-bridge.16eb2e0637af.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.cd00d54840ea.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.1f6e0f4cc303.mjs';
import { createUpdateStateController } from './core/update-state-controller.046847fd5404.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.7bb5c6c33586.mjs';
import { settingsSectionFromRoute } from './core/settings-route.c8cdd340c156.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.1d096a981df6.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.4a4e347a595f.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.d6a0a95b5244.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.bccb4f72269f.mjs';
import { projectNameFromPath } from './shell/project-picker.f6071776c6bf.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.3faf1d0be1ed.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.d73cd04cad1c.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.c0f830e06652.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.127b6f481a38.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.dd380390fe79.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.1eea8f4006c7.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.ded3e37b4c64.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.df86a086e3e0.mjs'), import('./views/workroom/terminal-client.4b363fbff400.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.47b4bd10192d.mjs'), import('./control-plane/route-registry.bf692a915bf1.mjs'), import('./control-plane/live-domain-workspace.66e4d7b41deb.mjs'), import('./views/browser/browser-view.8d71eb3c0ffb.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.671314ae02f3.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.05ae93f8f43d.mjs'), import('./views/onboarding/onboarding-view.1b6bc0a505bc.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.74b029cb074f.mjs'), import('./views/settings/settings-view.ba4ce73119c2.mjs')]);
