import { createRouter } from './core/router.50206a47d58f.mjs';
import { createUiStore } from './core/ui-store.bc6408fcb15a.mjs';
import { createUiBus } from './core/ui-bus.08052f5d2336.mjs';
import { createApiClient } from './core/api-client.819831de2b86.mjs';
import { createLayoutStore } from './core/layout-store.93c462c0e869.mjs';
import { createResizableRegionController } from './core/resizable-region.230c07151543.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.9cc766410838.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.26098a591bb4.mjs';
import { normalizeExperience } from './core/experience-policy.ffa17be7f92b.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.050936d53bbc.mjs';
import { createViewStateBridge } from './core/view-state-bridge.200a52888a76.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.e731c0c2a811.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.940533758cd7.mjs';
import { createUpdateStateController } from './core/update-state-controller.e32c5feababc.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.0f172d1e072e.mjs';
import { settingsSectionFromRoute } from './core/settings-route.7d74de5b558e.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.fa1b40f5c38e.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.2eeed86a7c7d.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.4c04818fd304.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.140400387d79.mjs';
import { projectNameFromPath } from './shell/project-picker.a767f2bf354e.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.0c77e5873eb2.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.ad6c190c4ea9.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.e00c98be5bf4.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.9fad9908943c.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.a936b8d82603.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.7a14604e72e5.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.52ffd1e9df19.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.8c49f5739ccb.mjs'), import('./views/workroom/terminal-client.560a24dffd0a.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.3aed7817a300.mjs'), import('./control-plane/route-registry.c24f04cd80e4.mjs'), import('./control-plane/live-domain-workspace.ed5b46597f61.mjs'), import('./views/browser/browser-view.4bb7ca3310b4.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.2a8ff21de78e.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.efb22a9c0ad5.mjs'), import('./views/onboarding/onboarding-view.609d69fe95f9.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.016aae8f685a.mjs'), import('./views/settings/settings-view.8214dd97d890.mjs')]);
