import { createRouter } from './core/router.626d0a381a38.mjs';
import { createUiStore } from './core/ui-store.9abd8c5ceb70.mjs';
import { createUiBus } from './core/ui-bus.e318fa3b272c.mjs';
import { createApiClient } from './core/api-client.5f9c57d3a7a9.mjs';
import { createLayoutStore } from './core/layout-store.9f4486f17725.mjs';
import { createResizableRegionController } from './core/resizable-region.8651ecd6612e.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.3993ed02ebde.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.e6dc99e8c522.mjs';
import { normalizeExperience } from './core/experience-policy.28d30388e1cc.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.c6d8a0c8df25.mjs';
import { createViewStateBridge } from './core/view-state-bridge.b1eeb99ca68e.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.165447b14ac9.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.354494df9d1e.mjs';
import { createUpdateStateController } from './core/update-state-controller.542fedb11b6d.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.2503621b7d13.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.ddedcceff4ec.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.b0c545a81a3d.mjs';
import { projectNameFromPath } from './shell/project-picker.44017549e4de.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.2fc13fa9962e.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.4595f7b81da9.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.b1defe1693c0.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.97e4ddeaf3cc.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.fd8c7fa231ae.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.c3898279755a.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.308349338baa.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.7a50e45f14be.mjs'), import('./control-plane/route-registry.f00466eea48f.mjs'), import('./control-plane/live-domain-workspace.5852c5b2f4ad.mjs'), import('./views/browser/browser-view.17fcdbd76bd0.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.ffdb5ebf2656.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.188393a3eee3.mjs'), import('./views/onboarding/onboarding-view.f41143e97b9e.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.3a8c85d35ba3.mjs'), import('./views/settings/settings-view.f6bfbd3b38d7.mjs')]);
