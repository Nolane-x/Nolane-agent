import { createRouter } from './core/router.f72033e70e7a.mjs';
import { createUiStore } from './core/ui-store.a04ea0049d44.mjs';
import { createUiBus } from './core/ui-bus.30f01f70cd54.mjs';
import { createApiClient } from './core/api-client.866624573dac.mjs';
import { createLayoutStore } from './core/layout-store.50928931d041.mjs';
import { createResizableRegionController } from './core/resizable-region.be2624de0cc0.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.97eb8793d513.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.a1fc9d215436.mjs';
import { normalizeExperience } from './core/experience-policy.13ef7d99fa06.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.f773a798b13d.mjs';
import { createViewStateBridge } from './core/view-state-bridge.627711b77abe.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.97a708370e97.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.5701d14ec212.mjs';
import { createUpdateStateController } from './core/update-state-controller.713644eb7a2d.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.debf21bc5d47.mjs';
import { settingsSectionFromRoute } from './core/settings-route.f18d2739dca7.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.b3d6f17da53b.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.ecbd425c2928.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.0c4e356b8bd0.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.b264b1e81bf4.mjs';
import { projectNameFromPath } from './shell/project-picker.ea940adefaac.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.2c04149ede32.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.8791f9ab318c.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.f9bee95f6d5d.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.0ee1f711d69a.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.b4ed830649e5.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.bef48ebb69b5.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.35d8a6c95d51.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.2fafa916a28d.mjs'), import('./views/workroom/terminal-client.5a3ffb21af66.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.0d5dea17d584.mjs'), import('./control-plane/route-registry.c83cf8f25e2e.mjs'), import('./control-plane/live-domain-workspace.6dfe5eb31ed8.mjs'), import('./views/browser/browser-view.8666e211b99d.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.840927cea199.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.63784591f357.mjs'), import('./views/onboarding/onboarding-view.5b009199fd0c.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.44e75e125da4.mjs'), import('./views/settings/settings-view.ef5833ce2703.mjs')]);
