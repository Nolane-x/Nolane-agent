import { createRouter } from './core/router.b2eaa199e5e2.mjs';
import { createUiStore } from './core/ui-store.3e91ac69a307.mjs';
import { createUiBus } from './core/ui-bus.6b65e1703069.mjs';
import { createApiClient } from './core/api-client.cc3d7f30d64e.mjs';
import { createLayoutStore } from './core/layout-store.d79ca117015b.mjs';
import { createResizableRegionController } from './core/resizable-region.010f5b0145f4.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.81f16a272339.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.5c4bcbb9dd65.mjs';
import { normalizeExperience } from './core/experience-policy.429179ea9a28.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.042327692a7c.mjs';
import { createViewStateBridge } from './core/view-state-bridge.883078b39cb4.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.c5e18b55efdc.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.baed84a1ba18.mjs';
import { createUpdateStateController } from './core/update-state-controller.81348a3fee42.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.5a31eddc38f5.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.c0150bff59fe.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.2485a49ab11c.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.fac9144fc316.mjs';
import { projectNameFromPath } from './shell/project-picker.ea0bc4c6944c.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.6952082465a2.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.a576f924208d.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.ba6b2aa756c6.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.50a34abeac8f.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.67e305a6c3fa.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.724f6299e800.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.a81a2d0ba0ea.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.73fb20fc2e5c.mjs'), import('./views/workroom/terminal-client.63da61499218.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.f7347d94f302.mjs'), import('./control-plane/route-registry.1a5e8d6de7a1.mjs'), import('./control-plane/live-domain-workspace.90c65cc5b527.mjs'), import('./views/browser/browser-view.55408862fac3.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.74495f513f59.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.fa7a21c42a6b.mjs'), import('./views/onboarding/onboarding-view.8cf06e7aae68.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.e48bb36f75eb.mjs'), import('./views/settings/settings-view.60c60cb272be.mjs')]);
