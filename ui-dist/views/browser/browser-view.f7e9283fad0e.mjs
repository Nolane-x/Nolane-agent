import { renderBrowserInspector } from './browser-inspector.85ddcabba350.mjs';
import { defaultBrowserTimeline, renderBrowserTimeline } from './browser-timeline.d9729a8d5c8d.mjs';
...
export function createBrowserWorkspaceController({ api, projectId = null, projectName = null, missionId = null, goalId = null, language = 'en' } = {}) {
  ...
  let state = Object.freeze({ status: 'idle', language: languageKey(language), projectId: selectedProject, projectName: selectedProjectName, missionId: selectedMission, goalId: selectedGoal, urlDraft: 'about:blank', runtime: {}, detect: {}, session: null, tabs: Object.freeze([]), permissions: {}, errors: Object.freeze([]), sessionOpen: false, loadedAt: null, events: Object.freeze([]), screenshot: Object.freeze({ status: 'empty', dataUrl: null, bytes: 0, sha256: null, error: null }) });
  ...
  return Object.freeze({ load, refresh: load, setUrl, open, goto, close, captureScreenshot, installRuntime, snapshot: () => state });
}
