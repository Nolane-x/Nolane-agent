...
const originOf = (tabs = []) => { const value = tabs?.[0]?.url; if (!value || value === 'about:blank') return null; try { const parsed = new URL(value); return ['http:', 'https:'].includes(parsed.protocol) ? parsed.origin : null; } catch { return null; } };
