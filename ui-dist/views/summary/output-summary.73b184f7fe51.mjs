...
export function renderOutputSummary(state = {}) {
  ...
  const terminalSection = value.terminals?.length ? `<section><div class="summary-heading"><h3>${esc(copy('summary.terminals'))}</h3><span class="summary-count">${value.terminals.length}</span></div>${rows(value.terminals, empty.terminals, (item) => `<button class="summary-row" data-terminal-id="${esc(item.id)}"><span class="summary-icon" aria-hidden="true">›_</span><span><strong>${esc(item.label ?? item.shell ?? item.id)}</strong><small>${esc(item.state ?? '')}${item.cwd ? ` · ${esc(item.cwd)}` : ''}</small></span></button>`)}</section>` : '';
