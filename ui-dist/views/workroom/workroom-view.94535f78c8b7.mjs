...
export function createWorkroomModel({ projectId, missionId = null, returnPath = '/', language = 'en' } = {}) {
  ...
  return Object.freeze({
    ...
    snapshot() { return Object.freeze({ projectId: String(projectId), missionId: missionId ? String(missionId) : null, returnPath: String(returnPath || '/'), language: languageOf(language), panels: Object.freeze(Object.fromEntries(Object.entries(panels).map(([key, value]) => [key, Object.freeze({ ...value })]))), openFiles: Object.freeze([...openFiles]), activeFile, tree: Object.freeze([...tree]), file, draftContent, dirty: file ? draftContent !== String(file.content ?? '') : false, activeTab, diff, error, loading, agentTab, terminal: Object.freeze({ ...terminal }) }); },
  });
}
...
function copy(language) {
  ...
  return {
    ...
    local: vi ? 'Cục bộ' : 'Local', errors: vi ? 'lỗi' : 'errors', warnings: vi ? 'cảnh báo' : 'warnings', bytes: vi ? 'byte' : 'bytes', changed: vi ? 'đã thay đổi' : 'changed', unchanged: vi ? 'không thay đổi' : 'unchanged', original: vi ? 'Bản gốc' : 'Original', modified: vi ? 'Bản nháp' : 'Draft', previewUnavailable: vi ? 'Chưa có bản xem trước cho tệp này.' : 'No preview is available for this file yet.',
...
function renderAgentPane(snapshot, language) {
  ...
  const controls = `<header><div><button type="button" data-workroom-agent-tab="agent" aria-pressed="${!terminalOpen}">Agent</button><button type="button" data-workroom-agent-tab="terminal" aria-pressed="${terminalOpen}">${c.terminal}</button></div></header>`;
  if (!terminalOpen) return `<aside class="workroom-agent" aria-label="${c.agent}" data-panel="agent" data-open="${snapshot.panels.agent.open}">${controls}<div class="workroom-agent__body"><div class="agent-state-orb"><span></span></div><strong>${c.agentReady}</strong><p>${c.agentHint}</p><button type="button" data-workroom-action="terminal">${c.openTerminal}</button></div><footer><textarea data-workroom-steer placeholder="${c.steer}" aria-label="${c.steer}"></textarea><button type="button" data-workroom-action="steer" aria-label="${languageOf(language) === 'vi' ? 'Gửi chỉ dẫn' : 'Send direction'}">↑</button></footer></aside>`;
  return `<aside class="workroom-agent" aria-label="${c.agent}" data-panel="agent" data-open="${snapshot.panels.agent.open}">${controls}<div class="workroom-terminal" data-terminal-status="${escapeHtml(terminal.status)}"><header><div><strong>${escapeHtml(terminal.title)}</strong><small>${terminal.exited ? c.terminalDisconnected : escapeHtml(terminal.status)}</small></div><button type="button" data-workroom-action="terminal-close">${c.closeTerminal}</button></header><pre data-workroom-terminal-output aria-live="polite">${escapeHtml(terminal.output)}</pre><form data-workroom-terminal-form><label class="sr-only" for="workroom-terminal-input">${c.terminalInput}</label><input id="workroom-terminal-input" data-workroom-terminal-input autocomplete="off" spellcheck="false" placeholder="${c.terminalInput}"${terminal.exited ? ' disabled' : ''}><button type="submit"${terminal.exited ? ' disabled' : ''}>${c.send}</button></form></div></aside>`;
}
...
export function renderWorkroomView(snapshot, { language = snapshot?.language ?? 'en' } = {}) {
  ...
  return `<section class="workroom-view" data-project-id="${escapeHtml(snapshot.projectId)}" data-active-tab="${escapeHtml(snapshot.activeTab ?? 'editor')}"><header class="workroom-header"><div><a href="#${escapeHtml(snapshot.returnPath)}" data-route="${escapeHtml(snapshot.returnPath)}">← ${c.back}</a><h1>${c.studio}</h1><p>${c.description}</p></div><div><button type="button" data-workroom-action="command">⌘ ${c.command}</button><button type="button" data-workroom-action="terminal"${terminalDisabled}>＋ ${c.terminal}</button></div></header>${error}<div class="workroom-tabs"><button type="button" data-workroom-tab="editor" aria-pressed="${snapshot.activeTab === 'editor'}">${c.editor}</button><button type="button" data-workroom-tab="changes" aria-pressed="${snapshot.activeTab === 'changes'}">${c.changes}${snapshot.dirty ? ' •' : ''}</button><button type="button" data-workroom-tab="preview" aria-pressed="${snapshot.activeTab === 'preview'}">${c.preview}</button><span></span><button type="button" data-workroom-action="layout">${c.layout}</button></div><div class="workroom-grid" data-active-file="${escapeHtml(snapshot.activeFile ?? '')}"><aside class="workroom-files" data-panel="files" data-open="${snapshot.panels.files.open}"><header><strong>${c.files}</strong></header><label><span>⌕</span><input type="search" data-workroom-filter placeholder="${c.filterFiles}" aria-label="${c.filterFiles}"></label><div data-workroom-tree>${renderTree(snapshot, language)}</div></aside><main class="workroom-editor" data-panel="editor" data-open="${snapshot.panels.editor.open}"><header>${snapshot.activeFile ? `<span>${escapeHtml(snapshot.activeFile)}</span><span class="workroom-editor-actions"><button type="button" data-workroom-action="diff"${snapshot.file ? '' : ' disabled'}>${c.diff}</button><button type="button" data-workroom-action="save"${!snapshot.dirty ? ' disabled' : ''}>${snapshot.loading ? c.saving : c.save}</button></span>` : `<span>${c.noFile}</span>`}</header>${renderEditor(snapshot, language)}</main>${renderAgentPane(snapshot, language)}</div><footer class="workroom-statusbar"><span>main</span><span>0 ${c.errors}</span><span>0 ${c.warnings}</span><i></i><span>${c.local}</span><span>UTF-8</span></footer></section>`;
}
