...
import { renderProjectPicker } from '../../shell/project-picker.c4820072ba9e.mjs';
...
function modelPickerOptions(model) {
  ...
  return [
    ...
    ...readyModelProfiles(model.models, model.providers).slice(0, 50).map((item) => {
...
export function createHomeController({ api, language = 'en' } = {}) {
  ...
  return Object.freeze({
    ...
    setMenu(menu){ return patch({menu}); }, setIntent(intent){ return patch({intent}); }, setProject(id){ return patch({selectedProjectId:String(id ?? ''),projectMenuOpen:false,projectQuery:''}); }, setProjectMenu(open){ return patch({projectMenuOpen:Boolean(open)}); }, setProjectQuery(query){ return patch({projectQuery:String(query ?? '')}); }, setModel(id){ return patch({selectedModel:id}); }, addSkill(id){ return patch({selectedSkillIds:selectedSkillIds([...state.selectedSkillIds,id],state.skills)}); }, removeSkill(id){ return patch({selectedSkillIds:selectedSkillIds(state.selectedSkillIds.filter((item) => item !== String(id ?? '')),state.skills)}); }, setLanguage(next){ language=next; return patch({language:next}); },
