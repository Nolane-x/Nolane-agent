import { createRouter } from './core/router.ea652520f983.mjs';
import { createUiStore } from './core/ui-store.097841afe24a.mjs';
import { createUiBus } from './core/ui-bus.1b5bf4d49508.mjs';
import { createApiClient } from './core/api-client.b01942798996.mjs';
import { createLayoutStore } from './core/layout-store.0048ca803769.mjs';
import { createResizableRegionController } from './core/resizable-region.bee04fe37dfa.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.561e76d611a3.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.c346220b70f4.mjs';
import { normalizeExperience } from './core/experience-policy.1d10ac3569b8.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.189eadf2d500.mjs';
import { createViewStateBridge } from './core/view-state-bridge.ca21f2a1b4d5.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.203fe198b04f.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.425a3d5808e0.mjs';
import { createUpdateStateController } from './core/update-state-controller.3747892b9729.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.564846b08387.mjs';
import { settingsSectionFromRoute } from './core/settings-route.c773fce579df.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.259a00cc16fd.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.abc3c5c6e6f7.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.db4d8edb4981.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.dc84f06ee4bc.mjs';
import { projectNameFromPath } from './shell/project-picker.bf0b8f961abe.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.29c7a08a45ea.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.f49d8c01234d.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.a182ca505646.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.e6d6d444868b.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.09f7c9721d46.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.33477acbd034.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.0b1dd2503c02.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.d60b93bd40cc.mjs'), import('./views/workroom/terminal-client.2ce426c26d0c.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.dd6a60bb71dd.mjs'), import('./control-plane/route-registry.e24c9ac2e1a9.mjs'), import('./control-plane/live-domain-workspace.02a453854a25.mjs'), import('./views/browser/browser-view.28329da8232e.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.50e8de2bce56.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.65e6deacd0d9.mjs'), import('./views/onboarding/onboarding-view.d815bdf4d1c3.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.3b6048f368cd.mjs'), import('./views/settings/settings-view.6392d66c0486.mjs')]);
