import { createRouter } from './core/router.8b920315806c.mjs';
import { createUiStore } from './core/ui-store.f39f2c3c96f4.mjs';
import { createUiBus } from './core/ui-bus.9c9123c65913.mjs';
import { createApiClient } from './core/api-client.c9f808f19080.mjs';
import { createLayoutStore } from './core/layout-store.c2cbb62b1797.mjs';
import { createResizableRegionController } from './core/resizable-region.bd00e95c2de2.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.0d520b653821.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.317b444a886e.mjs';
import { normalizeExperience } from './core/experience-policy.4578fdde59ca.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.dadbe719ad94.mjs';
import { createViewStateBridge } from './core/view-state-bridge.b0018d7c475b.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.4bd58a3bfe65.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.77a1a2446818.mjs';
import { createUpdateStateController } from './core/update-state-controller.d9c7646bbfb4.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.4e84c95d2685.mjs';
import { settingsSectionFromRoute } from './core/settings-route.21f98fd464ff.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.b13d8d4f0113.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.ad5f86304a52.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.d869a86957be.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.afc1b2b30ce8.mjs';
import { projectNameFromPath } from './shell/project-picker.d040335ff898.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.deef08d12879.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.7e302b01aa67.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.6d9b0860b848.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.eaa7af89f340.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.6f268573092a.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.ca6558dbfa27.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.22128f81526f.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.a7ad4c8b8f76.mjs'), import('./views/workroom/terminal-client.4815d3fc78c9.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.3be7f05e6420.mjs'), import('./control-plane/route-registry.387fd5805a0d.mjs'), import('./control-plane/live-domain-workspace.13bf3f4f6cde.mjs'), import('./views/browser/browser-view.d099d4181436.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.1449187a4d92.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.1478b3287698.mjs'), import('./views/onboarding/onboarding-view.00041703aadf.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.3a52fc1e5d3e.mjs'), import('./views/settings/settings-view.d459acfa509c.mjs')]);
