import { createRouter } from './core/router.63dda8dbb5ee.mjs';
import { createUiStore } from './core/ui-store.5f7285eed208.mjs';
import { createUiBus } from './core/ui-bus.0d0a43758b2b.mjs';
import { createApiClient } from './core/api-client.fdb1b4ebe0df.mjs';
import { createLayoutStore } from './core/layout-store.a9c141b718e6.mjs';
import { createResizableRegionController } from './core/resizable-region.b62506cfe955.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.9d7cc8a018a0.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.93b2ad8af169.mjs';
import { normalizeExperience } from './core/experience-policy.118b33dbc193.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.06d9d5d51482.mjs';
import { createViewStateBridge } from './core/view-state-bridge.7f311fd4eecf.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.f39bff4be4bf.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.51de876689b5.mjs';
import { createUpdateStateController } from './core/update-state-controller.813bea5313c4.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.617c9041c2c6.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.624040619c31.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.c621961b785a.mjs';
import { projectNameFromPath } from './shell/project-picker.cfccb554e05a.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.ef7ceefd2cb8.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.d27084b81895.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.94bafb7aeee2.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.a219ca2c1b6e.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.1f1a229b06aa.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.b0425daddfc5.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.b084664bb381.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.972306fa117c.mjs'), import('./views/workroom/terminal-client.3f2762b567e3.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.1f3025519e94.mjs'), import('./control-plane/route-registry.95549847f771.mjs'), import('./control-plane/live-domain-workspace.59f3946c5020.mjs'), import('./views/browser/browser-view.a5b6331c4419.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.372a22c7bbea.mjs'), import('./views/settings/settings-view.5c3b39a56417.mjs')]);
