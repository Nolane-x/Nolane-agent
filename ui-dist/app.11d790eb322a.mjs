import { createRouter } from './core/router.ccf30d74ffa5.mjs';
import { createUiStore } from './core/ui-store.64c96e9383a7.mjs';
import { createUiBus } from './core/ui-bus.b55817c1fbfd.mjs';
import { createApiClient } from './core/api-client.a3a750f1d102.mjs';
import { createLayoutStore } from './core/layout-store.e9c8ec9e0b83.mjs';
import { createResizableRegionController } from './core/resizable-region.54188c6cd968.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.c12763c9b306.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.2e98e7c86a85.mjs';
import { normalizeExperience } from './core/experience-policy.2ea009688073.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.af2bd90aa282.mjs';
import { createViewStateBridge } from './core/view-state-bridge.cb514e8b320c.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.c538d3ba7c95.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.0a33be261791.mjs';
import { createUpdateStateController } from './core/update-state-controller.a22022432add.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.70f54a31020d.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.3a0a2c70a90c.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.1574fbc77034.mjs';
import { projectNameFromPath } from './shell/project-picker.59df981b6867.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.ea332c51b143.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.cd2dd46f1f2e.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.84ce838185dc.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.10f643eb6d20.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.71016b1491ac.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.87680dff8890.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.36423022ba7c.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.fc0146c75a3c.mjs'), import('./control-plane/route-registry.4d34646c38ed.mjs'), import('./control-plane/live-domain-workspace.d002a17619b1.mjs'), import('./views/browser/browser-view.6875cd8571b1.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.6401943e1f36.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.dc7e13b92f71.mjs'), import('./views/onboarding/onboarding-view.b911731656dd.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.b590abe998a5.mjs'), import('./views/settings/settings-view.e340e270204c.mjs')]);
