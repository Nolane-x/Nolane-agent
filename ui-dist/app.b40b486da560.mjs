import { createRouter } from './core/router.10980e5d5950.mjs';
import { createUiStore } from './core/ui-store.a90efddccdb0.mjs';
import { createUiBus } from './core/ui-bus.0007eb9fd497.mjs';
import { createApiClient } from './core/api-client.4f17c521c184.mjs';
import { createLayoutStore } from './core/layout-store.88decf4c0f0b.mjs';
import { createResizableRegionController } from './core/resizable-region.c039e72c058c.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.daea6ec7eb01.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.88fe22f236c6.mjs';
import { normalizeExperience } from './core/experience-policy.a95fd0f3233b.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.cd98bb3bb284.mjs';
import { createViewStateBridge } from './core/view-state-bridge.bdf5f3ca0d40.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.6d25071acf90.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.499240d43000.mjs';
import { createUpdateStateController } from './core/update-state-controller.d715a356bc12.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.d86f66104385.mjs';
import { settingsSectionFromRoute } from './core/settings-route.8f3412571019.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.b09d559c310e.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.105d9b771155.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.f9defb4fdb6a.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.df2cd5c889f9.mjs';
import { projectNameFromPath } from './shell/project-picker.db4905eaec35.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.aa74fae50e87.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.3ba5d5e4b6a5.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.d09521aedc80.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.2474316a9673.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.6c1471834cea.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.97c97aec79d6.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.ae6e03b6c1dd.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.4205172b653a.mjs'), import('./views/workroom/terminal-client.2480a8181d9e.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.1f4807d98b54.mjs'), import('./control-plane/route-registry.0fdd35a42154.mjs'), import('./control-plane/live-domain-workspace.9cdfe3b6a6ff.mjs'), import('./views/browser/browser-view.6abf8427f2fc.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.89f1e71121be.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.2981dc5ec8a1.mjs'), import('./views/onboarding/onboarding-view.6454b0135699.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.ffa96769373c.mjs'), import('./views/settings/settings-view.cba943e0708e.mjs')]);
