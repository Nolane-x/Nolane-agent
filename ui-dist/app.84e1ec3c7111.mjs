import { createRouter } from './core/router.60b2d5b7e282.mjs';
import { createUiStore } from './core/ui-store.b14cc077ab49.mjs';
import { createUiBus } from './core/ui-bus.460b0c38cc1c.mjs';
import { createApiClient } from './core/api-client.ef617c33aa36.mjs';
import { createLayoutStore } from './core/layout-store.62e5c4243f09.mjs';
import { createResizableRegionController } from './core/resizable-region.5df9cddb1a4d.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.532ff0eb3719.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.8c6dc2160d50.mjs';
import { normalizeExperience } from './core/experience-policy.bb25565a40f5.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.b55f3a18e748.mjs';
import { createViewStateBridge } from './core/view-state-bridge.768ee9593721.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.9623839777d9.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.a8f3d9ece222.mjs';
import { createUpdateStateController } from './core/update-state-controller.07e79c2aa3e1.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.74a7f3258b5c.mjs';
import { settingsSectionFromRoute } from './core/settings-route.b0e2edfa1ebe.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.da0657f21e2f.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.2e11e9c9fc58.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.47a8a4627ea7.mjs';
import { closeComposerPickers, isComposerPickerInteraction } from './components/composer-picker.052e4320c6ec.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.72a7a4156939.mjs';
import { projectNameFromPath } from './shell/project-picker.f769e99ec572.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.c1a32c339382.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.3c0e9f2b3457.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.707d380a302e.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.a667ead98928.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.fec8db73c460.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.0e531c81c1e5.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.dcad8ad973ce.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.4d0bc5cb91ef.mjs'), import('./views/workroom/terminal-client.d30c116405e0.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.3d48e2a6c27e.mjs'), import('./control-plane/route-registry.1d6c992183fb.mjs'), import('./control-plane/live-domain-workspace.4b1c71524ac4.mjs'), import('./views/browser/browser-view.08219a50fbe5.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.d5f99572dc50.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.44aef58d5064.mjs'), import('./views/onboarding/onboarding-view.d5cd567db16c.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.dfd968fc9cdb.mjs'), import('./views/settings/settings-view.209123c0f811.mjs')]);
