import { createRouter } from './core/router.c9c63ae98b80.mjs';
import { createUiStore } from './core/ui-store.d463b946228d.mjs';
import { createUiBus } from './core/ui-bus.2cba893367f9.mjs';
import { createApiClient } from './core/api-client.435cd8b86116.mjs';
import { createLayoutStore } from './core/layout-store.6316784cb401.mjs';
import { createResizableRegionController } from './core/resizable-region.74af1779a3ce.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.ede693f8bf9a.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.5ae2f3df050e.mjs';
import { normalizeExperience } from './core/experience-policy.a9365baa81b5.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.368c6cf8555a.mjs';
import { createViewStateBridge } from './core/view-state-bridge.7be218a5c8c3.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.2525e4adaadd.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.fd5fe8dbae50.mjs';
import { createUpdateStateController } from './core/update-state-controller.a1f17a1b3e5c.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.791ea3101379.mjs';
import { settingsSectionFromRoute } from './core/settings-route.3b9e84309e3e.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.8f102e2936a9.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.97e17bbc423b.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.9dae7bd970cb.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.08859b868141.mjs';
import { projectNameFromPath } from './shell/project-picker.1328492b2330.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.bfcf9da1019e.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.98b821dfd8f4.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.4601198dbb03.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.26c2f4e0b365.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.e3cc41f4c28f.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.ab8f74e93557.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.da596a3298a9.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.76be5a8a0cd4.mjs'), import('./views/workroom/terminal-client.943ec8d7148a.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.8f7432f0c99b.mjs'), import('./control-plane/route-registry.59fde6e95cbf.mjs'), import('./control-plane/live-domain-workspace.72bacd56a0af.mjs'), import('./views/browser/browser-view.bde0e76de63b.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.41503358c644.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.8f59fa66288b.mjs'), import('./views/onboarding/onboarding-view.30a89e5730d4.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.b61440e8ea14.mjs'), import('./views/settings/settings-view.7844a9757b3b.mjs')]);
