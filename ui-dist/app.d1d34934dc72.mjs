import { createRouter } from './core/router.1e2130046f16.mjs';
import { createUiStore } from './core/ui-store.fe46fc63c4ac.mjs';
import { createUiBus } from './core/ui-bus.0a2e41f18faa.mjs';
import { createApiClient } from './core/api-client.83e819e2f367.mjs';
import { createLayoutStore } from './core/layout-store.b2975a55c5c2.mjs';
import { createResizableRegionController } from './core/resizable-region.300d6d221bd1.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.af7afc7d34a7.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.ff45d67f4c55.mjs';
import { normalizeExperience } from './core/experience-policy.8ea8add7f65d.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.ab917258eb1f.mjs';
import { createViewStateBridge } from './core/view-state-bridge.3298b0735336.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.dac26c8f747b.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.7d9c94e2f4ef.mjs';
import { createUpdateStateController } from './core/update-state-controller.8edd4825e18e.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.31722a292ad3.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.35404e36758b.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.e0e1f1a44acb.mjs';
import { projectNameFromPath } from './shell/project-picker.5061940f829f.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.bfca67c59e22.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.219f606bdd9d.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.0b4c48dcadda.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.54981d1cd1d4.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.4e2c4a985ab2.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.77e143ee7773.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.699fa62da14a.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.19aa196ecdc4.mjs'), import('./views/workroom/terminal-client.aa5253fc3d42.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.003917228cba.mjs'), import('./control-plane/route-registry.276f83deb8e2.mjs'), import('./control-plane/live-domain-workspace.451375645f0b.mjs'), import('./views/browser/browser-view.48a84820205b.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.b9b15c339fb4.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.219ab0f1b5be.mjs'), import('./views/onboarding/onboarding-view.eb18fba9e058.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.428638318633.mjs'), import('./views/settings/settings-view.5405c258a032.mjs')]);
