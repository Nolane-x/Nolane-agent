import { createRouter } from './core/router.37588afa3280.mjs';
import { createUiStore } from './core/ui-store.067edc7c37a5.mjs';
import { createUiBus } from './core/ui-bus.272185fbe3a7.mjs';
import { createApiClient } from './core/api-client.c0db4a5daea9.mjs';
import { createLayoutStore } from './core/layout-store.23c13686fa6c.mjs';
import { createResizableRegionController } from './core/resizable-region.0bf2ad9e911f.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.b3e37df484f5.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.d3513b4d407b.mjs';
import { normalizeExperience } from './core/experience-policy.27a7be28f263.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.eea804bb4fff.mjs';
import { createViewStateBridge } from './core/view-state-bridge.31e6abba4c45.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.f56b427a12a6.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.d2c117008aee.mjs';
import { createUpdateStateController } from './core/update-state-controller.e041f07349ee.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.c30511a08f7a.mjs';
import { settingsSectionFromRoute } from './core/settings-route.34e6236bbb30.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.3c8523769b9e.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.a44ef8f69cad.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.fa171bac6f83.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.809fc6c9fbc7.mjs';
import { projectNameFromPath } from './shell/project-picker.a70cbff35b4d.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.a63f27ed624b.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.b84da68f60b4.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.89fc52fa0390.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.0c74acab2d1f.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.21dfd14c511f.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.4655a93c3376.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.2c60fda9b699.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.2b743df2c9bf.mjs'), import('./views/workroom/terminal-client.5d9f49cfbe3c.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.a7481fc91c3f.mjs'), import('./control-plane/route-registry.087855167c95.mjs'), import('./control-plane/live-domain-workspace.83398ac41f96.mjs'), import('./views/browser/browser-view.0987816a9f2f.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.75350e4ea4c5.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.a270fb5bef24.mjs'), import('./views/onboarding/onboarding-view.f78294b512c3.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.210644c60b04.mjs'), import('./views/settings/settings-view.280418363a27.mjs')]);
