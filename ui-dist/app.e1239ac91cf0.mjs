import { createRouter } from './core/router.0667d3afc684.mjs';
import { createUiStore } from './core/ui-store.765ad094e5aa.mjs';
import { createUiBus } from './core/ui-bus.55bc02c0e690.mjs';
import { createApiClient } from './core/api-client.2d0c921c4b40.mjs';
import { createLayoutStore } from './core/layout-store.930b5022eef1.mjs';
import { createResizableRegionController } from './core/resizable-region.7d97627b475e.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.75b4d3810a52.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.b510e8ee88b5.mjs';
import { normalizeExperience } from './core/experience-policy.d46a70b8fd64.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.d48ccdb80d6d.mjs';
import { createViewStateBridge } from './core/view-state-bridge.5d74523b98cf.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.c01cadcaacbe.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.2342bf03ea27.mjs';
import { createUpdateStateController } from './core/update-state-controller.ae7db850cf54.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.4ee21d02c727.mjs';
import { settingsSectionFromRoute } from './core/settings-route.46bbd93a00d5.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.e7cbcc59b9ca.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.15bb2926352c.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.6535e5027795.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.0901f19d536b.mjs';
import { projectNameFromPath } from './shell/project-picker.922c9d1feb22.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.38c3a66626ae.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.6e856e548d61.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.672a39417197.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.de8da7d7f642.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.15fb46477310.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.bc8f2af1a5ea.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.f5a1916cac02.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.bdf9fd7a64a4.mjs'), import('./views/workroom/terminal-client.7900fdb3d8a5.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.0bc5e4750a4f.mjs'), import('./control-plane/route-registry.8542cbd0caa2.mjs'), import('./control-plane/live-domain-workspace.3ed37c0d65ae.mjs'), import('./views/browser/browser-view.35a4cf3890e5.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.a53976ea9728.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.7025d43cc426.mjs'), import('./views/onboarding/onboarding-view.f3e173ff280f.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.fb7a78d9bcdd.mjs'), import('./views/settings/settings-view.1a3467c031b9.mjs')]);
