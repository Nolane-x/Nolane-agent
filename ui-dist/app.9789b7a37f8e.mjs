import { createRouter } from './core/router.639a6e53d399.mjs';
import { createUiStore } from './core/ui-store.f84a1646e7a5.mjs';
import { createUiBus } from './core/ui-bus.727c10064660.mjs';
import { createApiClient } from './core/api-client.3ec0e77bebdd.mjs';
import { createLayoutStore } from './core/layout-store.daba7bf7c639.mjs';
import { createResizableRegionController } from './core/resizable-region.e9e1993c3730.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.4c0ee0f61318.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.7e6572073ee9.mjs';
import { normalizeExperience } from './core/experience-policy.db9f4bef713a.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.391bcf104f72.mjs';
import { createViewStateBridge } from './core/view-state-bridge.7cbf990bb441.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.080aef66272f.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.ed931dc113d8.mjs';
import { createUpdateStateController } from './core/update-state-controller.d605d281fd25.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.81224b8c2efb.mjs';
import { settingsSectionFromRoute } from './core/settings-route.a80b087d5a74.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.00e439b501ac.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.4d4d45ecd16d.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.5df3609414c4.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.06a3f93e94f4.mjs';
import { projectNameFromPath } from './shell/project-picker.b2ec1dca8999.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.e19fdca3e599.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.ca13069d4848.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.28f43f9e5a0d.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.e73723389e65.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.4b473e2b78f1.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.b753ebeb5972.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.e2d34002e693.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.b205bed33197.mjs'), import('./views/workroom/terminal-client.2435872041cd.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.87e6803ff8d8.mjs'), import('./control-plane/route-registry.0cf853c1efb7.mjs'), import('./control-plane/live-domain-workspace.dfbbba6a4f89.mjs'), import('./views/browser/browser-view.3ce5cfd5ddf8.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.02fadec77a3d.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.ac07020a2304.mjs'), import('./views/onboarding/onboarding-view.fe1071c69628.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.c680b03fa3c0.mjs'), import('./views/settings/settings-view.889dbbfa86fb.mjs')]);
