import { createRouter } from './core/router.10945bba518b.mjs';
import { createUiStore } from './core/ui-store.191adb29575f.mjs';
import { createUiBus } from './core/ui-bus.06faba621d3a.mjs';
import { createApiClient } from './core/api-client.5143852b3d2b.mjs';
import { createLayoutStore } from './core/layout-store.ced1f939fd00.mjs';
import { createResizableRegionController } from './core/resizable-region.c7f98070c6d1.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.7bb2fc09b531.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.dc52c61d4338.mjs';
import { normalizeExperience } from './core/experience-policy.8da2ce2e79d0.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.059fdafea889.mjs';
import { createViewStateBridge } from './core/view-state-bridge.7bb376486708.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.6057a14279b2.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.294358b1abef.mjs';
import { createUpdateStateController } from './core/update-state-controller.c4526923c295.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.7805e7d40957.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.7f682de31201.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.826fb8404636.mjs';
import { projectNameFromPath } from './shell/project-picker.e03bcffd07ec.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.086f439883e9.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.0453c5c56694.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.ef666dfb8624.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.ff602ee225e7.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.5bf490c06b3a.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.7b0c56f78755.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.1074fd4ba351.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.c3cd06597714.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.a65b06336249.mjs'), import('./control-plane/route-registry.265b469002aa.mjs'), import('./control-plane/live-domain-workspace.1233991ef163.mjs'), import('./views/browser/browser-view.2adc748f468b.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.3f511b89b216.mjs'), import('./views/settings/settings-view.2c2eede0dab1.mjs')]);
...
document.addEventListener('click',async(event)=>{
  ...
  const projectAction=event.target.closest?.('[data-project-action]'); if(projectAction && ['new','add','none'].includes(projectAction.dataset.projectAction)){event.preventDefault();if(['new','add'].includes(projectAction.dataset.projectAction)){await requestProjectCreation();}else{window.dispatchEvent(new CustomEvent('nolane:project-selected',{detail:{projectId:'',source:'sidebar'}}));}return;}
