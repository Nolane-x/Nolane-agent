import { createRouter } from './core/router.384c49457eb2.mjs';
import { createUiStore } from './core/ui-store.cc1cff5794b0.mjs';
import { createUiBus } from './core/ui-bus.37b216169984.mjs';
import { createApiClient } from './core/api-client.91cb2c68f8df.mjs';
import { createLayoutStore } from './core/layout-store.355a79f40e39.mjs';
import { createResizableRegionController } from './core/resizable-region.c32572fcab5e.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.fabae9b1067a.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.740b6f6442cc.mjs';
import { normalizeExperience } from './core/experience-policy.876bf8e520a9.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.4b65645c5384.mjs';
import { createViewStateBridge } from './core/view-state-bridge.9f119d9439bd.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.b9ed19185aa7.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.926c6b75b066.mjs';
import { createUpdateStateController } from './core/update-state-controller.db0aba5bdc1d.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.0a0f6eefd42f.mjs';
import { settingsSectionFromRoute } from './core/settings-route.5069e274e95c.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.6ba0f8db9fea.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.18750a39c1af.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.04237098d329.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.6f3f03d30dd2.mjs';
import { projectNameFromPath } from './shell/project-picker.342e267e3100.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.4cced6685aa0.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.ab2733526138.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.a6a603b2ff3c.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.1e36c866b0a0.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.57f2b8d2db98.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.147cd1825d7d.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.34416869252c.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.2620fed3c854.mjs'), import('./views/workroom/terminal-client.bd2bbb7a2afd.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.576f076a7b72.mjs'), import('./control-plane/route-registry.9e022cdd3c69.mjs'), import('./control-plane/live-domain-workspace.21358dadf281.mjs'), import('./views/browser/browser-view.5d6e9fc60634.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.3c6cc9038159.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.6f7c1cefeb2d.mjs'), import('./views/onboarding/onboarding-view.b969cb414722.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.dfe1ca039c0c.mjs'), import('./views/settings/settings-view.bd7f294b590a.mjs')]);
