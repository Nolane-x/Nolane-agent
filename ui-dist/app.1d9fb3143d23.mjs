import { createRouter } from './core/router.d22d74932980.mjs';
import { createUiStore } from './core/ui-store.1932a64136b6.mjs';
import { createUiBus } from './core/ui-bus.8fd00a018ef2.mjs';
import { createApiClient } from './core/api-client.dc296ed738ad.mjs';
import { createLayoutStore } from './core/layout-store.9d20adbbf877.mjs';
import { createResizableRegionController } from './core/resizable-region.3ab0e2d16fe5.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.d6d891a88bdc.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.f5081053d224.mjs';
import { normalizeExperience } from './core/experience-policy.1542f37da024.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.24b42f70b151.mjs';
import { createViewStateBridge } from './core/view-state-bridge.bb49949be55f.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.37f8afe7d9a1.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.0a7b3e3cb0ad.mjs';
import { createUpdateStateController } from './core/update-state-controller.e6862a4664b6.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.dfa6bd2f5abe.mjs';
import { settingsSectionFromRoute } from './core/settings-route.0b80d2988d15.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.8cc7b2e3de50.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.da951d07b270.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.81289eb74f16.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.5d69af77d656.mjs';
import { projectNameFromPath } from './shell/project-picker.4135446e706a.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.2b79316be69f.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.8d3bc05ae62b.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.f88b5f7a26dc.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.4056461861dc.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.200472847b5f.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.0fc7c4c62ce2.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.fb6b9280d364.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.930b448ab94f.mjs'), import('./views/workroom/terminal-client.b810cb1804b4.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.7d73e26b460a.mjs'), import('./control-plane/route-registry.8434db576b4a.mjs'), import('./control-plane/live-domain-workspace.2c53a366553d.mjs'), import('./views/browser/browser-view.d1812747c45c.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.12e23a31ffe9.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.5dbab95e56b4.mjs'), import('./views/onboarding/onboarding-view.298ed2690762.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.05736969fb6c.mjs'), import('./views/settings/settings-view.c52fac02f6a3.mjs')]);
...
async function render(path) {
  ...
  const state=await router.navigate(path,{replace:true});if(router.current()!==state)return;currentRouteState=state;document.title=`${localizeRouteTitle(state.path,state.title,cachedPreferences.language)} — Nolane Agent`;
