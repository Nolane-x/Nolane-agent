import { createRouter } from './core/router.d1b66d0f164e.mjs';
import { createUiStore } from './core/ui-store.996c70564b11.mjs';
import { createUiBus } from './core/ui-bus.f913eeef7775.mjs';
import { createApiClient } from './core/api-client.e40bc31bc1e6.mjs';
import { createLayoutStore } from './core/layout-store.9d7239f38976.mjs';
import { createResizableRegionController } from './core/resizable-region.7859bb7b95aa.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.08854efd647f.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.b45a3ad56ad2.mjs';
import { normalizeExperience } from './core/experience-policy.f5dc34277761.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.509890df6e7f.mjs';
import { createViewStateBridge } from './core/view-state-bridge.100f0e620eeb.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.60f7baa69010.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.4526c3ede65b.mjs';
import { createUpdateStateController } from './core/update-state-controller.36de21efd187.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.c77401d40981.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.5c45fae116ad.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.1538e75a1ad5.mjs';
import { projectNameFromPath } from './shell/project-picker.96d64617b9c7.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.1d3bd939cbe7.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.8af9d4b2f5d0.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.a51d8a2d5f00.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.a4db619504a9.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.26c01359e60f.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.2c0ed4852997.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.d7d04cd3beed.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.e90109f5ad52.mjs'), import('./control-plane/route-registry.90ecc502439d.mjs'), import('./control-plane/live-domain-workspace.bc8e38ea4678.mjs'), import('./views/browser/browser-view.479a5097d96e.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.172a3769deb0.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.7f08413ae8c8.mjs'), import('./views/onboarding/onboarding-view.e759e4ddabef.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.ac18efd5cfdb.mjs'), import('./views/settings/settings-view.50d3cf419e0a.mjs')]);
