import { createRouter } from './core/router.5d9bb76eeca6.mjs';
import { createUiStore } from './core/ui-store.c1986b5c8e94.mjs';
import { createUiBus } from './core/ui-bus.46b2a426fc4e.mjs';
import { createApiClient } from './core/api-client.5989f37a2bc9.mjs';
import { createLayoutStore } from './core/layout-store.23e4d9fc9bd7.mjs';
import { createResizableRegionController } from './core/resizable-region.419d7ec8502a.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.a07553fd3617.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.015d183a6a8b.mjs';
import { normalizeExperience } from './core/experience-policy.86e6cdb15cb9.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.d4efc3d22a6c.mjs';
import { createViewStateBridge } from './core/view-state-bridge.16f9c04bfde1.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.0338e8fc33ac.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.a4bf651953cd.mjs';
import { createUpdateStateController } from './core/update-state-controller.545d5f97bea0.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.3a5117f36f65.mjs';
import { settingsSectionFromRoute } from './core/settings-route.2ac9742fa091.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.83a6cad55b66.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.8686a5e29c76.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.b180fd6603a0.mjs';
import { closeComposerPickers, isComposerPickerInteraction } from './components/composer-picker.4abab312d321.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.5d574daeb2bd.mjs';
import { projectNameFromPath } from './shell/project-picker.8dece91dd0d7.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.892452114a6a.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.2cb9d00c8e5d.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.6465ba2b1876.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.f99feef56b0e.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.781f06de6e9b.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.9a4f837d3baa.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.eb6362e9ff96.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.4137787534e1.mjs'), import('./views/workroom/terminal-client.34a76e71f212.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.f83b9e85be73.mjs'), import('./control-plane/route-registry.e1975890afed.mjs'), import('./control-plane/live-domain-workspace.a2d889a8f637.mjs'), import('./views/browser/browser-view.b175e8c8308f.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.3ac5e3e5898b.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.9cf20a416dd5.mjs'), import('./views/onboarding/onboarding-view.2e457d2a524a.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.650d73053d03.mjs'), import('./views/settings/settings-view.f6bee7a9f196.mjs')]);
