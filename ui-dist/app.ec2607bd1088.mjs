import { createRouter } from './core/router.379219b0f7ed.mjs';
import { createUiStore } from './core/ui-store.1ba12ced241f.mjs';
import { createUiBus } from './core/ui-bus.8d18634b6560.mjs';
import { createApiClient } from './core/api-client.e0f91fa69649.mjs';
import { createLayoutStore } from './core/layout-store.18f3ca7243c4.mjs';
import { createResizableRegionController } from './core/resizable-region.1e0de7f35e65.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.c99ecb2d0015.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.d989f729adfe.mjs';
import { normalizeExperience } from './core/experience-policy.c555a9d66195.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.7fac0fc4c742.mjs';
import { createViewStateBridge } from './core/view-state-bridge.63d9178716e6.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.805071aba017.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.44dc6f5038bf.mjs';
import { createUpdateStateController } from './core/update-state-controller.e79bb2db06ab.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.b41c681452c7.mjs';
import { settingsSectionFromRoute } from './core/settings-route.91563b1f31d2.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.0e99ca97ca31.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.2f778ec5da72.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.3b3e188afc67.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.ec72ce13fc49.mjs';
import { projectNameFromPath } from './shell/project-picker.588b777823eb.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.1c167ac9cf79.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.ec0575fe1f4b.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.894fa787ccea.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.c41f3627f0ec.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.e9f327978323.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.e710b070b4ba.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.a8b2fcb0cba0.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.1ddc7c0790f2.mjs'), import('./views/workroom/terminal-client.24d2a8171e28.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.c942d94fb324.mjs'), import('./control-plane/route-registry.9810e983ff34.mjs'), import('./control-plane/live-domain-workspace.7ab9fef65d21.mjs'), import('./views/browser/browser-view.715072c14972.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.4e418a6923e5.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.05fa54309ff9.mjs'), import('./views/onboarding/onboarding-view.14e348d1132b.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.560d99755b75.mjs'), import('./views/settings/settings-view.3c002678d225.mjs')]);
