import { createRouter } from './core/router.8ad3cfefc897.mjs';
import { createUiStore } from './core/ui-store.98d02bc728af.mjs';
import { createUiBus } from './core/ui-bus.4024b322e7e2.mjs';
import { createApiClient } from './core/api-client.b150a0f7e968.mjs';
import { createLayoutStore } from './core/layout-store.61b278bcdb3f.mjs';
import { createResizableRegionController } from './core/resizable-region.0272ee7ce882.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.4c8cdf93c681.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.8009e696bf22.mjs';
import { normalizeExperience } from './core/experience-policy.9583e2b65519.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.3fed10b29574.mjs';
import { createViewStateBridge } from './core/view-state-bridge.ba049b056256.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.242c110410fa.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.813b4f2d69a2.mjs';
import { createUpdateStateController } from './core/update-state-controller.fed07cf9ed30.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.ba71e2c1d7b8.mjs';
import { settingsSectionFromRoute } from './core/settings-route.3c3799f622e4.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.264d81d56bbc.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.2ae3a36b8eac.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.b1c627c33a43.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.32a7f46e170d.mjs';
import { projectNameFromPath } from './shell/project-picker.2b40a4067139.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.39e46d78f4be.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.e97856759297.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.6f4bbacef98c.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.bc757287d5c5.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.2a576aa85291.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.76ec7375d1fc.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.3d50935dcfd1.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.f5556bc895dd.mjs'), import('./views/workroom/terminal-client.3af87fdb8a3e.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.47ae7d8e47af.mjs'), import('./control-plane/route-registry.150756241c64.mjs'), import('./control-plane/live-domain-workspace.7d7f49b2fdd4.mjs'), import('./views/browser/browser-view.15257cda233d.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.0702122c449b.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.3996ef468115.mjs'), import('./views/onboarding/onboarding-view.cec439502c50.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.b539a9833265.mjs'), import('./views/settings/settings-view.f6a5848689e9.mjs')]);
