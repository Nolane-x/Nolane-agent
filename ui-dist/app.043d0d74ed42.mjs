import { createRouter } from './core/router.df2c2d40688d.mjs';
import { createUiStore } from './core/ui-store.7ccbdbea8782.mjs';
import { createUiBus } from './core/ui-bus.eaeed6ce175e.mjs';
import { createApiClient } from './core/api-client.2864b9f21b22.mjs';
import { createLayoutStore } from './core/layout-store.0d5d31a11b51.mjs';
import { createResizableRegionController } from './core/resizable-region.f45cea102486.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.b378de6d1664.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.64ffd15358fa.mjs';
import { normalizeExperience } from './core/experience-policy.3e825ba3bea2.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.d19760ab2cca.mjs';
import { createViewStateBridge } from './core/view-state-bridge.abd2664471cb.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.6aa7f9cd7f7c.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.b01c27218b15.mjs';
import { createUpdateStateController } from './core/update-state-controller.f0005579bf38.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.85532a63a889.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.2fd84d203dab.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.1b46c1bb1b51.mjs';
import { projectNameFromPath } from './shell/project-picker.c85d9c58061f.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.15f052ce6030.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.692c6857f35c.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.ff4ba96df88f.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.3cd9bcac36fa.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.49fdfe4d9f2b.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.4ed8beab02e3.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.f6f91e7d1a2e.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.db8167bd7060.mjs'), import('./control-plane/route-registry.1614c2479890.mjs'), import('./control-plane/live-domain-workspace.5a1b2a1c6c84.mjs'), import('./views/browser/browser-view.7eda2e646bf4.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.97b7f37a55e2.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.49f15db50a8e.mjs'), import('./views/onboarding/onboarding-view.a78e4a118bd3.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.44a7c882bde0.mjs'), import('./views/settings/settings-view.a06e1ff10f49.mjs')]);
