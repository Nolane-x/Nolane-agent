import { createRouter } from './core/router.e4b4e9d5f25b.mjs';
import { createUiStore } from './core/ui-store.ea774777d4a9.mjs';
import { createUiBus } from './core/ui-bus.8cfd8ee9115e.mjs';
import { createApiClient } from './core/api-client.e4c7a01904c5.mjs';
import { createLayoutStore } from './core/layout-store.7a9436fd4885.mjs';
import { createResizableRegionController } from './core/resizable-region.360cf4bfadd1.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.e3cda8b04eda.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.0dae740ccda3.mjs';
import { normalizeExperience } from './core/experience-policy.a91f73ce6bfe.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.1c70b8d719f6.mjs';
import { createViewStateBridge } from './core/view-state-bridge.fb9e865c6ffb.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.fefb70854f61.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.2f4981e48759.mjs';
import { createUpdateStateController } from './core/update-state-controller.8114e12ca7d1.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.8fd1c3def017.mjs';
import { settingsSectionFromRoute } from './core/settings-route.a54a2202ba4a.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.66f8fef2ac7d.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.4c5d55b4b249.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.8341fe8a2e9d.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.7fbc8412a1db.mjs';
import { projectNameFromPath } from './shell/project-picker.b2562f901109.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.eb46acfe57d9.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.01740612a913.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.8c87c77248bb.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.4f0379b4869f.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.7667ad3da849.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.d29523a96fd9.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.dc876855b933.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.405ed62e2fbc.mjs'), import('./views/workroom/terminal-client.ae6df4645b67.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.27a3f4408314.mjs'), import('./control-plane/route-registry.293558e38e17.mjs'), import('./control-plane/live-domain-workspace.91da3cd2e940.mjs'), import('./views/browser/browser-view.29067ddbdbf6.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.8319610b2333.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.5e0a062d1f2f.mjs'), import('./views/onboarding/onboarding-view.788bda4e6850.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.201f9a83beb6.mjs'), import('./views/settings/settings-view.d8c898c91eb7.mjs')]);
