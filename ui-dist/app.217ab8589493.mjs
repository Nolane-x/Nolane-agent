import { createRouter } from './core/router.7e304c0262b0.mjs';
import { createUiStore } from './core/ui-store.2a41167de794.mjs';
import { createUiBus } from './core/ui-bus.25e200169f02.mjs';
import { createApiClient } from './core/api-client.76b1b0f5c104.mjs';
import { createLayoutStore } from './core/layout-store.4f002208328e.mjs';
import { createResizableRegionController } from './core/resizable-region.71ec78035eaa.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.f35e2fa1076d.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.563004c0c0e3.mjs';
import { normalizeExperience } from './core/experience-policy.9f605dc8b957.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.b1ee840a2b67.mjs';
import { createViewStateBridge } from './core/view-state-bridge.d74d3c23818b.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.5acefb122c79.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.8bcf8b9dcef6.mjs';
import { createUpdateStateController } from './core/update-state-controller.e1d0e76c2562.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.13213d1f582a.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.de9df7eac2e9.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.802a4f08382c.mjs';
import { projectNameFromPath } from './shell/project-picker.edf4ec79028b.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.ccd2b283fd21.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.a33fe3849ba8.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.91ea8a55ca2a.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.74f2a7816c96.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.6ce7768328c1.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.be229fb816d9.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.e9f700d75754.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.2243b3755cb1.mjs'), import('./control-plane/route-registry.6ccde0e63432.mjs'), import('./control-plane/live-domain-workspace.e24a11b72b47.mjs'), import('./views/browser/browser-view.59da16c72751.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.ed90ca6fe0e4.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.eb2dda17c470.mjs'), import('./views/onboarding/onboarding-view.b0943ee9549c.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.3f8a88fa9fb0.mjs'), import('./views/settings/settings-view.457c0551fe77.mjs')]);
