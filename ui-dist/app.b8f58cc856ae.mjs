import { createRouter } from './core/router.40c64d6558d4.mjs';
import { createUiStore } from './core/ui-store.4e9b58314151.mjs';
import { createUiBus } from './core/ui-bus.5cfc27b46f58.mjs';
import { createApiClient } from './core/api-client.26420b309a61.mjs';
import { createLayoutStore } from './core/layout-store.1d66cc9f9a02.mjs';
import { createResizableRegionController } from './core/resizable-region.7799a9cd5a01.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.ac149a06dfdc.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.48d378120ea4.mjs';
import { normalizeExperience } from './core/experience-policy.7eb1fe65edca.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.68a3f7e19a03.mjs';
import { createViewStateBridge } from './core/view-state-bridge.e0dc86308d94.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.c273e868d97c.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.9673ee42bb84.mjs';
import { createUpdateStateController } from './core/update-state-controller.188af74c7643.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.872cf892ba75.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.c004cf1cc0a2.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.d308369dfe21.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.880d30195b3b.mjs';
import { projectNameFromPath } from './shell/project-picker.bf0a5ced1896.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.5b491414e2e9.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.de9282d0b5b2.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.8af9eb1bc697.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.186420b948dc.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.c5df11a560eb.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.d4a054795430.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.122ca980d7e6.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.17c411017406.mjs'), import('./views/workroom/terminal-client.c7271ad216bb.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.9c9855c62102.mjs'), import('./control-plane/route-registry.22926bd549f8.mjs'), import('./control-plane/live-domain-workspace.dde9add394f7.mjs'), import('./views/browser/browser-view.9e522bd286b4.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.70bb7211c583.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.9ae161eba011.mjs'), import('./views/onboarding/onboarding-view.9014934d020a.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.4bf31b645c14.mjs'), import('./views/settings/settings-view.2fe9f59b8aeb.mjs')]);
