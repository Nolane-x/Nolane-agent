import { createRouter } from './core/router.d97ec07e41f5.mjs';
import { createUiStore } from './core/ui-store.1a907a598612.mjs';
import { createUiBus } from './core/ui-bus.b6218c69c89e.mjs';
import { createApiClient } from './core/api-client.eeaa4d64d17e.mjs';
import { createLayoutStore } from './core/layout-store.4d4924d626a9.mjs';
import { createResizableRegionController } from './core/resizable-region.b9745d5b44b6.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.3392c0505e3a.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.80d357378298.mjs';
import { normalizeExperience } from './core/experience-policy.8ba55c6e8cbc.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.aeea210da27b.mjs';
import { createViewStateBridge } from './core/view-state-bridge.0ffdbded180c.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.628006c21be1.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.6eef304434a0.mjs';
import { createUpdateStateController } from './core/update-state-controller.bd1b4ef7fa8e.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.2a56f0df2eee.mjs';
import { settingsSectionFromRoute } from './core/settings-route.667425861172.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.fc7eeb6dd9c4.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.608fe74fc08d.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.e1a6c74eb7f3.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.6c6792a57010.mjs';
import { projectNameFromPath } from './shell/project-picker.055bc9cd8657.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.734be354569b.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.0ef290bff110.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.770ed9238ae3.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.b0b47f1bb5b8.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.3bd7e6568d3d.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.86571393bb6d.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.3e759a04182f.mjs'), import('./views/workroom/terminal-client.590417a945cd.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.0dbfe09bb415.mjs'), import('./control-plane/route-registry.80c6b3adf6dd.mjs'), import('./control-plane/live-domain-workspace.618d7148a421.mjs'), import('./views/browser/browser-view.a9e306a573c5.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.bbfdb6cea8ca.mjs'), import('./views/settings/settings-view.b7c78032f6f5.mjs')]);
