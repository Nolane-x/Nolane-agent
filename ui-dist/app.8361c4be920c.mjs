import { createRouter } from './core/router.e1718d1b7964.mjs';
import { createUiStore } from './core/ui-store.6bb958480e91.mjs';
import { createUiBus } from './core/ui-bus.857a7ca49f71.mjs';
import { createApiClient } from './core/api-client.067de6c12896.mjs';
import { createLayoutStore } from './core/layout-store.73e16eddd632.mjs';
import { createResizableRegionController } from './core/resizable-region.22b15ad78d74.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.86241134b922.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.e14786abb80c.mjs';
import { normalizeExperience } from './core/experience-policy.2f441baa10e0.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.59901503f93f.mjs';
import { createViewStateBridge } from './core/view-state-bridge.e6a0a2be3bdb.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.30411ead10be.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.921f8317a813.mjs';
import { createUpdateStateController } from './core/update-state-controller.6694697a012e.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.49d8e13b8e21.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.5454f878968b.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.bfe2c2e3a715.mjs';
import { projectNameFromPath } from './shell/project-picker.fe176478e7e2.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.b8050a8b6933.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.50835f46858a.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.9189dd38a163.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.6bb0d98eddda.mjs');
...
router.register({ id: 'projects', pattern: /^\/projects(?:\?.*)?$/, title: 'Projects', load: async () => {
  const { createProjectsController, renderProjectsView } = await import('./views/projects/project-view.9f5f554ea5b1.mjs');const controller=createProjectsController({api,language:cachedPreferences.language});await controller.load();let root=null;const view={render:()=>renderProjectsView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-project-search]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{if(e.target.closest('[data-project-action="add"]')){window.dispatchEvent(new CustomEvent('nolane:project-create-requested',{detail:{source:'projects-view'}}));}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view;
} });

router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.ddc818d32216.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.6df5e4f9b2cc.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.1c0a6a30ae7d.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.d75217af10d2.mjs'), import('./control-plane/route-registry.c3a89651e115.mjs'), import('./control-plane/live-domain-workspace.700491caee37.mjs'), import('./views/browser/browser-view.23e7921eb38f.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.1d1052da1fa9.mjs'), import('./views/settings/settings-view.7304b1af4de4.mjs')]);
