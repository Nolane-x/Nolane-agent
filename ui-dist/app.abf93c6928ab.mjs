import { createRouter } from './core/router.e34710249bd4.mjs';
import { createUiStore } from './core/ui-store.d9af8a2c618d.mjs';
import { createUiBus } from './core/ui-bus.50f993a869e8.mjs';
import { createApiClient } from './core/api-client.ff248664ecec.mjs';
import { createLayoutStore } from './core/layout-store.368bfae6d1cf.mjs';
import { createResizableRegionController } from './core/resizable-region.313920c483d0.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.d0004df2f64a.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.57da7931b96c.mjs';
import { normalizeExperience } from './core/experience-policy.f3b374e38074.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.e1050d34d36d.mjs';
import { createViewStateBridge } from './core/view-state-bridge.7a318a4a3e44.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.4677901fcad9.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.54fcfcd162c4.mjs';
import { createUpdateStateController } from './core/update-state-controller.55ff69ac7a46.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.bb358496d5a5.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.b0ea52518546.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.1dd9afb6fcee.mjs';
import { projectNameFromPath } from './shell/project-picker.af5775f9d393.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.8ab91700b3b8.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.f1ef1b9c5a20.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.3f7cbbb385ab.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.1b6284e8dd44.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.fac93c98fadc.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.50e5ea221227.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.505aae30d0f9.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.de1408d6e3d8.mjs'), import('./control-plane/route-registry.2b3e174d6bcf.mjs'), import('./control-plane/live-domain-workspace.7ba2db5d3637.mjs'), import('./views/browser/browser-view.af93795628db.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.c7452cab4570.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.32f67c01ccd2.mjs'), import('./views/onboarding/onboarding-view.41aaa13ea9ab.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.44d14ea31958.mjs'), import('./views/settings/settings-view.b90e4fd70156.mjs')]);
