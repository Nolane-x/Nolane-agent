import { createRouter } from './core/router.c42dc14fe61e.mjs';
import { createUiStore } from './core/ui-store.a68fb66107fd.mjs';
import { createUiBus } from './core/ui-bus.e369858c5830.mjs';
import { createApiClient } from './core/api-client.7b752f6fd1db.mjs';
import { createLayoutStore } from './core/layout-store.e3bd16146b2f.mjs';
import { createResizableRegionController } from './core/resizable-region.68239e3322a1.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.2a1fd42f9293.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.c2d447732247.mjs';
import { normalizeExperience } from './core/experience-policy.ca06a056e303.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.cc75593f930a.mjs';
import { createViewStateBridge } from './core/view-state-bridge.88ed9ff99b96.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.4752990678f3.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.3ca3a95218c6.mjs';
import { createUpdateStateController } from './core/update-state-controller.f0ec84e49762.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.f3e87460a69b.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.a95ea557990a.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.0be8fa00d8a0.mjs';
import { projectNameFromPath } from './shell/project-picker.d9d784939e6b.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.f47ef9ac7420.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.d93bca600c8e.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.ee9d40a6b8b3.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.0d27a2291fdd.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.f5e002baf7b4.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.29a2aed1634d.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.05ca9444ae4b.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.bd8068beccc7.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.f7f70cc00f64.mjs'), import('./control-plane/route-registry.4168902ece4c.mjs'), import('./control-plane/live-domain-workspace.29f91ff15210.mjs'), import('./views/browser/browser-view.750f95680fab.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.e4e74523aecb.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.9ca110bdebe1.mjs'), import('./views/onboarding/onboarding-view.30d13973f765.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.88210ab6d9e8.mjs'), import('./views/settings/settings-view.8cdf0ec716c0.mjs')]);
