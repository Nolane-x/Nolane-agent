import { createRouter } from './core/router.b0fc0407fbbd.mjs';
import { createUiStore } from './core/ui-store.99157d506e6c.mjs';
import { createUiBus } from './core/ui-bus.7951fe79edb4.mjs';
import { createApiClient } from './core/api-client.2186d664fb44.mjs';
import { createLayoutStore } from './core/layout-store.30da41e7694c.mjs';
import { createResizableRegionController } from './core/resizable-region.34a7b5f0df0e.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.6694f1d179c8.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.a685dfc53811.mjs';
import { normalizeExperience } from './core/experience-policy.9e518a5ee296.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.fd1aee3357ff.mjs';
import { createViewStateBridge } from './core/view-state-bridge.ab3ceca37358.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.a52c2c213dc1.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.77b39954cef7.mjs';
import { createUpdateStateController } from './core/update-state-controller.cba5ea94e0f7.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.93c11014fada.mjs';
import { settingsSectionFromRoute } from './core/settings-route.a8082d8183db.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.2de402cf614f.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.282d05816fb8.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.baa430d119b5.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.024276a256b3.mjs';
import { projectNameFromPath } from './shell/project-picker.c3111ec21068.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.8054d822fa0e.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.0aaa06fe0043.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.a9721dfa3699.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.37150cdde7cc.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.a2a81b01c714.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.4fea50151a6d.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.62bd80e5b5b4.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.d7546bf46dc4.mjs'), import('./views/workroom/terminal-client.3458386c058b.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.b61e2bab1255.mjs'), import('./control-plane/route-registry.30f5351186bc.mjs'), import('./control-plane/live-domain-workspace.083e48f302db.mjs'), import('./views/browser/browser-view.cfcdb6286d19.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.b3366a84a771.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.59a16595674e.mjs'), import('./views/onboarding/onboarding-view.f52b341a6188.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.151b13305502.mjs'), import('./views/settings/settings-view.e4aa5c480ef1.mjs')]);
