import { createRouter } from './core/router.151ab1881816.mjs';
import { createUiStore } from './core/ui-store.e833dd4d8c64.mjs';
import { createUiBus } from './core/ui-bus.c574176e9373.mjs';
import { createApiClient } from './core/api-client.e9b144bd6e16.mjs';
import { createLayoutStore } from './core/layout-store.b36c3e8a0ec6.mjs';
import { createResizableRegionController } from './core/resizable-region.199224693b64.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.3b62f5189930.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.fda4908891f1.mjs';
import { normalizeExperience } from './core/experience-policy.590cd60f7932.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.34ce89a41b66.mjs';
import { createViewStateBridge } from './core/view-state-bridge.0ba795094851.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.f6fe1f3e9b1c.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.6ccfad7e66ae.mjs';
import { createUpdateStateController } from './core/update-state-controller.c217b1e65738.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.adb807fb4873.mjs';
import { settingsSectionFromRoute } from './core/settings-route.43732836bb35.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.fe5a118843a5.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.87e2cbc705d0.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.f5b70f43b4b3.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.8ac00e9af7da.mjs';
import { projectNameFromPath } from './shell/project-picker.1daf27e59cfd.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.0ba6e26613c5.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.592f8d98876f.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.58d5f252e114.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.0ad46033994e.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.7345ed95efe7.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.94978ff7c4d5.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.65bd492847d2.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.953156ce05e0.mjs'), import('./views/workroom/terminal-client.439ff867bd5a.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.875b922abfab.mjs'), import('./control-plane/route-registry.c1b69412ded7.mjs'), import('./control-plane/live-domain-workspace.94eed2e2fc75.mjs'), import('./views/browser/browser-view.71e5a5b7a1f5.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.2ff1697850f4.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.ea8927970de0.mjs'), import('./views/onboarding/onboarding-view.cad39b047355.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.c3a2ca751e3f.mjs'), import('./views/settings/settings-view.631fdd76c264.mjs')]);
