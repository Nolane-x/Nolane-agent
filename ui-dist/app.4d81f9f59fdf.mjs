import { createRouter } from './core/router.4c9575e82b29.mjs';
import { createUiStore } from './core/ui-store.e4e915af29fb.mjs';
import { createUiBus } from './core/ui-bus.a18dc39759da.mjs';
import { createApiClient } from './core/api-client.a4d434a3f235.mjs';
import { createLayoutStore } from './core/layout-store.efea618d0b8f.mjs';
import { createResizableRegionController } from './core/resizable-region.8c73fbfd41d3.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.99430c1b651f.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.4b29826e4e5c.mjs';
import { normalizeExperience } from './core/experience-policy.2357fcd30011.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.821f1d7f61c2.mjs';
import { createViewStateBridge } from './core/view-state-bridge.a57cedf8e66a.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.94d13e45c2de.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.a31983b6513d.mjs';
import { createUpdateStateController } from './core/update-state-controller.da18fbdc235c.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.844779514690.mjs';
import { settingsSectionFromRoute } from './core/settings-route.27339c49ade4.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.a8a1a0f63c3a.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.915bf1e354a4.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.9b14cc793300.mjs';
import { closeComposerPickers, isComposerPickerInteraction } from './components/composer-picker.c2897630cc06.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.63339d6f200a.mjs';
import { projectNameFromPath } from './shell/project-picker.b1e2f17e9b96.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.e172099278c8.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.245b1c518fca.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.128491e145b1.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.a24b28dde271.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.91000924fa5b.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.4bf70eb7cab5.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.9dbdf9ee43c5.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.37696c2e13b1.mjs'), import('./views/workroom/terminal-client.548a2e57e109.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.d9d89123dab3.mjs'), import('./control-plane/route-registry.486ba766bbde.mjs'), import('./control-plane/live-domain-workspace.7e5a625b563d.mjs'), import('./views/browser/browser-view.5918512a8a7a.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.7d2f3a045c0f.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.bdba3f962b98.mjs'), import('./views/onboarding/onboarding-view.8be68279152f.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.e8012e0de357.mjs'), import('./views/settings/settings-view.6e4d1dd371ea.mjs')]);
