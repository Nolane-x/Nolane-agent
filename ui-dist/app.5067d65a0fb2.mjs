import { createRouter } from './core/router.4de6216e3fb3.mjs';
import { createUiStore } from './core/ui-store.620c61a0042e.mjs';
import { createUiBus } from './core/ui-bus.3d196ea4c292.mjs';
import { createApiClient } from './core/api-client.0c99f78cc56a.mjs';
import { createLayoutStore } from './core/layout-store.b831bd598214.mjs';
import { createResizableRegionController } from './core/resizable-region.10bad18a42c4.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.a46dfcc7a12d.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.1de5ebd646ff.mjs';
import { normalizeExperience } from './core/experience-policy.3493026a7e95.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.45bfc47ed5df.mjs';
import { createViewStateBridge } from './core/view-state-bridge.3840b58cf584.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.2b6cab9bf155.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.9f8ba315f0f1.mjs';
import { createUpdateStateController } from './core/update-state-controller.6df7adcf6e79.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.a54643dd79f7.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.f82121b6dabd.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.4c74ec071356.mjs';
import { projectNameFromPath } from './shell/project-picker.d8343a4b7e77.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.c88db85b9b61.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.5e0fbe90b1f4.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.ef9a89d6ad0d.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.c30491ac9763.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.2708ba2b1093.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.28e6ddebd335.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.7ae9d2dfaf5a.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.da0291d86841.mjs'), import('./control-plane/route-registry.4664a10f665b.mjs'), import('./control-plane/live-domain-workspace.a8ee32299c65.mjs'), import('./views/browser/browser-view.cfb606376345.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.87ad9d29cc49.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.5acf9e9e63fb.mjs'), import('./views/onboarding/onboarding-view.a93afa2581e8.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.52d7acb7e0a8.mjs'), import('./views/settings/settings-view.62fdd8cbf572.mjs')]);
