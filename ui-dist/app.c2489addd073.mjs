import { createRouter } from './core/router.b2c1b91c4588.mjs';
import { createUiStore } from './core/ui-store.5a3a70e4d71e.mjs';
import { createUiBus } from './core/ui-bus.63e338d7674b.mjs';
import { createApiClient } from './core/api-client.017cc8e1505a.mjs';
import { createLayoutStore } from './core/layout-store.2faf3119c63a.mjs';
import { createResizableRegionController } from './core/resizable-region.3768e623e588.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.b91acc5e2d06.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.1f61d7a6f3e2.mjs';
import { normalizeExperience } from './core/experience-policy.2fe8a6ec16a2.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.0c7531678846.mjs';
import { createViewStateBridge } from './core/view-state-bridge.8dfa3285bebe.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.a8383591f8d0.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.13075d946e61.mjs';
import { createUpdateStateController } from './core/update-state-controller.061380a3af7f.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.855c3db8e493.mjs';
import { settingsSectionFromRoute } from './core/settings-route.89c63e511515.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.20cfcd5a28b6.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.533963085983.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.3769270052df.mjs';
import { projectNameFromPath } from './shell/project-picker.d493b2447295.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.8c0278b888f4.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.b8cf61488d13.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.57665304af81.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.86a6d7a543db.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.8c05dd9d43d6.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.ad4c9d1c7867.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.d4abbfe5f6e8.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.bbea11926433.mjs'), import('./views/workroom/terminal-client.26defdc63a42.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.86735ffbf84f.mjs'), import('./control-plane/route-registry.b09a435538a6.mjs'), import('./control-plane/live-domain-workspace.80751cc3f4ad.mjs'), import('./views/browser/browser-view.ccdc106b5a5a.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.fa8a0bf389ad.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.aade229279dd.mjs'), import('./views/onboarding/onboarding-view.82925f8ab369.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.ca3ac31acac4.mjs'), import('./views/settings/settings-view.5a1f4c6291cf.mjs')]);
