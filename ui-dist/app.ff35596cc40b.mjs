import { createRouter } from './core/router.d872991fa6ce.mjs';
import { createUiStore } from './core/ui-store.9ec35108bd2c.mjs';
import { createUiBus } from './core/ui-bus.b04887a4d88f.mjs';
import { createApiClient } from './core/api-client.c390f23c23ec.mjs';
import { createLayoutStore } from './core/layout-store.8a9c871f0e11.mjs';
import { createResizableRegionController } from './core/resizable-region.55f59172e3e4.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.c8306e246482.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.296c0217cbe8.mjs';
import { normalizeExperience } from './core/experience-policy.afb0947b7525.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.27403f37d773.mjs';
import { createViewStateBridge } from './core/view-state-bridge.dadb36fb5a14.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.31207decf6f6.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.a45148614311.mjs';
import { createUpdateStateController } from './core/update-state-controller.2172430354ae.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.7267f9749dd9.mjs';
import { settingsSectionFromRoute } from './core/settings-route.cd47d313a002.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.e17ebf626d6b.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.51e49c045277.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.f095929d361e.mjs';
import { projectNameFromPath } from './shell/project-picker.117602f4be71.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.9419abe311a2.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.2d8052478d1d.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.7acefb0e79c2.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.de48707199ba.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.53def1704589.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.19e6ab3e579e.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.9de6943729e8.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.0e1729e0ddea.mjs'), import('./views/workroom/terminal-client.664641d7f29d.mjs')]);
  ...
  const view = {
    ...
    mount(node) {
      ...
      const click = async (event) => {
        const file = event.target.closest?.('[data-workroom-file]'); const directory = event.target.closest?.('[data-workroom-directory]'); const tab = event.target.closest?.('[data-workroom-tab]'); const agentTab = event.target.closest?.('[data-workroom-agent-tab]'); const action = event.target.closest?.('[data-workroom-action]');
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.33665d13a663.mjs'), import('./control-plane/route-registry.88f3d4650b6b.mjs'), import('./control-plane/live-domain-workspace.a60296fb3d5d.mjs'), import('./views/browser/browser-view.f7e9283fad0e.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.ceb7a6544836.mjs'), import('./views/settings/settings-view.c455e2861d1b.mjs')]);
