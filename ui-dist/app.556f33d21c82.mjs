import { createRouter } from './core/router.1caba7001a31.mjs';
import { createUiStore } from './core/ui-store.49f499810a98.mjs';
import { createUiBus } from './core/ui-bus.4e7170a0af97.mjs';
import { createApiClient } from './core/api-client.412eb4d8fdaa.mjs';
import { createLayoutStore } from './core/layout-store.19dd7b58592b.mjs';
import { createResizableRegionController } from './core/resizable-region.22f05c96376d.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.48ed804a75ab.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.5dd4a64d53e8.mjs';
import { normalizeExperience } from './core/experience-policy.2ff9e299268e.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.17fccb2425d4.mjs';
import { createViewStateBridge } from './core/view-state-bridge.67f4927d70db.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.e974aabd5de2.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.9628cd95818f.mjs';
import { createUpdateStateController } from './core/update-state-controller.5082adbac5f4.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.9974cfcee0eb.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.d64a870bbf8f.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.fd812ccdf038.mjs';
import { projectNameFromPath } from './shell/project-picker.88b541baab7b.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.9bb094d5c4d3.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.d82ce8aa577b.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.4a7fcfd8f122.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.7ab73b391f63.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.ffedbbed9dcd.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.e75d0f4dd991.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.396cbea53aa2.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.5d947da94b51.mjs'), import('./control-plane/route-registry.d03d11fd3da1.mjs'), import('./control-plane/live-domain-workspace.7585c510417d.mjs'), import('./views/browser/browser-view.7caebc43b136.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.85b25f0a2426.mjs'), import('./views/settings/settings-view.57fa306aabb1.mjs')]);
