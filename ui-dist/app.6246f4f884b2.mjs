import { createRouter } from './core/router.531d3acfb35f.mjs';
import { createUiStore } from './core/ui-store.12c740a0b31d.mjs';
import { createUiBus } from './core/ui-bus.5456851e0f62.mjs';
import { createApiClient } from './core/api-client.9f75f6e7617d.mjs';
import { createLayoutStore } from './core/layout-store.0875f57b32d0.mjs';
import { createResizableRegionController } from './core/resizable-region.6f46183ec965.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.c8b26afaebab.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.79b3ef4b927b.mjs';
import { normalizeExperience } from './core/experience-policy.b2e2936172d5.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.36c2613a344f.mjs';
import { createViewStateBridge } from './core/view-state-bridge.f6361970e167.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.0cdf4bb1ee1a.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.99158d1bbefc.mjs';
import { createUpdateStateController } from './core/update-state-controller.ab4942b3ee42.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.35ffb8b05c53.mjs';
import { settingsSectionFromRoute } from './core/settings-route.f1bcc47745b3.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.d74b52c9c82c.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.765965c9330f.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.e45374bc3827.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.9dffdb77bc8f.mjs';
import { projectNameFromPath } from './shell/project-picker.259c7cf7ba88.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.e4775425e7de.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.729e4482aa07.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.7338a1535241.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.991e3194b63b.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.e7c31c0d0f96.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.589b6f833803.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.167de2dc106c.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.f6da7bcf7999.mjs'), import('./views/workroom/terminal-client.5405eac802de.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.d70cb6fdcb0a.mjs'), import('./control-plane/route-registry.7e76ec2d7aef.mjs'), import('./control-plane/live-domain-workspace.96746e0c28c1.mjs'), import('./views/browser/browser-view.ff6f43b10d4f.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.6fa4f8c92284.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.733220c75c4c.mjs'), import('./views/onboarding/onboarding-view.6d5c349cbf3c.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.da0a747ed210.mjs'), import('./views/settings/settings-view.07674b09d4d4.mjs')]);
