import { createRouter } from './core/router.c9e23f1d6bb4.mjs';
import { createUiStore } from './core/ui-store.bec3ba348fc2.mjs';
import { createUiBus } from './core/ui-bus.e54a1a16ce98.mjs';
import { createApiClient } from './core/api-client.20ba500eba6b.mjs';
import { createLayoutStore } from './core/layout-store.87e0093d297c.mjs';
import { createResizableRegionController } from './core/resizable-region.b5b9038dca7a.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.48c7ae59521a.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.0338e874b130.mjs';
import { normalizeExperience } from './core/experience-policy.ec912c8a27cf.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.254d2edd0a59.mjs';
import { createViewStateBridge } from './core/view-state-bridge.2905a914e9a6.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.4f3cf31633dc.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.c6a8d083193a.mjs';
import { createUpdateStateController } from './core/update-state-controller.806ec23e603c.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.54fe85bb5867.mjs';
import { settingsSectionFromRoute } from './core/settings-route.4dc06e913a65.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.8bdd7d51bb15.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.86894692e939.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.5444ccf30861.mjs';
import { closeComposerPickers, isComposerPickerInteraction } from './components/composer-picker.edfcf44cedb9.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.7b90ccf1faa4.mjs';
import { projectNameFromPath } from './shell/project-picker.44de15213bc3.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.85e10bc003d0.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.30af14b96496.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.fd653677bd4a.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.6ebc99fa7b91.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.10ab5246dc26.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.a883ff5c9cde.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.43b13486cebe.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.639dec8f1e63.mjs'), import('./views/workroom/terminal-client.515e6a0cf612.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.7ff5573967db.mjs'), import('./control-plane/route-registry.64bdc35e7cc6.mjs'), import('./control-plane/live-domain-workspace.89c9325bd302.mjs'), import('./views/browser/browser-view.af4ba8099334.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.ed3098c0be34.mjs'), import('./views/settings/settings-view.fc2142e5008e.mjs')]);
