import { createRouter } from './core/router.d35ac2043a31.mjs';
import { createUiStore } from './core/ui-store.652badcba79b.mjs';
import { createUiBus } from './core/ui-bus.79b870cb567a.mjs';
import { createApiClient } from './core/api-client.da36931b17a5.mjs';
import { createLayoutStore } from './core/layout-store.33d677aeae2b.mjs';
import { createResizableRegionController } from './core/resizable-region.5594ecc1635f.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.634e8e01824e.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.85f304f2533c.mjs';
import { normalizeExperience } from './core/experience-policy.073e8ef54933.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.5528bf9e228a.mjs';
import { createViewStateBridge } from './core/view-state-bridge.627f6ee65af7.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.f3acc2211f4f.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.7f7c4f5472e2.mjs';
import { createUpdateStateController } from './core/update-state-controller.21a5456ac362.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.5bab690ed414.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.0ed5c951d9f8.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.c1251a045624.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.cad457c139b0.mjs';
import { projectNameFromPath } from './shell/project-picker.4dc0ef74f02d.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.753c2719f78b.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.57027675e9a7.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.14fc84244b8d.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.6aa5c07f1497.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.bb90c96f377c.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.de969941f1a0.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.1713aca84afe.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.7e80c78268fc.mjs'), import('./views/workroom/terminal-client.1cc7a8525e78.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.c2721226e870.mjs'), import('./control-plane/route-registry.913d837c8c35.mjs'), import('./control-plane/live-domain-workspace.7cf7f8de1fb0.mjs'), import('./views/browser/browser-view.9a29c82aefaf.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.78c47f611582.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.7656805789f2.mjs'), import('./views/onboarding/onboarding-view.e46716524d4f.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.7cec0db22856.mjs'), import('./views/settings/settings-view.4883137968fb.mjs')]);
