import { createRouter } from './core/router.f0ec78030e50.mjs';
import { createUiStore } from './core/ui-store.4dae14fd4895.mjs';
import { createUiBus } from './core/ui-bus.3bf86c3b8ee1.mjs';
import { createApiClient } from './core/api-client.78e5f6bdfe1f.mjs';
import { createLayoutStore } from './core/layout-store.b02f837f78f2.mjs';
import { createResizableRegionController } from './core/resizable-region.33c1b1ec055e.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.44bf19d0af5f.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.c3d847d0b093.mjs';
import { normalizeExperience } from './core/experience-policy.19e05596d52b.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.212a6bb52b16.mjs';
import { createViewStateBridge } from './core/view-state-bridge.a97d30dec046.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.a7ca2b19b74f.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.ddead9c8e759.mjs';
import { createUpdateStateController } from './core/update-state-controller.f489e5e24037.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.e384016be130.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.ce201b7ffb7f.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.5da8dc6e8bb8.mjs';
import { projectNameFromPath } from './shell/project-picker.9dfb1648a0f3.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.7eb1ca84940b.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.6fe0dbd2a92c.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.4f892b3775b2.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.3b0ff231cb44.mjs');
...
router.register({ id: 'projects', pattern: /^\/projects(?:\?.*)?$/, title: 'Projects', load: async () => {
  const { createProjectsController, renderProjectsView } = await import('./views/projects/project-view.61aaf47e49e3.mjs');const controller=createProjectsController({api,language:cachedPreferences.language});await controller.load();let root=null;const view={render:()=>renderProjectsView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-project-search]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{if(e.target.closest('[data-project-action="add"]')){window.dispatchEvent(new CustomEvent('nolane:project-create-requested',{detail:{source:'projects-view'}}));}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view;
} });

router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.54550670f621.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.2b91a675efad.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.be86732715bd.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.8f26a8d2c7dc.mjs'), import('./control-plane/route-registry.faded249cda4.mjs'), import('./control-plane/live-domain-workspace.48c534c44647.mjs'), import('./views/browser/browser-view.a9771eafe82a.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.295302d5317a.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.67a271091e91.mjs'), import('./views/onboarding/onboarding-view.6a425babdc3d.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.ad21e1bbfe75.mjs'), import('./views/settings/settings-view.a822c9ea2d85.mjs')]);
