import { createRouter } from './core/router.6c6dc4bffbc6.mjs';
import { createUiStore } from './core/ui-store.9fce6040dcfb.mjs';
import { createUiBus } from './core/ui-bus.b9256528a988.mjs';
import { createApiClient } from './core/api-client.ae3d13b8d7b2.mjs';
import { createLayoutStore } from './core/layout-store.a9357e24dccb.mjs';
import { createResizableRegionController } from './core/resizable-region.df0ca2ca950d.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.981675637c01.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.ad848b9f6a13.mjs';
import { normalizeExperience } from './core/experience-policy.bc99dbb80d46.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.9f16860ef116.mjs';
import { createViewStateBridge } from './core/view-state-bridge.3cce479fcdfc.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.e53b90a799fb.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.86e7e1eae2f9.mjs';
import { createUpdateStateController } from './core/update-state-controller.42e2aed546e2.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.0ad34ec54f44.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.437a4b429c5b.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.edda0609c88e.mjs';
import { projectNameFromPath } from './shell/project-picker.15d18f9c99f5.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.ae338f83ff6a.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.a0e8e1968690.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.93d1dd61ee3d.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.651d167958af.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.1a0ef7f4f68c.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.609e1490d969.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.3fcb263cc314.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.2a71a1716977.mjs'), import('./control-plane/route-registry.e792641b65f3.mjs'), import('./control-plane/live-domain-workspace.4e9ca928672d.mjs'), import('./views/browser/browser-view.973588786bb0.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.52085c7b21e4.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.088a2b846415.mjs'), import('./views/onboarding/onboarding-view.b383091e596f.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.848f89fdc8eb.mjs'), import('./views/settings/settings-view.076541b444ae.mjs')]);
