import { createRouter } from './core/router.4131f3d721f1.mjs';
import { createUiStore } from './core/ui-store.f743cadbe60c.mjs';
import { createUiBus } from './core/ui-bus.4d3855607a77.mjs';
import { createApiClient } from './core/api-client.894e48704361.mjs';
import { createLayoutStore } from './core/layout-store.ec988e03efe8.mjs';
import { createResizableRegionController } from './core/resizable-region.a172f6b9f89b.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.eeb4528f3386.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.99a69e6ca2c2.mjs';
import { normalizeExperience } from './core/experience-policy.66b1ed2b810c.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.4804d40312d8.mjs';
import { createViewStateBridge } from './core/view-state-bridge.f09b62c8145c.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.73e1401bff14.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.065f907eaaf6.mjs';
import { createUpdateStateController } from './core/update-state-controller.9850b80a48ba.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.79e1c8bcc196.mjs';
import { settingsSectionFromRoute } from './core/settings-route.3428c6b33de7.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.53fa89677911.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.651d0f0f1383.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.f74e9bcd3590.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.c02558ecc469.mjs';
import { projectNameFromPath } from './shell/project-picker.69d5be4989da.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.4589d7f31974.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.c8de22f8feb0.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.d43682e3e186.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.520b1f434c4f.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.86f9663119cc.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.e49dc7edec36.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.2a02538a636e.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.1fca81c4428a.mjs'), import('./views/workroom/terminal-client.7f002ec35f9f.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.3d4251ffbed4.mjs'), import('./control-plane/route-registry.30816aa06aed.mjs'), import('./control-plane/live-domain-workspace.9fa558ab1701.mjs'), import('./views/browser/browser-view.4c8409367f7f.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.1cea3b889607.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.63e47e4d8264.mjs'), import('./views/onboarding/onboarding-view.275f754d08fa.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.90e56194fa8b.mjs'), import('./views/settings/settings-view.d9a533dcf60b.mjs')]);
