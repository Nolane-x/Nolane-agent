import { createRouter } from './core/router.7644df6a5e5a.mjs';
import { createUiStore } from './core/ui-store.9e2df4fecabe.mjs';
import { createUiBus } from './core/ui-bus.5d67325f6b07.mjs';
import { createApiClient } from './core/api-client.48f9b8f100c9.mjs';
import { createLayoutStore } from './core/layout-store.f77936b5984e.mjs';
import { createResizableRegionController } from './core/resizable-region.85bd36ede8a1.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.d3707af6fa0b.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.7f93b8e2edaa.mjs';
import { normalizeExperience } from './core/experience-policy.ec82bc239034.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.22030f724807.mjs';
import { createViewStateBridge } from './core/view-state-bridge.cf0c6bb97f18.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.3d9716b10dd4.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.a6f794630181.mjs';
import { createUpdateStateController } from './core/update-state-controller.a337bedd4f22.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.9ee8f537591c.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.4de327a34d59.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.1453357c1141.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.c4de85de2d4a.mjs';
import { projectNameFromPath } from './shell/project-picker.a93912b599a1.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.fdac9fecb828.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.ca0ec696cbd0.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.bac98975bbe0.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.efb16dcaf9a8.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.61f5867bb076.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.e3b3830dbff4.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.f407727c8181.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.0b4ede34d9f2.mjs'), import('./views/workroom/terminal-client.0a00cd778c44.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.bb7258ba4717.mjs'), import('./control-plane/route-registry.dbcce41e1717.mjs'), import('./control-plane/live-domain-workspace.eed10c0def12.mjs'), import('./views/browser/browser-view.d2e29e8b16ba.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.a2a3b8561169.mjs'), import('./views/settings/settings-view.2bedf1f609f4.mjs')]);
