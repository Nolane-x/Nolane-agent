import { createRouter } from './core/router.17a193036c45.mjs';
import { createUiStore } from './core/ui-store.2bbf40f64812.mjs';
import { createUiBus } from './core/ui-bus.7a9cec719ec2.mjs';
import { createApiClient } from './core/api-client.080458eb8e76.mjs';
import { createLayoutStore } from './core/layout-store.74d4958cd719.mjs';
import { createResizableRegionController } from './core/resizable-region.71b2c67e6b73.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.f3dd288b7f26.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.8e299de6d778.mjs';
import { normalizeExperience } from './core/experience-policy.dad3490879da.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.62e82f4263b3.mjs';
import { createViewStateBridge } from './core/view-state-bridge.46cba17b43e4.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.3380b2e3708b.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.e274a64ba36b.mjs';
import { createUpdateStateController } from './core/update-state-controller.3d6150e6c6c4.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.9f9174ce0661.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.769347323372.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.733947a12b6b.mjs';
import { projectNameFromPath } from './shell/project-picker.be5fcb858552.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.dac955291ebf.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.571da5e0a62a.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.32bd35dd551c.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.8c6413551b77.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.29889ace9732.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.58c32ab7d689.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.317555204b64.mjs'), import('./control-plane/route-registry.6ff1ae2ea1a5.mjs'), import('./control-plane/live-domain-workspace.13ce28ed75f3.mjs'), import('./views/browser/browser-view.52e9af7c8534.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.f1e2bfb0ecb6.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.0a45db69e83c.mjs'), import('./views/onboarding/onboarding-view.210c808171c1.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.7b6a6eb69730.mjs'), import('./views/settings/settings-view.c55d0d6dba08.mjs')]);
