import { createRouter } from './core/router.69037ef70710.mjs';
import { createUiStore } from './core/ui-store.2c06ca8c31e9.mjs';
import { createUiBus } from './core/ui-bus.c3b420ec5488.mjs';
import { createApiClient } from './core/api-client.a4482adf8488.mjs';
import { createLayoutStore } from './core/layout-store.8a90087d0d22.mjs';
import { createResizableRegionController } from './core/resizable-region.7608e5cd30a3.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.bf6a4b295c5c.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.e1730849f02f.mjs';
import { normalizeExperience } from './core/experience-policy.eaa3fcde7cad.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.6bf026035c04.mjs';
import { createViewStateBridge } from './core/view-state-bridge.71bf886f6c7b.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.7d1cffc5c476.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.a684d08233f9.mjs';
import { createUpdateStateController } from './core/update-state-controller.6988879699ee.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.9d2c566966a0.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.9679dcf38cdb.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.f442729de070.mjs';
import { projectNameFromPath } from './shell/project-picker.44587175fb56.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.218801976b0e.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.2fe37389be85.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.e047e437af09.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.eff7928f2f19.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.b926121b307a.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.61eace554267.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.4767fb33eefa.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.b6af09c8efca.mjs'), import('./views/workroom/terminal-client.c9f68d735069.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.bc5492ebd606.mjs'), import('./control-plane/route-registry.7a4b4b02384c.mjs'), import('./control-plane/live-domain-workspace.dfe60af8af70.mjs'), import('./views/browser/browser-view.6b8dfdbb92eb.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.1c5b3fbd4249.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.166cf28dd923.mjs'), import('./views/onboarding/onboarding-view.1e56e971ce61.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.38612d276611.mjs'), import('./views/settings/settings-view.e0343c76d933.mjs')]);
