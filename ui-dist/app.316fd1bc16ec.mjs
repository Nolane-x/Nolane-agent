import { createRouter } from './core/router.71752f91dc4d.mjs';
import { createUiStore } from './core/ui-store.9835c596760a.mjs';
import { createUiBus } from './core/ui-bus.ac3db1d69532.mjs';
import { createApiClient } from './core/api-client.b190f9672a7e.mjs';
import { createLayoutStore } from './core/layout-store.fb43ba56bcda.mjs';
import { createResizableRegionController } from './core/resizable-region.b9177526ed51.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.3b63c3629f2d.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.1c96998f1e60.mjs';
import { normalizeExperience } from './core/experience-policy.3f414038a98b.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.593f4b091c0b.mjs';
import { createViewStateBridge } from './core/view-state-bridge.2ca0164d2aca.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.34f3c98a6d03.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.f14aadb905ca.mjs';
import { createUpdateStateController } from './core/update-state-controller.068f38b8381d.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.381daa02f41d.mjs';
import { settingsSectionFromRoute } from './core/settings-route.ac7ec5544160.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.d0cc2db400f3.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.7928a174d8cf.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.6444a6feab84.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.87ac6fe87ac8.mjs';
import { projectNameFromPath } from './shell/project-picker.da294d75694a.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.70e31f51d41d.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.2aa4c9cccc14.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.7b74a0a52af1.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.22534f5ecdb3.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.498aa8ca52bc.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.b3232670dbc9.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.d18a69942090.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.ebddf3260285.mjs'), import('./views/workroom/terminal-client.21e983a0af61.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.89d745194458.mjs'), import('./control-plane/route-registry.c1d38a37eaa3.mjs'), import('./control-plane/live-domain-workspace.6ffaae5e3c49.mjs'), import('./views/browser/browser-view.34817314817f.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.8a18f500a084.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.9c97459dcaa4.mjs'), import('./views/onboarding/onboarding-view.e7ab09abb2f6.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.5bb5ffd2f946.mjs'), import('./views/settings/settings-view.489ea32f4aea.mjs')]);
