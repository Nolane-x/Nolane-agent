import { createRouter } from './core/router.7422dac7821c.mjs';
import { createUiStore } from './core/ui-store.2c385b696e7c.mjs';
import { createUiBus } from './core/ui-bus.d1096214daf9.mjs';
import { createApiClient } from './core/api-client.cf4eafbe514e.mjs';
import { createLayoutStore } from './core/layout-store.a46fa6fcb661.mjs';
import { createResizableRegionController } from './core/resizable-region.8220d2c2ee06.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.6d172746b1b9.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.240c7180e775.mjs';
import { normalizeExperience } from './core/experience-policy.621cf349fb0b.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.198601aa563c.mjs';
import { createViewStateBridge } from './core/view-state-bridge.cad9ae2a1d9d.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.d4a92500fe50.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.d5b38e5b933b.mjs';
import { createUpdateStateController } from './core/update-state-controller.6482e1098d99.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.8b254fe4f95c.mjs';
import { settingsSectionFromRoute } from './core/settings-route.826db12dcd3d.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.5ed4be11ce44.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.1a39c6051af1.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.b1b423be1c03.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.0fb6f0bde598.mjs';
import { projectNameFromPath } from './shell/project-picker.4a820c13326f.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.004c6d5aace2.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.88d8fac37320.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.4c0ff89aef02.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.7277480337b1.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.fdc3443a2567.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.2244d8d17dec.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.da2f085ea1fa.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.120488c8efe2.mjs'), import('./views/workroom/terminal-client.62a18b39e8dd.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.fb069b68efba.mjs'), import('./control-plane/route-registry.a3ac85daa388.mjs'), import('./control-plane/live-domain-workspace.ae50a3bee331.mjs'), import('./views/browser/browser-view.36b562d1f3bb.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.aa952338fced.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.470763cf1569.mjs'), import('./views/onboarding/onboarding-view.e2fd6cc3d7ba.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.94edf8bfc83b.mjs'), import('./views/settings/settings-view.03ba7e39dfeb.mjs')]);
