import { createRouter } from './core/router.6e5f10044e2a.mjs';
import { createUiStore } from './core/ui-store.f855b00bf98b.mjs';
import { createUiBus } from './core/ui-bus.8ff9bbb2e0b5.mjs';
import { createApiClient } from './core/api-client.6b968f94f016.mjs';
import { createLayoutStore } from './core/layout-store.de980c056f71.mjs';
import { createResizableRegionController } from './core/resizable-region.d3ecd7de30f7.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.4c77431d1cf9.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.7747a941f9e1.mjs';
import { normalizeExperience } from './core/experience-policy.79b81ff8cac9.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.3959b1bb135e.mjs';
import { createViewStateBridge } from './core/view-state-bridge.f4e8d8e1be37.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.7b7e77a0923d.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.f9e78db191aa.mjs';
import { createUpdateStateController } from './core/update-state-controller.5463e656007c.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.209372001db0.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.7d91e85f2e5b.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.bf8971fcd081.mjs';
import { projectNameFromPath } from './shell/project-picker.5b5b27c5c327.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.83ce048dd42c.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.25e217a3f165.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.3121783ad8d8.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.e90631274dc4.mjs');
...
router.register({ id: 'projects', pattern: /^\/projects(?:\?.*)?$/, title: 'Projects', load: async () => {
  const { createProjectsController, renderProjectsView } = await import('./views/projects/project-view.ef7a6f8376f7.mjs');const controller=createProjectsController({api,language:cachedPreferences.language});await controller.load();let root=null;const view={render:()=>renderProjectsView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-project-search]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{if(e.target.closest('[data-project-action="add"]')){window.dispatchEvent(new CustomEvent('nolane:project-create-requested',{detail:{source:'projects-view'}}));}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view;
} });

router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.a425fe235101.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.4af9506fb01b.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.14063d1c67a8.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.fa86b485445d.mjs'), import('./control-plane/route-registry.8bc6ce05a741.mjs'), import('./control-plane/live-domain-workspace.126a6c0b251e.mjs'), import('./views/browser/browser-view.e5c6d9d61351.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.72dc046d3d05.mjs'), import('./views/settings/settings-view.b0f27e3c7cc6.mjs')]);
