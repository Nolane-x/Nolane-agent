import { createRouter } from './core/router.0617b39c3fa5.mjs';
import { createUiStore } from './core/ui-store.10aeae586ca2.mjs';
import { createUiBus } from './core/ui-bus.4d62042a63ad.mjs';
import { createApiClient } from './core/api-client.1ddee0236aa0.mjs';
import { createLayoutStore } from './core/layout-store.37d33e781ebb.mjs';
import { createResizableRegionController } from './core/resizable-region.9d7fa7a8901c.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.77c39f36d7c1.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.11cc9f625fe8.mjs';
import { normalizeExperience } from './core/experience-policy.1ee5cdf4ff08.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.21c5ada3b2c5.mjs';
import { createViewStateBridge } from './core/view-state-bridge.bf40573c8c74.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.5849673d2c30.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.f039d8e2ec4b.mjs';
import { createUpdateStateController } from './core/update-state-controller.13e0cdfa3497.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.14f37ff35a46.mjs';
import { settingsSectionFromRoute } from './core/settings-route.1229aa70a1a4.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.fc550ab72f2c.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.fdd65844846b.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.d8a2751a343f.mjs';
import { closeComposerPickers, isComposerPickerInteraction } from './components/composer-picker.9ec014f5771f.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.592ea4c53212.mjs';
import { projectNameFromPath } from './shell/project-picker.ddabdfd3b2d6.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.474706c06b0a.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.765964e62c17.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.35fec6a3cadd.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.6d8b7c880d99.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.6ed783dd62bb.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.94b92b380113.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.3388e4ca43d7.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.4d2fa2dfb966.mjs'), import('./views/workroom/terminal-client.c753ca985c05.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.001995d00c33.mjs'), import('./control-plane/route-registry.2fb1f219f9db.mjs'), import('./control-plane/live-domain-workspace.76201142b4b7.mjs'), import('./views/browser/browser-view.025c064fcb72.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.c84cfc158ba8.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.6303c2750cce.mjs'), import('./views/onboarding/onboarding-view.3020d2c0ac3e.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.f7a34a7abed5.mjs'), import('./views/settings/settings-view.b2ada0336219.mjs')]);
  const controller=createSettingsController({api});await controller.load();if(pendingSettingsLanguage!==null)controller.set('general.language',pendingSettingsLanguage);const requestedSection=settingsSectionFromRoute(path,{categories:controller.snapshot().catalog?.categories});if(requestedSection)controller.selectCategory(requestedSection);let mountedRoot=null;let inputTimer=null;let authPollTimer=null;let authPollAttempts=0;
...
async function render(path, { intent = null } = {}) {
  const requestIntent = intent ?? ++renderEpoch;
  if (requestIntent !== renderEpoch) return false;
  if (currentRouteState) persistCurrentSession();
  activeViewCleanup?.(); activeViewCleanup=null; resizer?.destroy(); resizer=null;
  await refreshShellData();
  if (requestIntent !== renderEpoch) return false;
  const state=await router.navigate(path,{replace:true});if(requestIntent !== renderEpoch || router.current()!==state)return false;currentRouteState=state;document.title=`${localizeRouteTitle(state.path,state.title,cachedPreferences.language)} — Nolane Agent`;
...
window.addEventListener('hashchange',()=>{const path=routeFromHash();if(onboardingRequired&&path!=='/onboarding'){location.hash='/onboarding';return;}void render(path,{intent:++renderEpoch});});
document.addEventListener('click',async(event)=>{
  ...
  const updateAction=event.target.closest?.('[data-update-action]')?.dataset.updateAction;if(updateAction){event.preventDefault();if(updateAction==='missions'){location.hash='/missions';return;}if(updateAction==='check')await updateStateController.check();if(updateAction==='download')await updateStateController.download();if(updateAction==='defer')await updateStateController.defer();if(updateAction==='ignore')await updateStateController.ignore();if(updateAction==='install')await updateStateController.install();renderUpdateNoticeRoot();return;}
