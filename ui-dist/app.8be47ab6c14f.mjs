import { createRouter } from './core/router.5d78739d94dd.mjs';
import { createUiStore } from './core/ui-store.e7b5778c1707.mjs';
import { createUiBus } from './core/ui-bus.b46a70e8eb1b.mjs';
import { createApiClient } from './core/api-client.53b9e1642732.mjs';
import { createLayoutStore } from './core/layout-store.2e39c1f335bd.mjs';
import { createResizableRegionController } from './core/resizable-region.048c3d47dc52.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.5ab509dc59f9.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.94fb37eb1a7c.mjs';
import { normalizeExperience } from './core/experience-policy.f9b0ddd6046d.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.9ae1361bbf23.mjs';
import { createViewStateBridge } from './core/view-state-bridge.8d40f3750fb1.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.5423ae559fc6.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.d40a2ecc68e2.mjs';
import { createUpdateStateController } from './core/update-state-controller.2f7876e0c0ac.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.f0e64d5e7e4e.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.85079c5aa02e.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.c4883b726cd2.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.10ba39dd6070.mjs';
import { projectNameFromPath } from './shell/project-picker.b93d39b8940f.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.c2edfb83a606.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.d4d1a1f645e2.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.2f894811a130.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.90b802ae2832.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.7927df453c20.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.6b44ec46d215.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.98019a72411e.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.ab38a1392abe.mjs'), import('./views/workroom/terminal-client.7d23a998404d.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.44e3a4c3696e.mjs'), import('./control-plane/route-registry.566df9e00d98.mjs'), import('./control-plane/live-domain-workspace.a61d91edb4c6.mjs'), import('./views/browser/browser-view.9caa81c6bcf0.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.c7cebaa4be44.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.1ca2ea072abe.mjs'), import('./views/onboarding/onboarding-view.df278fd0a854.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.4a071c425f88.mjs'), import('./views/settings/settings-view.5632f0f30006.mjs')]);
