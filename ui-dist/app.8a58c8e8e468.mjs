import { createRouter } from './core/router.a25005482331.mjs';
import { createUiStore } from './core/ui-store.a9c407f77cf1.mjs';
import { createUiBus } from './core/ui-bus.cae473fa019c.mjs';
import { createApiClient } from './core/api-client.911e65a00019.mjs';
import { createLayoutStore } from './core/layout-store.7ddcca0a1d81.mjs';
import { createResizableRegionController } from './core/resizable-region.ff14e731f5d1.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.6ba639852fe7.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.2a010ecfcd79.mjs';
import { normalizeExperience } from './core/experience-policy.07532be3987d.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.d00d7e1bf956.mjs';
import { createViewStateBridge } from './core/view-state-bridge.024f9dad6682.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.2e45a9e115d4.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.e9b177e18cb6.mjs';
import { createUpdateStateController } from './core/update-state-controller.7bcb2532466d.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.53e2f68c49a8.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.c558a44eb4a3.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.9685b4b5cf9c.mjs';
import { projectNameFromPath } from './shell/project-picker.d8223678b586.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.bd3eddce87ee.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.1f3ed265eed2.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.8ebf2a46ccde.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.4d7fab01c2a8.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.dc4b4a3b0cfa.mjs'), import('./control-plane/route-registry.ec3cb834bc2d.mjs'), import('./control-plane/live-domain-workspace.a1cf1662115f.mjs'), import('./views/browser/browser-view.061649e5e7b5.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.8a08bc12420a.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.cf574c80aa4f.mjs'), import('./views/onboarding/onboarding-view.f3e29a5334b8.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.9d5546da9f6d.mjs'), import('./views/settings/settings-view.9e0d74f0d367.mjs')]);
