import { createRouter } from './core/router.d77f6c76dfee.mjs';
import { createUiStore } from './core/ui-store.641abefe6bcf.mjs';
import { createUiBus } from './core/ui-bus.02883b73bc7d.mjs';
import { createApiClient } from './core/api-client.de2f86138163.mjs';
import { createLayoutStore } from './core/layout-store.6f30d053e6b2.mjs';
import { createResizableRegionController } from './core/resizable-region.4f1cc0f729aa.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.2fa53e481510.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.64d3e601d039.mjs';
import { normalizeExperience } from './core/experience-policy.773f538ef1ac.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.db3a7fd2c750.mjs';
import { createViewStateBridge } from './core/view-state-bridge.7f3d5000bc60.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.3de253e5594b.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.3511124e6561.mjs';
import { createUpdateStateController } from './core/update-state-controller.714ca7d29c88.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.9de1ddd56102.mjs';
import { settingsSectionFromRoute } from './core/settings-route.b1ae0532837c.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.060306e074e6.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.7e2c4d0ee8a0.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.7a8dba3e4be4.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.d65a0eb7c4b5.mjs';
import { projectNameFromPath } from './shell/project-picker.a0c095e921ff.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.65de4cd45450.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.7f0bdf9d232d.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.45edd7a8bdde.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.84ed6618ec42.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.47d07d001cbb.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.1dfb20542e61.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.c423be62508e.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.2b124d0480b6.mjs'), import('./views/workroom/terminal-client.4c7b63b171df.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.a634f253956b.mjs'), import('./control-plane/route-registry.f552460fbfd2.mjs'), import('./control-plane/live-domain-workspace.7a72f9099b22.mjs'), import('./views/browser/browser-view.437d8475472d.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.e47a9f9c8ba1.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.cce2038f516f.mjs'), import('./views/onboarding/onboarding-view.87a6c7960739.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.7ee89d05ea99.mjs'), import('./views/settings/settings-view.02f7b209f543.mjs')]);
