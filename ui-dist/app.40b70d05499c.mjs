import { createRouter } from './core/router.ee8ba275f4b5.mjs';
import { createUiStore } from './core/ui-store.20efebdab291.mjs';
import { createUiBus } from './core/ui-bus.044c86e33e9b.mjs';
import { createApiClient } from './core/api-client.b9404b6ef6fe.mjs';
import { createLayoutStore } from './core/layout-store.fe8328e694ae.mjs';
import { createResizableRegionController } from './core/resizable-region.780296941a76.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.11698474de76.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.1c3682671d56.mjs';
import { normalizeExperience } from './core/experience-policy.29bd49a81ae1.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.3dc78f4b24c2.mjs';
import { createViewStateBridge } from './core/view-state-bridge.d53c1d959bb1.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.70fff3a0e547.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.0c0acedabece.mjs';
import { createUpdateStateController } from './core/update-state-controller.07d81166bcde.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.ebbaa85e1617.mjs';
import { settingsSectionFromRoute } from './core/settings-route.2238dab07d47.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.1ea91847cda6.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.682af60ee382.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.9bb66f4fa1f7.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.9aa181b1af94.mjs';
import { projectNameFromPath } from './shell/project-picker.67e71708de75.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.2157d7b740b1.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.59b00d458fa8.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.1a3c3f7f843c.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.aeb764d45fad.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.557586f3da6a.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.74727197202e.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.0eae2aacea0d.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.cfbcb4641444.mjs'), import('./views/workroom/terminal-client.8ef483bf8cd4.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.f4b508cba8ce.mjs'), import('./control-plane/route-registry.9944cd4eac05.mjs'), import('./control-plane/live-domain-workspace.bfbd4663f15a.mjs'), import('./views/browser/browser-view.106c085840c9.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.fb0bb8082129.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.adcdaa8dddc9.mjs'), import('./views/onboarding/onboarding-view.14b6e711a049.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.87ba8a53b246.mjs'), import('./views/settings/settings-view.c0794582c671.mjs')]);
