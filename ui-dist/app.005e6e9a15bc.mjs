import { createRouter } from './core/router.433ce9e65163.mjs';
import { createUiStore } from './core/ui-store.59c374e5e759.mjs';
import { createUiBus } from './core/ui-bus.bef85362080e.mjs';
import { createApiClient } from './core/api-client.d56f135448c8.mjs';
import { createLayoutStore } from './core/layout-store.b31f1223ee8a.mjs';
import { createResizableRegionController } from './core/resizable-region.5d836da2f0f4.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.3d165423a63f.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.0c5c249b04b0.mjs';
import { normalizeExperience } from './core/experience-policy.3ec82b81cc10.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.ae1173cfca71.mjs';
import { createViewStateBridge } from './core/view-state-bridge.d4bf0f0ba174.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.2acf36708f24.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.0efc05f9b91d.mjs';
import { createUpdateStateController } from './core/update-state-controller.36fdb7d5911e.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.4a9fcae64d5a.mjs';
import { settingsSectionFromRoute } from './core/settings-route.130a8ea08184.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.a13fe6d30010.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.c18b6e2a478f.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.b29fc0d62aed.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.fc341ef78a64.mjs';
import { projectNameFromPath } from './shell/project-picker.c4674477fe07.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.5cfeaa80682f.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.1b57f4f2bd4b.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.6354b8cdb06f.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.0aa4d4f58abb.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.9ac8c3bf81c5.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.eff57f2eb50a.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.9943cb9285f3.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.357235ea2664.mjs'), import('./views/workroom/terminal-client.b06e8daa800e.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.515be5760eed.mjs'), import('./control-plane/route-registry.27dc26f565cd.mjs'), import('./control-plane/live-domain-workspace.7f621ff2d2be.mjs'), import('./views/browser/browser-view.f7ad37c211f1.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.345bd1b21ab3.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.add99ae430bf.mjs'), import('./views/onboarding/onboarding-view.d2fa291c2de0.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.62179dae55df.mjs'), import('./views/settings/settings-view.8ccab9886f5c.mjs')]);
