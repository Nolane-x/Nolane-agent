import { createRouter } from './core/router.da84095bcdfc.mjs';
import { createUiStore } from './core/ui-store.a8b2b6a154d4.mjs';
import { createUiBus } from './core/ui-bus.d279bea8a759.mjs';
import { createApiClient } from './core/api-client.f2b6faccf38a.mjs';
import { createLayoutStore } from './core/layout-store.b886828eef03.mjs';
import { createResizableRegionController } from './core/resizable-region.2d72343829b5.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.f571b83ab411.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.2fdcb47e64d5.mjs';
import { normalizeExperience } from './core/experience-policy.75de6e6c16ce.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.562e1308cb74.mjs';
import { createViewStateBridge } from './core/view-state-bridge.ff001c81d6a1.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.0bfb84be2cac.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.826eced5798d.mjs';
import { createUpdateStateController } from './core/update-state-controller.9b914bcc70f8.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.915464d25f9c.mjs';
import { settingsSectionFromRoute } from './core/settings-route.c8c588ce32e8.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.3a6ba7ebfb75.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.1a1783113e68.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.4cde9f4e9457.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.4611f05b4396.mjs';
import { projectNameFromPath } from './shell/project-picker.b925006b7ac9.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.1b6e70a29ca0.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.4d66aa09cecb.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.c125b31668f3.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.af319da8ab05.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.9fea09d2c26b.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.148d7c7d8593.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.08762c50baec.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.ee0070d3a2be.mjs'), import('./views/workroom/terminal-client.31534bd0732e.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.f00c742b9154.mjs'), import('./control-plane/route-registry.51fe938599fa.mjs'), import('./control-plane/live-domain-workspace.5cf6b45b9cff.mjs'), import('./views/browser/browser-view.5180d0027a90.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.c509a15e3ba3.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.fa41b16b57d5.mjs'), import('./views/onboarding/onboarding-view.be10d0af72ae.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.7776615fb3d7.mjs'), import('./views/settings/settings-view.f7f11e6b5dd6.mjs')]);
