import { createRouter } from './core/router.35b9588ec896.mjs';
import { createUiStore } from './core/ui-store.a4a51356f7f5.mjs';
import { createUiBus } from './core/ui-bus.98fbdb5bf3f8.mjs';
import { createApiClient } from './core/api-client.1a25dca9412a.mjs';
import { createLayoutStore } from './core/layout-store.a08e8078749e.mjs';
import { createResizableRegionController } from './core/resizable-region.996f1c4a2c23.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.8c936351e8c6.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.ac96f2f62904.mjs';
import { normalizeExperience } from './core/experience-policy.a2fcff370b88.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.1d2fba1132f4.mjs';
import { createViewStateBridge } from './core/view-state-bridge.2689f1411393.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.606d354ef7d7.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.dfe1b21ea4c7.mjs';
import { createUpdateStateController } from './core/update-state-controller.c8b7c8e5e093.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.aa6532a071be.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.8e446cf51c48.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.6d868ea3f89f.mjs';
import { projectNameFromPath } from './shell/project-picker.78b7ca40cf73.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.7896453e58ea.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.49d0f8805498.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.75c862f5d01e.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.bc919def620b.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.956e59f4ebd7.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.b9891abde924.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.90b922731341.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.ce5421dcf5aa.mjs'), import('./control-plane/route-registry.f3ac730c66c9.mjs'), import('./control-plane/live-domain-workspace.6fb11c0d678c.mjs'), import('./views/browser/browser-view.dff9c7fcae82.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.74cdc1b719b5.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.b5ac6fe35238.mjs'), import('./views/onboarding/onboarding-view.a0c898bbb04c.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.9e107e47fc57.mjs'), import('./views/settings/settings-view.08269ff4621a.mjs')]);
