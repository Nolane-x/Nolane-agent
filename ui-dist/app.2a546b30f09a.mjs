import { createRouter } from './core/router.0fe428c57272.mjs';
import { createUiStore } from './core/ui-store.f205e166da2e.mjs';
import { createUiBus } from './core/ui-bus.7dc8b4ca90e2.mjs';
import { createApiClient } from './core/api-client.62dfd6ef141e.mjs';
import { createLayoutStore } from './core/layout-store.19b07c054b0d.mjs';
import { createResizableRegionController } from './core/resizable-region.071122358386.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.3f6bc4af3b81.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.2512262c2552.mjs';
import { normalizeExperience } from './core/experience-policy.da0d04f87d4f.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.536860ba922d.mjs';
import { createViewStateBridge } from './core/view-state-bridge.9c28b718255c.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.5ffe03dc1169.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.2229eab53be1.mjs';
import { createUpdateStateController } from './core/update-state-controller.7a121a58f0e3.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.f0e8d7ccf48a.mjs';
import { settingsSectionFromRoute } from './core/settings-route.6e27f473015b.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.4642395c668a.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.869e9354b859.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.19623b893fe8.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.e0970d2cbda2.mjs';
import { projectNameFromPath } from './shell/project-picker.b2f581916d43.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.50671a25423a.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.58067fa74949.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.5dfe6f4fc117.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.47e2868afb08.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.23559b3a1c5f.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.7208acebab28.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.59ede7766abc.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.098e82476aa5.mjs'), import('./views/workroom/terminal-client.aa6a601914b2.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.23cc04ece9c6.mjs'), import('./control-plane/route-registry.7f70cdf98fc5.mjs'), import('./control-plane/live-domain-workspace.ea0c115ffe84.mjs'), import('./views/browser/browser-view.e0b476a69c00.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.1acf59fd6f85.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.e57e23677307.mjs'), import('./views/onboarding/onboarding-view.60b67cc459ae.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.2b78bae36bf1.mjs'), import('./views/settings/settings-view.bc909ece6326.mjs')]);
