import { createRouter } from './core/router.90dbbc540131.mjs';
import { createUiStore } from './core/ui-store.f7b9366cbfd6.mjs';
import { createUiBus } from './core/ui-bus.eb35c9bd6394.mjs';
import { createApiClient } from './core/api-client.5a0cc8fab86e.mjs';
import { createLayoutStore } from './core/layout-store.b2a902e33670.mjs';
import { createResizableRegionController } from './core/resizable-region.b4516c825789.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.73ef7fd27ade.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.08633091e97a.mjs';
import { normalizeExperience } from './core/experience-policy.5a4b4c6c524d.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.c71db1e4fdcf.mjs';
import { createViewStateBridge } from './core/view-state-bridge.d50c4a24c5bd.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.2d0fb56d9d89.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.36c9dbd9b15d.mjs';
import { createUpdateStateController } from './core/update-state-controller.ce5ee5e0e3cf.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.551108fc9ad4.mjs';
import { settingsSectionFromRoute } from './core/settings-route.18caf21e3a6b.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.316e34215432.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.7455a1b37056.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.ea8a59f23e18.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.de2f1c79a3e7.mjs';
import { projectNameFromPath } from './shell/project-picker.c08d9a76b470.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.8ff390608c0c.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.aea7a8921029.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.e0c5c85d10a5.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.6925c16cba89.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.ac2ea5662f53.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.bbb93a170d46.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.a7310df3f6d3.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.976e336cb1a5.mjs'), import('./views/workroom/terminal-client.1686878b3f8e.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.1ce56496296d.mjs'), import('./control-plane/route-registry.19512986ff3d.mjs'), import('./control-plane/live-domain-workspace.3d35a41ce08a.mjs'), import('./views/browser/browser-view.cb0230b8ee30.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.f3982ff7d515.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.7dbd29f50257.mjs'), import('./views/onboarding/onboarding-view.9b6e0f51f05e.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.e6d6555725cb.mjs'), import('./views/settings/settings-view.3462a3e22daa.mjs')]);
