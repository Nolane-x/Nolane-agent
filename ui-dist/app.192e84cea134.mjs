import { createRouter } from './core/router.f240a8ac1b02.mjs';
import { createUiStore } from './core/ui-store.8ef73009d514.mjs';
import { createUiBus } from './core/ui-bus.6197b7ef647e.mjs';
import { createApiClient } from './core/api-client.93751c122d1d.mjs';
import { createLayoutStore } from './core/layout-store.275d9a33b6dc.mjs';
import { createResizableRegionController } from './core/resizable-region.ecd19bf1b6f7.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.78359e3a4791.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.e6c3fe38e8a9.mjs';
import { normalizeExperience } from './core/experience-policy.7cee13db2cca.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.7c6d95d84b86.mjs';
import { createViewStateBridge } from './core/view-state-bridge.b137099389b0.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.845ba3272b05.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.c96ec95497c8.mjs';
import { createUpdateStateController } from './core/update-state-controller.06fca8538392.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.fd87935bcc9b.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.eebb12823fea.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.5add9f41161f.mjs';
import { projectNameFromPath } from './shell/project-picker.97db74817eef.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.eba6ae076261.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.26482240b647.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.a779007a31e9.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.2b1349d6bbbb.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.39bffb36cc3e.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.2fe153025234.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.d7eebec45650.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.e63540d588ea.mjs'), import('./views/workroom/terminal-client.8757129108e6.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.ef9c368cd661.mjs'), import('./control-plane/route-registry.c4d21babbbe3.mjs'), import('./control-plane/live-domain-workspace.5f8008bbdc8e.mjs'), import('./views/browser/browser-view.9c6a34c71e27.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.4378330acc7d.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.bc55c6d0a2f0.mjs'), import('./views/onboarding/onboarding-view.5427f0d89ce5.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.bb1a9405f0d7.mjs'), import('./views/settings/settings-view.c2909b19069d.mjs')]);
