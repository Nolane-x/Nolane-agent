import { createRouter } from './core/router.931fdad8e3e1.mjs';
import { createUiStore } from './core/ui-store.41bfbd7402d6.mjs';
import { createUiBus } from './core/ui-bus.5f523f2c8492.mjs';
import { createApiClient } from './core/api-client.38bbdb9eedba.mjs';
import { createLayoutStore } from './core/layout-store.4499d66e2e48.mjs';
import { createResizableRegionController } from './core/resizable-region.7c89845085a7.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.998403b28261.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.6d73b6069ee9.mjs';
import { normalizeExperience } from './core/experience-policy.bfdc232179d8.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.e3c608629c57.mjs';
import { createViewStateBridge } from './core/view-state-bridge.f1400a6bd6d6.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.706a193f0604.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.5a4044c14ec2.mjs';
import { createUpdateStateController } from './core/update-state-controller.6b484923bd2e.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.245d5204451c.mjs';
import { settingsSectionFromRoute } from './core/settings-route.88da9b1a0ec1.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.2612600e2ea1.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.d3e015433af9.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.1bf23ce43106.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.21ac5825124d.mjs';
import { projectNameFromPath } from './shell/project-picker.d6df23f8bbb0.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.080dfaa79c43.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.3afb679bb2f2.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.97a2dd66c443.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.f673683d8ff7.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.ee8ef0d7beb3.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.8ea0b1c1d22d.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.5a2eb80cba6e.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.00a36cc128ef.mjs'), import('./views/workroom/terminal-client.7d41beaffaec.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.6eab7917cb4c.mjs'), import('./control-plane/route-registry.0d15f727b1a9.mjs'), import('./control-plane/live-domain-workspace.03b40834638f.mjs'), import('./views/browser/browser-view.74c1b2440413.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.077959738af2.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.52dc2e633563.mjs'), import('./views/onboarding/onboarding-view.058466d8050f.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.a774439ffaa0.mjs'), import('./views/settings/settings-view.ccc9e95d6c78.mjs')]);
