import { createRouter } from './core/router.bf91af501bbd.mjs';
import { createUiStore } from './core/ui-store.b827672a629c.mjs';
import { createUiBus } from './core/ui-bus.62b92dc69241.mjs';
import { createApiClient } from './core/api-client.755fc1c61da2.mjs';
import { createLayoutStore } from './core/layout-store.754d38d29265.mjs';
import { createResizableRegionController } from './core/resizable-region.4f66b078e392.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.993410335479.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.61a3c78344eb.mjs';
import { normalizeExperience } from './core/experience-policy.6f70e93a0167.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.e143bb62891a.mjs';
import { createViewStateBridge } from './core/view-state-bridge.642c3d8c6003.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.45be0cda746f.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.029e9a0ad2ed.mjs';
import { createUpdateStateController } from './core/update-state-controller.38474aa7ceca.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.dd6313b82fe7.mjs';
import { settingsSectionFromRoute } from './core/settings-route.eed043432158.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.45549486e930.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.87d42efd3333.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.31248099a4ec.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.56c1f81ce0ff.mjs';
import { projectNameFromPath } from './shell/project-picker.30955cec88ac.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.6e291ac0226e.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.f13023a67dc2.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.553ec5d6e4a4.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.b2cbdb8c4fc2.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.cee52dfb11d6.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.516bf3b6a1e6.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.0fcda9351c53.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.a56ffe504838.mjs'), import('./views/workroom/terminal-client.9d6ded8609cf.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.22ba61f01f89.mjs'), import('./control-plane/route-registry.822fd1abe775.mjs'), import('./control-plane/live-domain-workspace.126280785b6b.mjs'), import('./views/browser/browser-view.5742baafdeaa.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.74a32f5a0b82.mjs'), import('./views/settings/settings-view.976c336f1def.mjs')]);
