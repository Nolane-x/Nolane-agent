...
export function createRouter({ initialPath = '/' } = {}) {
  ...
  async function resolve(path, requestId) {
    ...
    const state = Object.freeze({ path: normalized, route, view, title: route.title ?? route.id });
    if (requestId === navigationId) current = state;
    return state;
  }
  ...
  async function navigate(path, { replace = false } = {}) {
    ...
    return resolve(normalized, requestId);
  }

  async function back() {
    if (index > 0) index -= 1;
    return resolve(history[index], ++navigationId);
  }
