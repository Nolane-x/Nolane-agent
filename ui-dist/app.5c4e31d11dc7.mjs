import { createRouter } from './core/router.7d5f56da3283.mjs';
import { createUiStore } from './core/ui-store.27dc307dbdd2.mjs';
import { createUiBus } from './core/ui-bus.9a4d052f1805.mjs';
import { createApiClient } from './core/api-client.00f3bb9b1dda.mjs';
import { createLayoutStore } from './core/layout-store.a438b5c3fedf.mjs';
import { createResizableRegionController } from './core/resizable-region.032941defbc2.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.641bf35df6dd.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.5fe9a0b72b14.mjs';
import { normalizeExperience } from './core/experience-policy.096a35857221.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.7c0b61905fbe.mjs';
import { createViewStateBridge } from './core/view-state-bridge.158f49fe1b73.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.631f38b50da0.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.bb4d54f9e5a1.mjs';
import { createUpdateStateController } from './core/update-state-controller.0a76a3aceca0.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.42e0ff8b364a.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.28a5547a916f.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.7d96e63b207c.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.160497a07d83.mjs';
import { projectNameFromPath } from './shell/project-picker.ac0e5057b986.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.c94472f53c37.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.44011f6fd68b.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.96b5704096ed.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.aae76974d8b2.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.2dcaf4d73954.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.20b689f4f8ee.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.26d4b949788b.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.488bcd6c6caf.mjs'), import('./views/workroom/terminal-client.a6eb8303ff9d.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.a23d1b40eda7.mjs'), import('./control-plane/route-registry.bd07c543040f.mjs'), import('./control-plane/live-domain-workspace.796593b24462.mjs'), import('./views/browser/browser-view.281103b71588.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.c52142eed92c.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.8cb4198124a3.mjs'), import('./views/onboarding/onboarding-view.04f0d97c4da7.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.ac548c52089f.mjs'), import('./views/settings/settings-view.f1d84a84cc01.mjs')]);
