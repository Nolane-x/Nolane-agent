import { createRouter } from './core/router.01fb711124ae.mjs';
import { createUiStore } from './core/ui-store.a536da6a44cf.mjs';
import { createUiBus } from './core/ui-bus.a674eeab9130.mjs';
import { createApiClient } from './core/api-client.7d8577564fc9.mjs';
import { createLayoutStore } from './core/layout-store.cce67de2df38.mjs';
import { createResizableRegionController } from './core/resizable-region.18f4ac931fba.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.18ff2a700aed.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.ceffe7bca0d5.mjs';
import { normalizeExperience } from './core/experience-policy.3fda49a7f92c.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.e3248bf65503.mjs';
import { createViewStateBridge } from './core/view-state-bridge.6d086ebed5f7.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.1408f36b27f3.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.fc9e6e1b9972.mjs';
import { createUpdateStateController } from './core/update-state-controller.88b4b3ba8972.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.b6143ee95a39.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.f377e4bddc9e.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.db240bb60f6f.mjs';
import { projectNameFromPath } from './shell/project-picker.ed148f377512.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.93d2102abeb8.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.f0bdfcd4b33d.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.0c1d8be329a0.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.48aa09c8de91.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.a00cd871e449.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.9ac6a5dde2c5.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.68a0f0ca6f2a.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.58e028bbe60c.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.4d91eec5fc36.mjs'), import('./control-plane/route-registry.82b8895cb19a.mjs'), import('./control-plane/live-domain-workspace.8e9711fe8ed8.mjs'), import('./views/browser/browser-view.18d54bffdf86.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.a2868bb5b750.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.451cb31c08e8.mjs'), import('./views/onboarding/onboarding-view.463a10c1ef1d.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.3fb68525e370.mjs'), import('./views/settings/settings-view.563d2806fff9.mjs')]);
