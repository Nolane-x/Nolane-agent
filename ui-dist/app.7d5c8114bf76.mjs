import { createRouter } from './core/router.371ecafbca15.mjs';
import { createUiStore } from './core/ui-store.ce4a3b169707.mjs';
import { createUiBus } from './core/ui-bus.a9d9bb6124d2.mjs';
import { createApiClient } from './core/api-client.d1feeecabffe.mjs';
import { createLayoutStore } from './core/layout-store.01b1d865dc37.mjs';
import { createResizableRegionController } from './core/resizable-region.a023e2aaca46.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.11be15a3bbdb.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.07f0ec3a8921.mjs';
import { normalizeExperience } from './core/experience-policy.19e6b7d6c808.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.d598218dbbea.mjs';
import { createViewStateBridge } from './core/view-state-bridge.e93a22082580.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.7a91c5ff7d82.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.32a4f085e2c0.mjs';
import { createUpdateStateController } from './core/update-state-controller.bc2bcba2d4ef.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.70ab8caa1aab.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.d51cbefa6daa.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.1bd0605f80ab.mjs';
import { projectNameFromPath } from './shell/project-picker.c4820072ba9e.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.2f51407d335c.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.81f69d37a043.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.a2744111b3db.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.12924a4117b8.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.f836b729caa7.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.851885832496.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.dd1d0bc1db70.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.7994f03228b1.mjs'), import('./control-plane/route-registry.db845508bfe5.mjs'), import('./control-plane/live-domain-workspace.c04e0df8503c.mjs'), import('./views/browser/browser-view.85ae901f8f9f.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.bbc80e6fe866.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.37216a896ec0.mjs'), import('./views/onboarding/onboarding-view.04a14bed2e5e.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.7231f560125f.mjs'), import('./views/settings/settings-view.e5a60f1841c4.mjs')]);
