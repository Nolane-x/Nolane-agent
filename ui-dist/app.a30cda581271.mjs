import { createRouter } from './core/router.6e66a317ee15.mjs';
import { createUiStore } from './core/ui-store.37d4e9afd2ec.mjs';
import { createUiBus } from './core/ui-bus.066e2f1c181c.mjs';
import { createApiClient } from './core/api-client.c94a8c8b5295.mjs';
import { createLayoutStore } from './core/layout-store.ea1658f171bd.mjs';
import { createResizableRegionController } from './core/resizable-region.a3bd277bb62d.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.f4006bccfa5e.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.66d06cc345a5.mjs';
import { normalizeExperience } from './core/experience-policy.93d8e1eeed8a.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.e39e05d9004d.mjs';
import { createViewStateBridge } from './core/view-state-bridge.f377597b0014.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.580153402764.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.7f0e1ee76957.mjs';
import { createUpdateStateController } from './core/update-state-controller.d708b9fd4744.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.68fe0904694b.mjs';
import { settingsSectionFromRoute } from './core/settings-route.cd826f308de4.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.eb933ed29185.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.a91b5f36d239.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.4544cc1e8fd8.mjs';
import { closeComposerPickers, isComposerPickerInteraction } from './components/composer-picker.a5f73288a30d.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.7ddcf44a63d9.mjs';
import { projectNameFromPath } from './shell/project-picker.d754cb7c30dd.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.09173e6d4191.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.2ceffc7b078b.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.16e5d19f1f07.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.98556b22458c.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.5accba1727ef.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => {
  const { createReviewController, renderReviewView } = await import('./views/review/review-view.4ecb92085bbc.mjs');
...
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.93d6322f2e9d.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, cache: 'path', title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.0eb2692bb3fd.mjs'), import('./views/workroom/terminal-client.6e52b96ec341.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.bf00e09ef733.mjs'), import('./control-plane/route-registry.6f40061920a7.mjs'), import('./control-plane/live-domain-workspace.da3c24dfecb6.mjs'), import('./views/browser/browser-view.405a48bd582c.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.876022f3f82d.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.590cc355d51b.mjs'), import('./views/onboarding/onboarding-view.730df53a28fe.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.94b6d9ae8090.mjs'), import('./views/settings/settings-view.d93fc4447fc5.mjs')]);
