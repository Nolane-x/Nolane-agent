import { createRouter } from './core/router.d24b7219ae93.mjs';
import { createUiStore } from './core/ui-store.f6a9ded90179.mjs';
import { createUiBus } from './core/ui-bus.7ff3e73bf8a6.mjs';
import { createApiClient } from './core/api-client.88f10ddac2a3.mjs';
import { createLayoutStore } from './core/layout-store.f3b2bca5b30f.mjs';
import { createResizableRegionController } from './core/resizable-region.488ae4f4826b.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.49d9f6619df2.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.8a8c8f002887.mjs';
import { normalizeExperience } from './core/experience-policy.2281ed7b1d94.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.ae8ab250502c.mjs';
import { createViewStateBridge } from './core/view-state-bridge.1151a410d6e8.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.1bfca67bba31.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.de254a9d2e46.mjs';
import { createUpdateStateController } from './core/update-state-controller.c5bdea8be013.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.e6d1e28bc4a3.mjs';
import { settingsSectionFromRoute } from './core/settings-route.408ce48a1622.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.4fa07f3f8684.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.d9234af4f8e9.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.3fe794ffeb5c.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.dff48d03f420.mjs';
import { projectNameFromPath } from './shell/project-picker.091d38362a91.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.0bd0bff4b8a9.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.4e7b456e41dd.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.4c951fb7df71.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.41b3ef4bd59a.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.3e73a4da04dd.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.a594131eb731.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.d418c8ac03ec.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.a747037fda79.mjs'), import('./views/workroom/terminal-client.ddf7e4507cc8.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.17d0f0447b7a.mjs'), import('./control-plane/route-registry.52d30b1270b1.mjs'), import('./control-plane/live-domain-workspace.d10036dcc0cc.mjs'), import('./views/browser/browser-view.1e8c7d03228e.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.9f1bb8a1e08b.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.0d81446a6dfd.mjs'), import('./views/onboarding/onboarding-view.add80e4e6056.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.2d9eadc151e4.mjs'), import('./views/settings/settings-view.2916de835720.mjs')]);
