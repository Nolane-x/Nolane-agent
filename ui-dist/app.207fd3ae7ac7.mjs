import { createRouter } from './core/router.07c37029be50.mjs';
import { createUiStore } from './core/ui-store.dc288db5c39f.mjs';
import { createUiBus } from './core/ui-bus.82ecf58f6928.mjs';
import { createApiClient } from './core/api-client.207c92efc7b0.mjs';
import { createLayoutStore } from './core/layout-store.a9ecee50a9e8.mjs';
import { createResizableRegionController } from './core/resizable-region.a34f1d660e8d.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.0d9b3d10887e.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.33bdb64e7b36.mjs';
import { normalizeExperience } from './core/experience-policy.97bd07a1b665.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.2fa60cfb1904.mjs';
import { createViewStateBridge } from './core/view-state-bridge.35abef4b5682.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.dd7a945d418e.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.43e148dc97eb.mjs';
import { createUpdateStateController } from './core/update-state-controller.db7bc8ddd764.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.9aa2139dc0ec.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.f3e1017f95fe.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.2de430aaa46a.mjs';
import { projectNameFromPath } from './shell/project-picker.b1e6aaed9c4e.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.de8ee993cc15.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.8d17357334c1.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.9e11e0c9e273.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.cce6bb5018da.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.575c5124188b.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.1055dc93ae4e.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.a93f2568e579.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.54fe81f191ae.mjs'), import('./control-plane/route-registry.dbaba4ba9256.mjs'), import('./control-plane/live-domain-workspace.a38e057ed8b0.mjs'), import('./views/browser/browser-view.78fdf4590286.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.ce3a31bf330c.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.ace41266ede6.mjs'), import('./views/onboarding/onboarding-view.8dc1c75ba8a2.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.ba4de98283ca.mjs'), import('./views/settings/settings-view.5c3824d73238.mjs')]);
