import { createRouter } from './core/router.a6be43d08ad8.mjs';
import { createUiStore } from './core/ui-store.bb31668bdff8.mjs';
import { createUiBus } from './core/ui-bus.a649d336986e.mjs';
import { createApiClient } from './core/api-client.3c8980302333.mjs';
import { createLayoutStore } from './core/layout-store.9af2271a58cd.mjs';
import { createResizableRegionController } from './core/resizable-region.ed9c9ca989b4.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.a4b2b5fac681.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.7763deaefb02.mjs';
import { normalizeExperience } from './core/experience-policy.0347257e4382.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.e93248e3e151.mjs';
import { createViewStateBridge } from './core/view-state-bridge.cba0d05846d2.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.8cf8ab6d996d.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.fb23353398b1.mjs';
import { createUpdateStateController } from './core/update-state-controller.600cd495edd4.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.3eb51ef11f50.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.a96773067d87.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.5c96cfe8a332.mjs';
import { projectNameFromPath } from './shell/project-picker.938b8e0b0f32.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.da94a4a5c931.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.36835a58357b.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.c9b7b6b89854.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.0dd8c686b910.mjs');
...
router.register({ id: 'projects', pattern: /^\/projects(?:\?.*)?$/, title: 'Projects', load: async () => {
  const { createProjectsController, renderProjectsView } = await import('./views/projects/project-view.84a9ceb19619.mjs');const controller=createProjectsController({api,language:cachedPreferences.language});await controller.load();let root=null;const view={render:()=>renderProjectsView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-project-search]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{if(e.target.closest('[data-project-action="add"]')){window.dispatchEvent(new CustomEvent('nolane:project-create-requested',{detail:{source:'projects-view'}}));}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view;
} });

router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.bd57e94b59cd.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.f0903468d35b.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.4133bab2ce8b.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.e50378838729.mjs'), import('./control-plane/route-registry.4a660500d155.mjs'), import('./control-plane/live-domain-workspace.aa070c43ddc1.mjs'), import('./views/browser/browser-view.d32db58d507a.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.b57851f2cc53.mjs'), import('./views/settings/settings-view.26b6dfc4f6b9.mjs')]);
