import { createRouter } from './core/router.07ab17d6b943.mjs';
import { createUiStore } from './core/ui-store.b5e0f8005bdd.mjs';
import { createUiBus } from './core/ui-bus.69871a307161.mjs';
import { createApiClient } from './core/api-client.1cd3de54a3c4.mjs';
import { createLayoutStore } from './core/layout-store.b8aae58ef170.mjs';
import { createResizableRegionController } from './core/resizable-region.5ac7da2d5bed.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.d59c079a1525.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.1a51ce42d6d1.mjs';
import { normalizeExperience } from './core/experience-policy.6bd5cbaba555.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.f8b3e96a4112.mjs';
import { createViewStateBridge } from './core/view-state-bridge.324deb5314dc.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.98634b0c359c.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.24a3c14f45ba.mjs';
import { createUpdateStateController } from './core/update-state-controller.13fa612e55eb.mjs';
import { routeFromHash, scrubBootstrapToken } from './core/route-auth.2361a231d51d.mjs';
import { settingsSectionFromRoute } from './core/settings-route.f86048c449f5.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.0a2e9f5192f0.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.39bae7c3c2db.mjs';
import { closeOptionPickers, handleOptionPickerKeydown, selectOptionPicker, toggleOptionPicker } from './components/option-picker.b313ed9f9f2b.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.967cc8ce9e63.mjs';
import { projectNameFromPath } from './shell/project-picker.0e3c55f1563c.mjs';
import { openProjectCreateDialog } from './components/project-create-dialog.3ebcb0ff8b4b.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.f8bdc3036b7d.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.802f1bb6a63e.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.ef7778840ca7.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.9d44a45fe8e6.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.e4371a364864.mjs'); const model = createReviewModel({ missionId: routeFromHash().split('?')[0].split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.6dc129b70e55.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const [{ createWorkroomModel, renderWorkroomView }, { createTerminalClient, decodeTerminalOutput }] = await Promise.all([import('./views/workroom/workroom-view.c969999f2875.mjs'), import('./views/workroom/terminal-client.3b7bf6eca41d.mjs')]);
...
router.register({ id: 'control-plane', pattern: /^\/(?:control-plane(?:\/.*)?|browser)$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, normalizeControlPlanePath, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.1c86a5732414.mjs'), import('./control-plane/route-registry.60a84474de71.mjs'), import('./control-plane/live-domain-workspace.f9e112c68560.mjs'), import('./views/browser/browser-view.545aa8f8d26f.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.a444ea69435b.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.73c879c55e11.mjs'), import('./views/onboarding/onboarding-view.6b9b681bef39.mjs')]);
...
router.register({ id: 'settings', pattern: /^\/settings(?:\?.*)?$/, title: 'Settings', load: async ({ path }) => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.64caf0dabafb.mjs'), import('./views/settings/settings-view.baae98a59525.mjs')]);
