import { createRouter } from './core/router.6ab2106c186b.mjs';
import { createUiStore } from './core/ui-store.3e01c08e872f.mjs';
import { createUiBus } from './core/ui-bus.fed66a9cff83.mjs';
import { createApiClient } from './core/api-client.780659c06884.mjs';
import { createLayoutStore } from './core/layout-store.63922d630f10.mjs';
import { createResizableRegionController } from './core/resizable-region.b0a7eea59010.mjs';
import { applyPreferences, readCachedPreferences } from './core/preference-runtime.15296ad07274.mjs';
import { createLanguageSyncController } from './core/language-sync-controller.15623577163a.mjs';
import { normalizeExperience } from './core/experience-policy.72154f82984d.mjs';
import { createExperienceTransitionController } from './core/experience-transition-controller.c48301c46aa4.mjs';
import { createViewStateBridge } from './core/view-state-bridge.b3178fa5f70c.mjs';
import { captureViewState, restoreViewState } from './core/view-state-preserver.5dce620b53e2.mjs';
import { createSessionRestoreController } from './core/session-restore-controller.58ea9592d566.mjs';
import { createUpdateStateController } from './core/update-state-controller.46bcf590e5f7.mjs';
import { localizeRouteTitle, renderAppShell } from './shell/app-shell.7076c63b84a5.mjs';
import { renderUpdateNotice } from './components/update-notice/update-notice.f1e31eea23b6.mjs';
import { createSessionSidebarModel } from './shell/session-sidebar.06b04f678818.mjs';
import { projectNameFromPath } from './shell/project-picker.b3852f106a10.mjs';
import { createOutputSummaryController, renderOutputSummary } from './views/summary/output-summary.cc5fd4a25d3d.mjs';
import { BACKEND_ATLAS } from './generated/backend-atlas.97dca073a9fe.mjs';
...
router.register({ id: 'home', pattern: /^\/(?:\?.*)?$/, title: 'Chat', load: async () => {
  ...
  const { createHomeController, renderHomeView } = await import('./views/home/home-view.73c0317a4bcc.mjs');
...
router.register({ id: 'missions', pattern: /^\/missions(?:\?.*)?$/, cache: 'path', title: 'Activity', load: async () => {
  const { createActivityController, renderActivityView } = await import('./views/activity/activity-view.4822a93ddcde.mjs');
...
router.register({ id: 'review-mission', pattern: /^\/review\/.+$/, cache: 'path', title: 'Review & Ship', load: async () => { const { createReviewModel, renderReviewView } = await import('./views/review/review-view.8cb86add91bb.mjs'); const model = createReviewModel({ missionId: location.hash.slice(1).split('/').at(-1) || 'current' }); return { experienceLevel:'workspace',render: () => renderReviewView(model.snapshot(), { language: cachedPreferences.language }) }; } });
router.register({ id: 'review', pattern: '/review', title: 'Review Queue', load: async () => { const { createReviewController, renderReviewQueue } = await import('./views/review-queue/review-queue.1bccf71a1db7.mjs');const controller=createReviewController({api,language:cachedPreferences.language});await controller.load();return {experienceLevel:'workspace',render:()=>renderReviewQueue(controller.snapshot())}; } });

router.register({ id: 'workroom', pattern: /^\/workroom(?:\?.*)?$/, title: 'Studio', load: async () => {
  const { createWorkroomModel, renderWorkroomView } = await import('./views/workroom/workroom-view.9a5ff2b11352.mjs');
...
router.register({ id: 'control-plane', pattern: /^\/control-plane(?:\/.*)?$/, cache: 'path', title: 'Control Plane', load: async () => {
  const [{ createControlPlaneModel, renderControlPlaneShell }, { loadControlPlaneDomain, renderControlPlaneDomain }, { hasLiveDomainWorkspace, loadLiveDomainWorkspace, renderLiveDomainWorkspace }, browserView] = await Promise.all([import('./control-plane/control-plane-shell.c8247eb1c0f4.mjs'), import('./control-plane/route-registry.17f135251415.mjs'), import('./control-plane/live-domain-workspace.d40e66144a64.mjs'), import('./views/browser/browser-view.8176df2b1952.mjs')]);
...
router.register({ id: 'search', pattern: /^\/search(?:\?.*)?$/, title: 'Search', load: async () => { const { createSearchController, renderSearchView } = await import('./views/search/search-view.07bf6f7fa766.mjs');const controller=createSearchController({api,language:cachedPreferences.language,capabilities:BACKEND_ATLAS.entries});await controller.load();let root=null;const view={render:()=>renderSearchView(controller.snapshot()),mount(node){root=node;const input=e=>{if(e.target.matches('[data-global-search-input]')){controller.setQuery(e.target.value);rerenderView(root,view);}};const click=e=>{const filter=e.target.closest('[data-search-filter]');if(filter){controller.setFilter(filter.dataset.searchFilter);root.innerHTML=view.render();root.querySelector('[data-global-search-input]')?.focus();}};root.addEventListener('input',input);root.addEventListener('click',click);return()=>{root.removeEventListener('input',input);root.removeEventListener('click',click)}}};return view; } });

router.register({ id: 'onboarding', pattern: '/onboarding', title: 'Welcome', load: async () => {
  const [{ createOnboardingController }, { renderOnboardingView }] = await Promise.all([import('./views/onboarding/onboarding-controller.94cc0a486f1b.mjs'), import('./views/onboarding/onboarding-view.580bc53d3160.mjs')]);
...
router.register({ id: 'settings', pattern: '/settings', title: 'Settings', load: async () => {
  const [{ createSettingsController }, { renderSettingsView }] = await Promise.all([import('./views/settings/settings-controller.f14a704d03a1.mjs'), import('./views/settings/settings-view.189430bdffe7.mjs')]);
