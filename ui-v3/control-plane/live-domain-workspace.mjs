...
const DOMAIN_META = Object.freeze({
  ...
  extensions: { title: 'Extensions', kicker: 'Models, tools, skills, and providers', description: 'Connected providers, model profiles, MCP tools, plugins, and native skills with conservative readiness and trust states.', endpoints: [
    ...
    ['skills','Native skills',ctx=>'/api/nolane/orchestration/skills'],
  ]},
...
function rowDetail(item,language='en'){ if(!item||typeof item!=='object')return ''; const entries=Object.entries(item).filter(([key,value])=>!SECRET_KEY.test(key)&&value!=null&&['string','number','boolean'].includes(typeof value)).slice(0,3); return entries.map(([key,value])=>`${key}: ${safePrimitive(key,value,language)}`).join(' · '); }
...
export async function loadLiveDomainWorkspace({api,domain,projectId=null,missionId=null,language='en'}={}){
  const meta=localizedMeta(domain,language); if(!meta)throw new Error(`No live Control Plane workspace for ${domain}`);
  const context={projectId,missionId}; const started=Date.now();
  const records=await Promise.all(meta.endpoints.map(async definition=>{
    ...
    try{ const payload=await api.get(path); const rows=arrayCandidate(payload).slice(0,5); return Object.freeze({id,label,path,status:statusValue(payload),error:null,count:recordCount(payload),metrics:metrics(payload,language),rows:Object.freeze(rows.map((item,index)=>Object.freeze({title:rowTitle(item,index,language),detail:rowDetail(item,language)})))}); }
    catch(error){ return Object.freeze({id,label,path,status:'error',error:String(error?.payload?.error??error?.message??error),count:0,metrics:[],rows:[]}); }
  }));
...
function endpointCard(record,language){ const t=copy(language); const metricHtml=record.metrics.slice(0,6).map(([label,value])=>`<div><dt>${escapeHtml(label)}</dt><dd>${escapeHtml(value)}</dd></div>`).join(''); const rows=record.rows.map(item=>`<li><span>${escapeHtml(item.title)}</span>${item.detail?`<small>${escapeHtml(item.detail)}</small>`:''}</li>`).join(''); return `<article class="cp-adapter-card" data-adapter-status="${escapeHtml(statusClass(record.status))}"><header><div><p>${escapeHtml(record.label)}</p><code>${escapeHtml(record.path)}</code></div><span class="cp-status" data-tone="${escapeHtml(statusClass(record.status))}">${escapeHtml(record.status)}</span></header>${record.error?`<div class="cp-adapter-error" role="status">${escapeHtml(record.error)}</div>`:`<div class="cp-adapter-count"><strong>${record.count.toLocaleString()}</strong><span>${escapeHtml(t.records)}</span></div>`}${metricHtml?`<dl class="cp-adapter-metrics">${metricHtml}</dl>`:''}${rows?`<ul class="cp-adapter-rows">${rows}</ul>`:`<p class="cp-empty">${escapeHtml(record.error?t.adapterClosed:t.noRecords)}</p>`}</article>`; }
