# Forge Studio 2.27.0 limitations and non-claims
...
The item-level audit source is `docs/feature-audit-2.27.0.json`. The exhaustive incomplete and externally gated inventory is `docs/REMAINING-GAPS-2.27.0.md`.

# Retained platform, runtime, security, and release boundaries

## Memory, Skill, and Resource OS boundary

Forge Studio 2.27.0 implements governed memory lifecycle operations and shadow policy decisions. It does not change production memory policy automatically; a policy decision remains advisory until an explicit governed caller commits an allowed operation.
...
The deterministic local measurement is release-contract evidence. Forge Studio 2.27.0 does not claim benchmark superiority over Cursor, Codex, Claude Code, Copilot, Devin, or another agent.

The item-level source of truth is `docs/feature-audit-2.27.0.json`. The exhaustive open-item report is `docs/REMAINING-GAPS-2.27.0.md`.
