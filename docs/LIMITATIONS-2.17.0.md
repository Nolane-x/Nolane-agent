# Forge Studio 2.17.0 — remaining limits
...
The synthetic measurement in `docs/adaptive-work-fabric-measurement-2.17.0.json` exercises local in-memory runners. It does not invoke or benchmark external model providers, Playwright browsers, hosted workers, network services or production repositories.
...
The item-level source of truth is `docs/feature-audit-2.17.0.json`. The exhaustive open-item report is `docs/REMAINING-GAPS-2.17.0.md`.
...
The cloud stage is eligibility after the local stages only. Forge Studio 2.17.0 does not claim that a cloud agent, cloud sandbox, hosted control plane, or remote worker was operated by this local release matrix.
