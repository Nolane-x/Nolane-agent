...
# Nolane Agent Small-Model Superintelligence Research Program 1.0.0

**Ngày nghiên cứu:** 30-07-2026  
**Đối tượng kiểm toán:** Nolane Agent 2.18.0 và Frontier Requirements 3.0.0  
...
Nhưng trong **môi trường software engineering có repository, compiler, test, runtime, Git và verifier**, Nolane Agent có thể làm điều thực tế hơn và có giá trị hơn:
...
> “Hệ thống Nolane Agent dùng model 1B–3B hoàn thành đúng nhiều nhiệm vụ software engineering tương đương hoặc tốt hơn một agent dùng model rất lớn, với ít RAM, token và chi phí hơn.”
...
### Vòng 1 — Kiểm toán Nolane Agent hiện tại
...
- `NOLANE AGENT-FRONTIER-PROGRAM-DESIGN-3.0.0.md`;
- `NOLANE AGENT-FRONTIER-ROADMAP-3.0.0.md`;
...
4. Có thể làm incremental mà không phá Nolane Agent không?
5. Có thể chứng minh bằng ablation và held-out benchmark không?

---

## 2. Nolane Agent 3.0.0 đã rất mạnh, nhưng vẫn chưa đủ để san bằng model
...
Model lớn thường suy ra cấu trúc repository từ text. Nolane Agent không nên bắt model nhỏ làm lại việc đó mỗi lần. Nó phải cung cấp một representation có kiểu:
...
Model lớn có xác suất sinh candidate đúng cao hơn. Nolane Agent có thể bù bằng:
...
Model lớn có kiến thức rộng nhưng thường không nhớ chính xác lịch sử riêng của repository. Nolane Agent có thể tích lũy:
...
Thay vì fine-tune toàn bộ model sau mỗi nhiệm vụ, Nolane Agent có thể thay đổi:
...
**Kết luận:** Nolane Agent nên được thiết kế để 1M–300M module làm phần lớn quyết định rẻ, 1B–3B model làm reasoning/execution, và 7B+/frontier chỉ được escalation ở nút thắt thật sự khó.
...
Một reviewer LLM thường chia sẻ cùng điểm mù với executor. Nolane Agent cần **Verifier Mesh** gồm nhiều loại oracle độc lập:
...
Tiny Recursive Models đạt kết quả đáng chú ý trên ARC/puzzle [S09], nhưng miền này có state/action rất gọn. Nolane Agent nên dùng mạng cực nhỏ cho:
...
Một model lớn dùng token để lặp lại các thao tác logic. Nolane Agent có thể dùng model để **viết hoặc chọn solver**, sau đó solver làm hàng nghìn lần không cần model.
...
### 8.2 Những thứ Nolane Agent nên biên dịch
...
Không nhất thiết phải train một MoE khổng lồ. Nolane Agent có thể tạo MoE ở cấp runtime:
...
AgentArk nghiên cứu việc distill động lực multi-agent vào một agent duy nhất [S11]. Nolane Agent nên:
...
MemRL tách frozen reasoner khỏi evolving memory và học retrieval utility thay vì sửa trọng số [S13]. Đây là tuyến an toàn nhất cho Nolane Agent local.
...
Meta-TTL cho rằng chính policy học trong test-time cũng nên được tối ưu thay vì viết tay [S15]. Nolane Agent có thể học:
...
Tandem nghiên cứu LLM lớn cung cấp insight chiến lược ngắn cho SLM thực thi [S16]. RLM-Cascade cho thấy response-level routing/cascade có thể giảm chi phí đáng kể trong workload coding được nghiên cứu [S17]. Nolane Agent local có thể dùng:
...
ACE xem context như playbook được cập nhật có cấu trúc và báo cáo cải thiện trên agent benchmarks với model nhỏ [S20]. Meta Context Engineering tiếp tục tự tiến hóa skill và context artifact [S21]. Nolane Agent nên có:
...
SWE-Universe báo cáo pipeline tạo hàng trăm nghìn môi trường SWE có verifier từ PR thật [S25]. Nolane Agent không cần quy mô đó ngay; có thể tạo local corpus từ:
...
Open-SWE-Traces cho thấy dữ liệu trajectory SWE đa ngôn ngữ có thể distill năng lực agent mạnh vào model mở hiệu quả [S27]. Nolane Agent nên hỗ trợ ingest dataset công khai nhưng phải chuẩn hóa về schema state/action/effect của chính mình.
...
Nolane Agent nên xem đây là tuyến **nghiên cứu model tương lai**, không phải dependency bắt buộc 3.0.0.
...
### 16.1 Nolane Agent phải tự cải tiến harness như một thí nghiệm khoa học

Agentic Harness Engineering đề xuất ba lớp observability: component, experience và decision; mỗi thay đổi harness phải có prediction rồi đối chiếu outcome [S30]. Đây rất phù hợp với Decision Receipt của Nolane Agent.
...
3. thu verified trajectories từ model lớn và từ Nolane Agent;
...
## 54. Nơi Nolane Agent có thể vượt model lớn sớm nhất
...
Nolane Agent nên dùng escalation thay vì giả vờ model nhỏ đã ngang ở các miền này.
...
5. Tạo benchmark visible/hidden compositional cho Nolane Agent.
...
Để Nolane Agent dùng AI nhỏ nhưng ngang các agent dựa trên model vô cùng lớn, cần thay đổi cách nhìn:
...
Nếu thực hiện đúng, Nolane Agent có thể đạt ba cấp độ:
...
HiPER nghiên cứu hierarchical plan–execute RL và explicit credit assignment ở cả cấp planner lẫn executor [S37]. Nolane Agent có thể áp dụng mà không cần RL ngay từ đầu bằng cách ghi reward riêng:
...
Nolane Agent nên có coverage–risk curve:
...
Pack này là “trí tuệ chuyên biệt” của Nolane Agent, có thể mạnh lên mà không phình base model.
...
Ngoài adapter update, Nolane Agent nên nghiên cứu policy optimization training-free:
...
Đây là một lợi thế lớn cho model nhỏ: nó có thể tạo đáp án đúng hiếm hơn, nhưng Nolane Agent không được đánh mất đáp án đó sau khi đã tìm thấy.
...
> Các kết quả dưới đây là bằng chứng định hướng, không phải bảo đảm chúng sẽ tái hiện nguyên vẹn trong Nolane Agent. Mỗi hướng phải được kiểm tra lại bằng benchmark của dự án.
...
## Alpha.5 implementation update (2026-08-01)

Alpha.5 turns additional research proposals into bounded, testable infrastructure: matched-budget and same-quality-cost benchmark gates; quantization and OOD transfer gates; token-aware codemods; finite-domain SMT and stratified Datalog adapters; lazy file-backed specialist artifacts with read-only mmap contracts; multi-agent policy distillation; verified-outcome adaptation; and latent-memory expert routing with abstention and negative-transfer rollback. These modules establish measurement and control infrastructure only. No trained Nolane foundation model exists, and no result authorizes a parity, superiority or AGI claim.
...
## Beta.3 harness/runtime update (2026-08-01)

The beta.3 runtime wave strengthens the harness around any future small model without claiming a trained model exists. Provider streams are normalized into typed text/tool events; ACP requests are ordered, cancellable and replay-safe; delegation contexts are byte-bounded with explicit omissions; repository evidence is path-safe and content-addressed; browser mutations require approval; gateway delivery is idempotent; and usage/cost receipts form a tamper-evident chain. These controls reduce the amount of reliability a small model must learn implicitly, but they are infrastructure rather than intelligence results. Provider-real and same-condition comparative evaluation remain external.
...
## Beta.6 harness/runtime update (2026-08-01)

The beta.6 runtime wave further reduces hidden reliability burdens on any future small model. MCP authorization is explicit and replay-safe; browser and PTY actions are serialized behind bounded state machines; delegation logs and leases persist across restart; completion requires independent verification; gateway pressure and stale-heartbeat state are observable; media identity is content-addressed. These controls improve the harness around a model but are not model-training or intelligence evidence. `completeParityClaimAllowed=false` and `superiorityClaimAllowed=false` remain mandatory.
