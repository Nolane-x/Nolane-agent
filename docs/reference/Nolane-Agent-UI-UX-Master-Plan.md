...
# Nolane Agent 3.0.0 — UI/UX Master Architecture & Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use `superpowers:subagent-driven-development` (recommended) or `superpowers:executing-plans` to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Tái kiến trúc toàn bộ giao diện Nolane Agent 3.0.0 thành một sản phẩm AI-agent cao cấp, trang trọng, dễ dùng ở tầng ngoài, cực sâu ở tầng quản trị, nhưng nhẹ và phản hồi nhanh hơn đáng kể so với các desktop agent nặng.

**Architecture:** Nolane Agent 3.0.0 dùng kiến trúc hai tầng rõ ràng. **Tầng 1 — Forge Workspace** là mặt làm việc hằng ngày, chỉ hiển thị nhiệm vụ, hội thoại, hoạt động, sản phẩm đầu ra, review và workroom khi cần. **Tầng 2 — Forge Control Plane** chứa toàn bộ runtime, context, memory, evidence, repository intelligence, trust, governance, agent modes, local operations và nghiên cứu; nó được tải lười, truy cập có chủ đích và không làm nặng shell chính.
...
- Không viết lại Nolane Agent bằng React, Vue hoặc framework UI có virtual DOM trong đợt 3.0.0.
...
- Tầng 2 phải giữ đủ dữ liệu, bằng chứng, trạng thái và quyền kiểm soát cho nhà nghiên cứu, quản trị viên và người phát triển Nolane Agent.
...
Nolane Agent không nên tự định vị là “một IDE có thêm AI” và cũng không nên biến thành “một dashboard quản trị agent”. Định vị đúng là:

> **Nolane Agent là một command workspace cho AI engineering: đơn giản để giao việc, rõ ràng để giám sát, nghiêm ngặt để kiểm chứng và đủ sâu để quản trị toàn bộ hệ thống agent local.**
...
### Vòng A — Đọc mã nguồn Nolane Agent 2.22.0
...
`CENTER_SPECS` cho thấy Nolane Agent hiện có các center:
...
**Bài học cho Nolane Agent:** Home không phải nơi để đặt mọi dashboard. Sidebar nhiệm vụ và task workspace mới là xương sống.
...
**Bài học:** Nolane Agent không nên chọn giữa “chat app” và “IDE”. Nó cần một workspace agent-first, với Workroom mở ra khi nhiệm vụ thật sự cần code-level control.
...
Inspector hiện tại của Nolane Agent đã gần đúng. 3.0.0 cần nâng nó thành **Artifact Dock** với route theo context, tab ít hơn và summary tốt hơn.
...
Windsurf Plan Mode tạo Markdown plan có nút Implement. Kiro Specs tạo requirements/design/tasks artifacts. Nolane Agent đã có Goal OS và plan history, mạnh hơn về chiều sâu nhưng presentation còn kỹ thuật.
...
**Bài học:** Nolane Agent cần task status taxonomy chuẩn, thống nhất toàn sản phẩm:
...
**Bài học:** Nolane Agent không nên vẽ swarm animation. Tầng 1 chỉ cần:
...
**Bài học:** Nolane Agent đã có nền governance tốt; UI phải chuyển nó thành lời hứa dễ hiểu:
...
| Qoder Quest | Ba cột rất rõ, status board, review/terminal/browser theo nhiệm vụ | Không sao chép thuật ngữ Quest/Experts; Nolane Agent có hệ thống Goal OS riêng |
| CodeBuddy | Sidebar + conversation + context, multi-task dễ hiểu, Programming/Work mode | Không trộn quá nhiều tác vụ office vào IA coding chính nếu Nolane Agent chưa ưu tiên |
...
Nolane Agent có đủ backend và center để đi xa hơn họ về trust, evidence và governance. Việc còn thiếu là một lớp trải nghiệm làm những năng lực đó trở nên yên tĩnh, rõ ràng và có thứ bậc.

---
# Phần II — Kiến trúc trải nghiệm Nolane Agent 3.0.0
...
Nolane Agent phải tạo cảm giác:
...
`ui-ux-pro-max-skill` cung cấp thư viện phong cách, palette, typography, UX guidelines và đặc biệt kiến trúc token ba tầng. Nolane Agent không nên chọn một style pack nguyên khối. Ta chỉ lấy các nguyên tắc phù hợp:
...
Nolane Agent có hai density profile, không phải theme:
...
Electron official guidance nhấn mạnh đo đạc, tránh tải/chạy code quá sớm, không block main/renderer, giảm network và bundle code. Nolane Agent có lợi thế local asset và ESM. 3.0.0 phải giữ lợi thế đó.
...
Nolane Agent là desktop-first, nhưng phải làm việc ở cửa sổ nhỏ:
...
- [ ] Commit `test(ui): gate Nolane Agent 3.0 experience`.
...
# Phần XIII — Định nghĩa “UI triệu đô” cho Nolane Agent
...
UI triệu đô của Nolane Agent được đo bằng:
...
Đó là lợi thế mà Nolane Agent có thể vượt các agent UI khác: **đơn giản ở bề mặt nhưng không nghèo nàn; sâu ở bên trong nhưng không nặng nề.**
...
## 48. Dữ liệu thực tế từ Nolane Agent 2.22.0
...
Chỉ tám thay đổi này đã làm Nolane Agent trông như một sản phẩm agent cấp cao thay vì một tập hợp center.
...
Khi thực hiện đúng, tầng ngoài của Nolane Agent sẽ dễ dùng và thanh lịch như các agent app tốt nhất, trong khi tầng trong vẫn sâu hơn đáng kể. Đây không phải thiết kế “giống Codex/Cursor/Claude Code”. Đây là kiến trúc để Nolane Agent trở thành một **AI engineering workspace có control plane riêng**, với vẻ ngoài điềm tĩnh, tốc độ local và độ kiểm chứng mà sản phẩm khác thường giấu hoặc không có.
...
## Alpha.5 UI and product-quality update (2026-08-01)

Nolane Agent 5.0.0-alpha.5 adds active-brand auditing across the current shell, CLI, desktop and documentation surfaces; deterministic onboarding/examples; an offline SVG screenshot reference; semantic tab relationships, focus-visible and live-region contracts; reduced-motion CSS; and explicit responsive breakpoints at 640, 900, 1180 and 1440 px. These are source-level and static-quality guarantees. The audit deliberately reports `runtimeCertification=false` until Windows keyboard, screen-reader, contrast, visual-regression, DOM, CPU, memory and latency measurements are executed on the reference environment.
...
## Beta.3 native runtime surface update (2026-08-01)

Beta.3 moves eight previously residual runtime behaviors behind one shared Nolane-native orchestration boundary: ACP streaming, provider protocol assembly, repository intelligence, delegation context, browser/search execution, gateway adapters, command surfaces and usage accounting. Electron, HTTP, CLI and TUI must project the same ordered receipts rather than maintain private execution state. Browser mutations remain approval-gated; provider credentials remain references only; terminal output is ANSI-sanitized and bounded. This update raises local native-core conformance to 34 of 61 behavior contracts and 240 of 2,110 pinned upstream behavior paths. It does not replace Windows keyboard, screen-reader, responsive, visual or resource certification.
...
## Beta.6 native runtime conversion update (2026-08-01)

Beta.6 adds six production-wired Nolane-native runtime surfaces beneath the same Electron, HTTP, CLI and TUI projections: one-time PKCE MCP authorization, serialized browser supervision and dialog recovery, persistent async delegation logs and leases, bounded PTY replay and retry state, gateway heartbeat/memory/drain forensics, and content-addressed local media playback with barge-in. UI surfaces must consume the shared `runtimeWave6` snapshot rather than maintain private state. Local conformance is 52 of 75 behavior contracts and 413 of 2,110 pinned upstream paths. Provider-real, Windows and GUI certification remain external. `completeParityClaimAllowed=false` and `superiorityClaimAllowed=false`.
