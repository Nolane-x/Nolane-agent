# NolaneGuard Local & ForgeStudio 3.4.0 — Báo cáo dogfooding và kiểm toán độc lập nhiều vòng

**Ngày thực hiện:** 2026-07-31
**Đối tượng chính:** ForgeStudio 3.4.0 + ForgeStudio-LegacyExternalRuntime 2.29.0
...
### 1.2. ForgeStudio đã được chứng minh mạnh hơn NolaneNative chưa?

**Chưa.** Audit này chứng minh ForgeStudio có nhiều cơ chế governance/receipt/boundary thật. Nhưng vì NolaneNative autonomous runtime không khởi động được từ pack trong môi trường sạch, không thể làm phép so sánh công bằng end-to-end giữa hai agent trên cùng nhiệm vụ.

Kết luận chính xác là:

- **ForgeStudio không giả.**
...
| Artifact | SHA-256 |
|---|---|
| `ForgeStudio-3.4.0-source.zip` | `a2056bdf494b8b29a0463929032175b35f3e072546ce7206d5c18c06bde934f8` |
| `ForgeStudio-LegacyExternalRuntime-2.29.0.zip` | `fce31d6af6e6f9976b562d46d75f26bdfa203d2caddb910cf37c658ffcbc08ed` |
...
| 2 | Tái lập test ForgeStudio, eval, benchmark và feature-claim reachability. |
| 3 | Xây NolaneGuard Local qua ForgeStudio Workroom API theo TDD. |
...
| 9 | Phá boundary của chính ForgeStudio và prompt-injection path. |
| 10 | Mutation testing + API fuzzing. |
| 11 | Restart, concurrency, corruption và persistence-failure stress. |
| 12 | TDD remediation qua ForgeStudio Workroom API. |
...
ForgeStudio trả mã lỗi tương ứng `NOLANE_NATIVE_DEPENDENCY_MISSING`.
...
| ForgeStudio tìm và kiểm soát pack | `END_TO_END_VERIFIED` |
...
## 6. Loop 2 — ForgeStudio baseline, test và claim audit
...
## 7. Loop 3 — Xây NolaneGuard Local qua ForgeStudio
...
- tất cả file test và production được ghi qua `PUT /api/workroom/file` của ForgeStudio;
- project, mission, governance envelope, receipt, file hashes, workroom tree và repository index đi qua đường production ForgeStudio;
...
### 7.3. ForgeStudio subsystem được quan sát
...
### 7.4. Lỗi API semantics của ForgeStudio
...
## 13. Loop 9 — Phá chính ForgeStudio
...
### 13.4. Sửa cần thiết cho ForgeStudio
...
### 15.3. ForgeStudio restart

ForgeStudio server bị terminate và start lại:

- trước: 3 project;
- sau: 3 project;
- toàn bộ ID giống nhau.

Persistence/restart recovery cơ bản của ForgeStudio được phân loại `END_TO_END_VERIFIED`.

---

## 16. Loop 12 — Remediation qua ForgeStudio Workroom API
...
Các fix đều được ghi qua ForgeStudio `PUT /api/workroom/file` với expected hash:
...
## 18. Những gì trong ForgeStudio là thật, yếu, chưa nối hoặc chưa chứng minh
...
## 19. Điểm mạnh thật sự của ForgeStudio
...
## 20. Điểm yếu nghiêm trọng của ForgeStudio
...
### ForgeStudio instruction trust/quarantine reachability
...
| `evidence/loop-11-recovery-stress/` | Baseline stress + ForgeStudio restart |
...
2. Không có provider credential để chạy LLM qua ForgeStudio.
...
Kết quả này chứng minh package NolaneGuard Local tái lập được mà không phụ thuộc Git metadata. Nó không khắc phục vấn đề packaging của chính ForgeStudio/NolaneNative; đó vẫn là finding độc lập.
...
Điểm quan trọng nhất không phải thêm hàng trăm module mới. ForgeStudio cần:
...
> **Một ứng dụng local nhỏ, thực sự chạy được, được xây qua ForgeStudio Workroom boundary, bị phá thành công ở baseline, được harden bằng TDD và vượt lại cùng attack suite.**

Nó là bằng chứng rằng ForgeStudio APIs có thể tham gia tạo sản phẩm thật. Nó **không** phải bằng chứng NolaneNative tự xây, cũng không phải bằng chứng ForgeStudio đã vượt mọi agent.
