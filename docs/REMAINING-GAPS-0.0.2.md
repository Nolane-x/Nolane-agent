# Nolane Agent 0.0.2 — Remaining Gaps Report

- Tổng yêu cầu: **1150**
- Còn lại: **391**
- Hoàn thành một phần: **7**
- Cổng bên ngoài: **56**
- Chưa triển khai: **328**
- Audit SHA-256: `a0fe082cd5e14113eddf39073ebf429bc9c9763bbfdcbd52c72dacd23080dcd8`
- Receipt SHA-256: `86cb93f7c793b220a690e7f3bfd97362f76de21e4060ffc96eaf0ae653a9380b`
...
- **Trạng thái:** `partial`
- **Vì sao chưa hoàn tất:** Có một phần hành vi và test liên quan trong 0.0.2, nhưng acceptance criteria frontier đầy đủ chưa được đóng nên chưa được tính hoàn thành.
...
### 31.1 — Tạo interface embedding provider nhiều backend với capability probe rõ ràng.
...
### 31.3 — Giữ FeatureHashEmbeddingProvider làm fallback siêu nhẹ và ghi rõ degraded mode.
...
### 31.7 — Dùng retrieval hai tầng: FTS/BM25/graph thu hẹp rồi semantic rerank.
...
### 31.8 — Giới hạn semantic rerank vào khoảng 100–300 candidate thay vì toàn repository.
...
### 31.10 — Thêm local reranker nhẹ và fallback deterministic khi model không sẵn sàng.
...
### 31.13 — Tái sử dụng index giữa branch/worktree gần giống bằng content-addressed chunks.
...
### 31.14 — Dùng Merkle tree theo file, syntax chunk và symbol để chỉ embedding phần đổi.
...
### 31.15 — Version hóa embedding schema và rebuild có kiểm soát khi model đổi.
...
### 32.1 — Mô hình hóa Workspace, Repository, Package, Module, File và Symbol bằng ID ổn định.
...
### 32.14 — Tạo Architecture Drift Sentinel cho cycle, layer violation và duplicated logic.
...
### 33.14 — Tạo framework adapter có capability probe thay vì heuristic im lặng.
...
### 33.15 — Phân biệt source, generated, vendored, migration, lockfile và build output.
...
### 33.16 — Giữ ambiguity dưới dạng evidence, không đoán edge khi resolution không chắc.
...
### 33.17 — Có degraded fallback cho ngôn ngữ chưa hỗ trợ nhưng không tuyên bố parity.
...
### 33.18 — Yêu cầu target-platform runner và raw evidence trước production claim.
...
### 34.1 — Duy trì Context Posterior với nhiều cách hiểu task và probability được cập nhật.
...
### 34.2 — Không ghi memory dài hạn khi context posterior còn phân tán vượt ngưỡng.
...
### 34.3 — Duy trì Polyhypothesis Workspace tối đa 2–3 hypothesis đang hoạt động.
...
### 34.4 — Mỗi hypothesis có prediction, support, counter-evidence và falsification condition.
...
### 34.5 — Chọn probe theo Expected Information Gain trừ token, RAM, time và risk.
...
### 34.6 — Định tuyến lỗi theo perception, context, memory, causal, planning, execution, verification và goal.
...
### 34.7 — Chỉ cập nhật subsystem có posterior trách nhiệm cao bằng owner mask.
...
### 34.8 — Ghi Agency Ledger Intent→Action→Expected Effect→Actual Effect→Responsibility.
...
### 34.10 — Đóng gói episode nhân quả thay cho transcript bằng Episodic Binder.
...
### 34.15 — Ghi và cấm lặp lại strategy đã thất bại trong cùng recovery lease.
...
### 34.17 — Chỉ COMMIT khi hypothesis ưu thế, scope giới hạn và probe xác minh đã biết.
...
### 34.18 — Dừng khi criteria đã có receipt hoặc evidence mới không còn đủ information gain.
...
### 35.1 — Biên dịch yêu cầu tự nhiên thành specification có goal, criteria, non-goal và constraint.
...
### 35.2 — Phát hiện mâu thuẫn nghiêm trọng trong specification trước khi cho phép edit.
...
### 35.3 — Tạo Requirement Traceability Matrix requirement→decision→symbol→test→receipt.
...
### 35.4 — Duy trì Invariant Ledger với owner, severity và verifier.
...
### 35.5 — Lập Hierarchical Mission Plan mission→milestone→capability→contract→verification.
...
### 35.8 — Biến mỗi plan step thành Executable Plan có allowedFiles và forbiddenChanges.
...
### 35.9 — Tạo state capsule sau mỗi milestone để resume không cần đọc lại toàn chat.
...
### 35.10 — Revalidate assumption, repository map và dependency sau patch lớn.
...
### 35.14 — Chọn candidate theo correctness rồi semantic footprint và resource yield.
...
### 35.16 — Dùng Prospective Memory cho obligation phụ thuộc trạng thái tương lai.
...
### 35.17 — Giải quyết goal conflict, phân biệt hard constraint và trade-off có thể thương lượng.
...
### 35.18 — Xuất Completion Proof Bundle gồm criteria, symbols, tests, risks và rollback point.
...
### 36.1 — Tính semanticFootprint từ symbol, API, dependency, schema, control flow và permission.
...
### 36.2 — Đặt dynamic patch budget theo bug, feature, refactor, migration và security risk.
...
### 36.3 — Yêu cầu split task hoặc evidence khi patch vượt budget.
...
### 36.8 — Cấm opportunistic refactor ngoài phạm vi bugfix.
...
### 36.9 — Ghi changed, reverted và re-applied lines theo correction lineage.
...
### 36.10 — Không sửa generated code; phải sửa generator và tái sinh artifact.
...
### 36.15 — Phát hiện quyền filesystem/network hoặc side effect mới từ patch.
...
### 45.3 — Chạy Nolane Agent và đối thủ với cùng model, máy, token, time và permissions.
