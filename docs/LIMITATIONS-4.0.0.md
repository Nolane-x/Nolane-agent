# Forge Studio 4.0.0 — Limitations and non-claims

## Retained 3.3.0 boundary corpus

## World model and developmental learning boundaries

- Forge Studio **does not allow world models to commit files** or write durable memory.
- Forge Studio **does not treat simulation as observed evidence**; a real verification receipt remains required.
- Forge Studio **does not autonomously promote production policy** or rewrite its own core.
- Forge Studio **does not certify open-world long-term learning** from this bounded local release.
- Forge Studio **does not allow autotelic goals to delay critical missions** or bypass hard constraints.
- Forge Studio **does not claim AGI** and does not claim comparative superiority from world-model fixtures.

## Inherited 2.28 boundaries

- Forge Studio 2.28.0 does not certify direct host fault injection; adversarial fault scenarios are adapter-driven and bounded.
- Forge Studio 2.28.0 does not certify real competitor benchmark runs; no Codex, Claude Code, Cursor, or Copilot raw run is present in this release evidence.
- Forge Studio 2.28.0 does not certify a private held-out suite; contamination controls and schema contracts exist, but external private cases are absent.
- Forge Studio 2.28.0 does not certify a complete platform matrix; macOS and cross-platform sandbox certification remain external gates.
- Forge Studio 2.28.0 does not claim comparative superiority over any external agent.

## Collaboration and experience boundaries

- This release does not certify a visual oracle; screenshots and video hashes remain journey evidence, not proof of visual correctness.
- This release does not certify JetBrains parity. The production integration evidence covers the local web UI and VS Code source bridge only.
- This release does not certify cross-platform computer use on Windows, macOS, and Linux; browser replay is adapter-driven and locally deterministic.
- Adaptive topology returns governed recommendations and does not automatically create an unbounded swarm.
- Semantic merge detects evidence-backed contract and behavior conflicts but is not a universal program-equivalence proof.
- Review, playback, and steering create governed decisions or plans; they do not auto-merge, execute rewind, or run irreversible commands.
- This release does not claim benchmark superiority over Codex, Claude Code, Cursor, Copilot, or other external agents.

The item-level audit source is `docs/feature-audit-2.28.0.json`. The exhaustive incomplete and externally gated inventory is `docs/REMAINING-GAPS-2.28.0.md`.

# Retained platform, runtime, security, and release boundaries

## Memory, Skill, and Resource OS boundary

Forge Studio 2.28.0 implements governed memory lifecycle operations and shadow policy decisions. It does not change production memory policy automatically; a policy decision remains advisory until an explicit governed caller commits an allowed operation.

The compositional skill compiler produces typed, receipt-bound drafts and the registry requires transfer evidence. Forge Studio does not automatically promote skills, grant new capabilities, weaken verification, or expand filesystem/network authority.

Resource leases, process-tree evidence, and mission-owned cleanup are source-tested on the release runner. This does not certify direct OS budget enforcement on every target platform, kernel, container, process tree, file-descriptor implementation, or shared-memory accounting mode.

Replay and Stability–Plasticity checks use bounded verified outcome summaries. This release does not certify long-term skill survival, 7–30 day patch survival, broad cross-repository generalization, production neural embedding unload, or weight-level continual learning.

The content-addressed artifact store keeps raw bytes on disk and exposes bounded projections. Existing historical log stores are not all migrated to this store in 2.26.0, so requirement 40.12 remains partial.

The deterministic local measurement is release-contract evidence. Forge Studio 2.28.0 does not claim benchmark superiority over Cursor, Codex, Claude Code, Copilot, Devin, or another agent.

The item-level source of truth is `docs/feature-audit-2.28.0.json`. The exhaustive open-item report is `docs/REMAINING-GAPS-2.28.0.md`.

---

# Retained Forge Studio 2.25.0 limitations

## Long-Horizon Construction Engine boundary

Forge Studio 2.25.0 compiles structured requirement objects and verifies their internal consistency. It does not claim universal natural-language understanding or that every user intention has been decomposed perfectly.

The candidate selector requires 2–3 candidates with isolation receipts, but it does not create candidate worktrees directly. Worktree creation, cleanup and exact Git isolation remain responsibilities of a governed adapter and its receipts.

State capsules are written atomically and reject known repository, plan, invariant and Git-checkpoint drift. This does not certify cross-reboot recovery on every operating system, filesystem, crash point or storage failure mode.

Semantic Patch Intelligence scores known symbol, API, schema, dependency, control-flow, permission, generated-code and test-integrity evidence. It is not a complete causal simulator and cannot detect unknown dynamic behavior that has no static or runtime evidence.

Impact-based verification selects required stages but does not execute commands itself. Candidate comparison trusts supplied verification and isolation receipts only after their governing adapters validate them.

The construction control plane grants or denies bounded authorization; it does not mutate files directly, merge branches, create hosted pull requests or weaken acceptance criteria.

The synthetic measurement in `docs/long-horizon-construction-measurement-2.25.0.json` is deterministic Core contract evidence. Forge Studio 2.25.0 does not claim benchmark superiority over Cursor, Codex, Claude Code, Copilot, Devin or another agent.

The item-level source of truth is `docs/feature-audit-2.25.0.json`. The exhaustive incomplete and externally gated list is `docs/REMAINING-GAPS-2.25.0.md`.
...
Nolane Native is the only product runtime owned and shipped by Nolane Agent. External runtime archives, compatibility packs, adapters and executable paths are excluded; runtime purity is verified for source and release artifacts.
...
## Polyglot Runtime Intelligence boundary

Tree-sitter is not operated on the Linux release runner used for this release. The grammar-pack registry, version/digest checks and parse contract are source-tested, but no production Tree-sitter grammar pack is certified by 2.22.0.

The LSP fixture proves JSON-RPC framing and the definition, reference, hover, rename, type-definition, diagnostics and call-hierarchy contracts. Fixture LSP behavior is not production certification of TypeScript, Python, Rust, Go, Java or another language server.

The release runner probes installed `clangd` and `sourcekit-lsp` binaries. A successful version probe is capability evidence, not proof of every workspace, compiler database, SDK or platform configuration.

Control-flow and interprocedural data-flow graphs are not complete in this release. Build, test, type and runtime relations are fused only when exact evidence is supplied; unresolved dynamic dispatch remains ambiguous and is not guessed.

Runtime observations accept permission-bound events from operated callers. The in-memory measurement is not proof that every sandbox, browser, database driver or framework has been instrumented.

Forge Studio 2.22.0 does not claim to outperform Cursor, Codex, Claude Code, Copilot or Devin. Comparative superiority still requires independent unseen-repository benchmarks using the same model, tokens, hardware and time budget.

The item-level source of truth is `docs/feature-audit-2.23.0.json`. The exhaustive open-item report is `docs/REMAINING-GAPS-2.23.0.md`.

## Cognitive Decision Kernel boundary

The Cognitive Decision Kernel stores structured public decision metadata and content-addressed receipts. It does not store chain-of-thought, private reasoning, raw prompts, raw model output, credentials, authorization headers, cookies, or environment dumps.

The kernel authorizes or denies a bounded proposal. It does not directly mutate source files, write semantic memory, change permissions, promote skills, merge branches, or perform irreversible actions. Source mutation remains governed by the existing policy, patch, worktree, verification, and human-control layers.

Forge Studio 2.23.0 does not learn or rewrite its own policy. Context probabilities, hypothesis probabilities, error routing weights, and action scores are deterministic bounded policies in this release. Shadow evidence does not autonomously alter routing or promotion.

The Agency Ledger records intent, a command fingerprint, expected effect, actual effect, controllability and responsible actor. It does not store the raw command. Expected-versus-actual mismatch is evidence for later diagnosis; 2.23.0 does not yet provide a complete automatic false-success classifier for every tool.

Causal intervention is not production-complete. The release binds causal episodes and expected-versus-actual outcomes, but it does not operate a general causal intervention sandbox, learned world model, or counterfactual simulator.

Trajectory confidence calibration, weakest-link final confidence, executable plan state machines, and semantic/test/diff/information-gain no-progress detection remain partial.

Forge Studio 2.23.0 does not claim to outperform Cursor, Codex, Claude Code, Copilot, Devin, or another agent. Comparative superiority still requires independent unseen-repository benchmarks using the same model, token, hardware, time and human-intervention budgets.


## Verification and learned routing boundaries

- This release does not certify production multi-provider reviewer independence; deterministic adapters prove selection and disagreement contracts only.
- This release does not include a hidden hosted regression set that is inaccessible to the executor.
- This release does not certify long-term patch survival over 7–30 days.
- The verified-outcome bandit does not change production routing; it remains shadow-only behind existing hard constraints.
- Failure injection uses bounded adapters and does not certify direct operating-system fault injection across every platform.
- The release does not claim benchmark superiority over Cursor, Codex, Claude Code, Copilot, Devin, or any other agent.

## Current release sources of truth

- `docs/feature-audit-3.3.0.json`
- `docs/REMAINING-GAPS-3.3.0.md`

## Frontier Governance 3.0 non-claims

- Forge Studio 3.3.0 does not autonomously merge or publish cross-repository changes.
- Forge Studio 3.3.0 does not execute production policy promotion; constitutional authorization still requires human-controlled execution.
- Forge Studio 3.3.0 does not certify real 7–30 day field survival from the deterministic release fixture.
- Forge Studio 3.3.0 does not expand autonomy or capability through self-improvement candidates.
- Forge Studio 3.3.0 does not claim frontier superiority over Codex, Claude Code, Cursor, Copilot, or any other system.
- Cross-platform hosted recovery, JetBrains parity, production competitor runs, and independent comparative attestation remain external gates.

## Intelligence Completion Kernel 3.1 non-claims

- The 63 `external_gate` requirements are unchanged. Forge Studio does not claim evidence that requires independent operators, external cloud environments, partner systems, production traffic, or third-party attestations.
- The new services do not autonomously merge, publish, deploy, mutate production, disable verification, expand permissions, or promote policy.
- Counterfactual patch ablation works only through a caller-supplied isolated-workspace adapter and never applies the result to the original workspace.
- Dynamic program-analysis targets remain ambiguous when evidence is insufficient; the kernel does not guess them.
- Commit/architecture and issue/code links are observed or inferred relations, not proof of causality or defect location.
- Verified context utility is local evidence routing, not autonomous self-modification. Unverified outcomes cannot change learned utility.
- The deterministic measurement is a synthetic, in-memory release fixture. It is not an independent production benchmark and does not claim superiority over Cursor, Codex, Claude Code, GitHub Copilot, or any other system.
- The paged vector measurement proves bounded page reads in the fixture; it is not a claim of operating-system `mmap`, universal scale, or production latency.

## Legacy external runtime certified carry-forward boundary

- The 3.3.0 carry-forward gate does not claim that `nolane_native-agent-main.zip` was opened, decompressed, or re-executed in the current run. Its report explicitly records `archiveReadInCurrentRun: false`.
- Any protected-file, archive SHA, byte-count, required-entry, license, upstream-commit, matrix-receipt, or gate-receipt drift fails closed and requires direct archive verification or a new certification.
- Forge Studio 3.3.0 reuses `ForgeStudio-LegacyExternalRuntime-2.16.0.zip`; it does not publish a new NolaneNative pack or claim that the older artifact is bundled in the 3.3.0 core archives.
- The carry-forward certificate is not a third-party signature or independent attestation. It preserves Forge Studio's own prior content-addressed release evidence and does not satisfy external certification gates.


## Verified Mission Runtime 3.2 boundaries

- Verified outcomes only create score or learning value after a passing verification receipt; an unverified outcome does not create value.
- Actual tool effects require independent probes. A tool self-report is not accepted as effect evidence, and a false-success result does not authorize commit.
- Confidence calibration is local to lane, domain, and task kind. It is not a guarantee of correctness and does not replace verification.
- The process reaper does not kill unregistered processes. It fails closed on PID identity drift or any process outside the registered mission set.
- Raw mission logs are stored on disk with redaction and checksum chaining. RAM snapshots retain summaries, cursors, and offsets rather than raw records.
- The 3.2 measurement is deterministic and bounded except that Linux cleanup evidence runs a real local child tree and records only stable pass/fail boundaries, not timing or PID values.
- This release does not claim comparative superiority, independent production benchmark results, AGI, autonomous merge, or autonomous publish.

## Repository Truth Plane 3.3.0 boundaries

- Forge Studio **không tuyên bố Repository Truth Plane hiểu hoàn hảo mọi ngôn ngữ**; khi AST/LSP hoặc runtime adapter không có bằng chứng, stage phải hiện `unavailable` hoặc quan hệ phải ở trạng thái mơ hồ.
- Forge Studio **không tuyên bố quan hệ runtime là nhân quả nếu không có observation citation** độc lập gắn source hash.
- Forge Studio **không tuyên bố vượt Cursor, Codex hoặc Claude** từ measurement nội bộ này.
- Paged Twin Viewer **không tải toàn bộ graph** cho một page; cursor bị khóa vào đúng twin, level và parent query.
- 63 mục `external_gate` vẫn giữ nguyên; Repository Truth Plane không tự đóng các điều kiện cần cloud, đối tác, hệ điều hành hoặc đánh giá độc lập.

---

## Construction Safety Completion 3.4.0 boundaries

Forge Studio 3.5.0 hoàn thiện một cụm local Construction Safety Completion, nhưng vẫn giữ **63 external gate** và không biến chúng thành claim đã hoàn thành.

- Bản phát hành này **không tuyên bố vượt** Cursor, Codex, Claude Code, Copilot hoặc bất kỳ đối thủ nào.
- Mô phỏng và can thiệp nhân quả không phải bằng chứng production; chỉ outcome có verification receipt mới được dùng làm bằng chứng quan sát.
- Hidden regression được mã hóa để executor không đọc expected value; điều này không chứng minh bài test là không thể bị suy đoán hoặc đã chống mọi dạng contamination.
- Candidate worktree không tự động merge. Việc chọn candidate và merge vẫn chịu verification contract, independent review và quyền kiểm soát của người dùng.
- Independent reviewer trong runtime là contract về identity/provider và receipt; chứng nhận tổ chức hoặc bên thứ ba vẫn là external gate.
- Mutation probe chỉ chứng minh mutation cụ thể bị test bắt và file được phục hồi byte-chính-xác, không chứng minh toàn bộ test suite không có lỗ hổng.

## Audit references

- Item-level audit: `docs/feature-audit-3.5.0.json`
- Complete remaining-gaps report: `docs/REMAINING-GAPS-3.5.0.md`

## Retained historical boundaries

- Forge Studio does not autonomously merge or publish changes.
- Unverified outcome does not create value in the verified-outcome ledger.
- Unregistered process is not killed by the process leak reaper.
- Deterministic measurement is local, bounded and reproducible; it is not an independent production benchmark.
- Forge Studio does not perfectly understand every language.
- Forge Studio does not claim causal truth from an observation citation.
- The paged viewer does not load the entire graph for one page.


## Adaptive Learning & Trust Fabric 3.5.0 boundaries

- Adaptive Learning & Trust Fabric **does not change production routing automatically**; cohort canaries may disable a candidate but cannot promote it.
- It **does not use held-out tasks for tuning**; tuning and held-out task sets are required to be disjoint.
- It **does not certify long-term patch survival without delayed verified observations** from the bounded 7–30 day window.
- It **does not expose teacher oracle to executor**; challenge answers remain separated from executor payloads.
- It **does not claim benchmark superiority** over any model, agent, IDE, or competing product.

## Local Frontier Completion 4.0.0 boundaries

- Forge Studio 4.0.0 completes every locally provable partial requirement, but **does not bundle a production ONNX INT8 embedding model**; requirement 31.2 remains an external gate until a licensed signed model/tokenizer pack is supplied.
- Forge Studio 4.0.0 **does not bundle production tree-sitter grammars or language-server binaries** for every supported language; requirements 33.1–33.4 remain external gates rather than being satisfied with fixtures.
- The local polyglot evidence runtime records cited static and observed runtime relations; it **does not claim perfect semantic understanding of every language**.
- The private held-out benchmark encrypts verifier oracle data and keeps it out of executor projection; it **does not prove immunity to every contamination channel**.
- Forge Studio 4.0.0 **does not claim benchmark superiority** and requirement 45.3 remains external until a competitor run uses the same model, machine, token, time, permissions, and independent attestation.
- Local visual regression requires a human-approved baseline for critical oracles; reviewed ignore regions are evidence, not proof of universal visual correctness.
- Failure injection demonstrates containment of the enumerated scenarios in the local harness; it is not a certification for every operating system, driver, or production deployment.

## Audit references for 4.0.0

- Item-level audit: `docs/feature-audit-4.0.0.json`
- Complete remaining-gaps report: `docs/REMAINING-GAPS-4.0.0.md`
- Local completion measurement: `docs/local-frontier-completion-measurement-4.0.0.json`
