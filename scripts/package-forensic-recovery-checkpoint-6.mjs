...
import { createCheckpoint6DeliveryPlan, createCheckpoint6VerificationReport } from '../src/forensics/checkpoint-6-delivery-plan.mjs';
...
const CHECKPOINT = path.join(ROOT, 'docs/checkpoints/NOLANE-FORENSIC-RECOVERY-CHECKPOINT-6.json');
...
const temp = await mkdtemp(path.join(os.tmpdir(), 'nolane-forensic-cp6-'));
...
const copies = [
  ['docs/checkpoints/NOLANE-FORENSIC-RECOVERY-CHECKPOINT-6.md', 'CHECKPOINT.md'],
  ['docs/checkpoints/NOLANE-FORENSIC-RECOVERY-CHECKPOINT-6.json', 'CHECKPOINT.json'],
  ...
  ['datasets/trajectories/multi-runtime-v1/execution-receipt.json', 'multi-runtime-execution-receipt.json'],
  ['datasets/trajectories/multi-runtime-v1/recovery-receipt.json', 'mutation-recovery-receipt.json'],
];
...
await writeFile(output('checkpoint-6-specialist-suite-verification.json'), `${JSON.stringify(suite.verification, null, 2)}\n`);
...
await writeFile(output('third-party-provenance.json'), `${JSON.stringify(checkpoint.thirdPartyProvenance, null, 2)}\n`);
const matrix = JSON.parse(await readFile(path.join(MATRIX_ROOT, 'full-release-matrix.json'), 'utf8'));
await writeFile(output('VERIFICATION-REPORT.md'), createCheckpoint6VerificationReport({ gitHead: HEAD, baselineCommit: BASELINE, checkpoint, matrix }));
...
const manifest = await createDeliveryManifest({ root: OUTPUT, artifactPaths, checkpoint: 'forensic-recovery-checkpoint.6', gitHead: HEAD });
