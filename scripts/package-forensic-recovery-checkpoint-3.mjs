...
const PREFIX = 'NolaneAgent-5.0.0-beta.6-forensic-recovery-checkpoint.3';
const BASELINE = process.env.FORENSIC_BASELINE_COMMIT || '9559942c412a0581030bc40d2383b4a9b5bf0dda';
const HEAD = execFileSync('git', ['rev-parse', 'HEAD'], { cwd: ROOT, encoding: 'utf8' }).trim();
const CHECKPOINT = path.join(ROOT, 'docs/checkpoints/NOLANE-FORENSIC-RECOVERY-CHECKPOINT-3.json');
...
const suffixes = [
  ...
  'tool-router-model.json','tool-router-benchmark.json','tool-router-dataset-receipt.json','delivery-manifest.json','SHA256SUMS.txt',
];
...
const temp = await mkdtemp(path.join(os.tmpdir(), 'nolane-forensic-cp3-'));
...
await cp(path.join(ROOT, 'docs/superpowers/plans/2026-08-02-forensic-recovery-checkpoint-3.md'), path.join(changeDir, 'IMPLEMENTATION-PLAN.md'));
...
const evidenceFiles = [
  ...
  'models/tool-router/bootstrap-v1/model.json','models/tool-router/bootstrap-v1/benchmark.json','models/tool-router/bootstrap-v1/dataset-receipt.json',
  'docs/checkpoints/NOLANE-FORENSIC-RECOVERY-CHECKPOINT-3.md','docs/checkpoints/NOLANE-FORENSIC-RECOVERY-CHECKPOINT-3.json',
];
...
const copies = [
  ['docs/checkpoints/NOLANE-FORENSIC-RECOVERY-CHECKPOINT-3.md','CHECKPOINT.md'],['docs/checkpoints/NOLANE-FORENSIC-RECOVERY-CHECKPOINT-3.json','CHECKPOINT.json'],
  ...
  ['models/tool-router/bootstrap-v1/model.json','tool-router-model.json'],['models/tool-router/bootstrap-v1/benchmark.json','tool-router-benchmark.json'],['models/tool-router/bootstrap-v1/dataset-receipt.json','tool-router-dataset-receipt.json'],
];
...
const manifest = await createDeliveryManifest({ root: OUTPUT, artifactPaths, checkpoint: 'forensic-recovery-checkpoint.3', gitHead: HEAD });
