...
function verifyPipeline(value) {
  ...
  if (!/^[a-f0-9]{64}$/.test(String(receiptSha256 ?? '')) || canonicalSha256(base) !== receiptSha256) throw new Error('Checkpoint 8 pipeline evidence receipt mismatch');
  if (value.schema !== 'nolane.small-model.checkpoint-8-pipeline-evidence.v1' || value.status !== 'verified') throw new Error('Checkpoint 8 pipeline evidence is invalid');
  return value;
}

export async function generateForensicRecoveryCheckpoint8({ root = process.cwd(), writeOutputs = false } = {}) {
  ...
  const pipeline = verifyPipeline(await readJson(path.join(root, 'models/checkpoint-8/pipeline-evidence.json')));
  ...
  const verification = verifyForensicRecoveryCheckpoint8({
  ...
  const report = Object.freeze({
    schema: 'nolane.forensics.recovery-checkpoint-8.v1',
    product: 'Nolane Agent', productVersion: '5.0.0-beta.6', checkpoint: 'forensic-recovery-checkpoint.8', gitHead, verification,
    ...
    nextCheckpointEntryCriteria: [
      ...
      'Run AST and constraint skills on external repositories not packaged as Nolane fixtures.',
      'Expand from exact identifier codemods and finite-domain proofs to typed multi-file AST and property solvers.',
...
if (isMain) generateForensicRecoveryCheckpoint8({ writeOutputs: process.argv.includes('--write') })
